 ...
 * - 동기화의 사전적 의미는 타이밍을 맞춘다는 뜻으로, 한 쓰레드의 작업완료, 다음쓰레드의 작업시작을 맞추는 것이다.
 * - 내부적으로는 특정객체의 락을 획득/반환 처리하는 방식으로 이루어진다.
 *
 * 임계영역 critical-section
 * - 멀티쓰레드 환경에서 한번에 하나의 쓰레드만 접근해야 하는 공유자원
 * - 한 쓰레드의 작업이 시작되고, 마칠때까지 다른 쓰레드에 의해 방해받지 않도록 동기화 처리가 필요하다.
 * - 임계영역은 적을수록 멀티쓰레드 환경에서 성능이 좋다
 *
 * 교착상태 dead-lock
 * - 두 개 이상의 공유자원을 멀티쓰레드에서 배타적으로 확보한 상태를 얘기한다.
 * - 공유자원이 A, B일 때, "가" 쓰레드는 A를 확보, "나" 쓰레드는 B를 확보한 후, 서로우 공유자원을 기다리는 상태
 ...
 * - 공정성이 없는 방식이다. 기아상태의 쓰레드가 나올수 있다.
 * </pre>
 */
public class App {
    /**
