...
where
    a.menu_name = '한우딸기국밥'; -- 테이블 별칭을 생략가능하나 되도록 작성한다.
...
-- 카테고리(오른쪽)를 조회 (연결된 메뉴(왼쪽)가 없는 카테고리도 포함)
select
    *
from
    tbl_menu m right outer join tbl_category c
        on m.category_code = c.category_code; -- 25행 (21행 + 4행(식사,음료,디젙,,퓨전))
...
-- SELF JOIN
-- ----------------------------------
-- 자가조인, 같은 테이블을 좌우측에 두고 다른 테이블인양 조인.
