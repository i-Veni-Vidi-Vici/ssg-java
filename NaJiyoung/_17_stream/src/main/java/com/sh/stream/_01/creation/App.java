...
public class App {
    public static void main(String[] args) {
        ...
        System.out.println();
        System.out.println(list); // [1, 4, 2, 3, 5] - 원본은 변하지 않음

        int[] arr = {3, 2, 4, 1, 5};
        IntStream stream2 = Arrays.stream(arr);
        stream2.sorted().forEach((n) -> System.out.print(n + " "));
        System.out.println();
        System.out.println(Arrays.toString(arr)); // [3, 2, 4, 1, 5] - 원본은 변하지 않음

        Set<Integer> set = Set.of(3, 2, 4, 1, 5);
        set.stream().sorted().forEach(System.out::println);
        System.out.println(set); // [4, 3, 2, 1, 5] - 원본은 변하지 않음
        System.out.println();
