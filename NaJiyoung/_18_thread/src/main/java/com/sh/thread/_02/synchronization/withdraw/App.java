 ...
 * - 동기화의 사전적의미는 타이밍을 맞춘다는 뜻으로, 한 쓰레드의 작업완료, 다음쓰레드의 작업시작을 맞추는 것이다.
 * - 내부적으로는 특정객체의 락을 획득/반환 처리하는 방식으로 이루어진다.
 *
 * 임계영역 critical-section
 * - 멀티쓰레드 환경에서 한번에 하나의 쓰레드만 접근해야 하는 공유자원
 * - 한쓰레드의 작업이 시작되고, 마칠때까지 다른 쓰레드에 의해 방해받지 않도록 동기화처리가 필요하다.
 ...
 *
 * 동기화처리 (저수준)
 * 1. synchronized 메소드 : 해당객체가 임계영역으로 지정되고 락을 획득한 하나의 쓰레드만 접근 허용
