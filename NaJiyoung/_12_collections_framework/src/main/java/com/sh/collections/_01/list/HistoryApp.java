 ...
 * 3. 특정 url로 이동시 forwardStack이 비어있지 않다면, 제거한다. ex) forwardStack에 naver가 있지만 current에서 youtube로 직접 타이핑 쳐서 들어갔을 때, naver는 제거한다.
 * 4. 뒤로가기를 누르면, 현재 url은 forwardStack에 쌓이고, backwardStack의 url을 current에 대입한다.
...
public class HistoryApp {
    ...
    public void goToUrl(String url) {
        ...
        this.current = url;
        // 앞으로가기 - ex) forwardStack에 naver가 있지만 current에서 youtube로 직접 타이핑 쳐서 들어갔을 때, naver는 제거한다.
        if(!this.forwardStack.isEmpty()) // forwardStack에 naver 있는 경우
            this.forwardStack.clear(); // naver는 제거한다.
    }
