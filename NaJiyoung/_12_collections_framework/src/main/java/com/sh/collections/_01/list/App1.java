...
//        List<String> list = new ArrayList<String>();
        List<String> list = new ArrayList<>(); // 타입추론 (자바 1.7부터 가능)
...
//        System.out.println(list.get(5)); // java.lang.IndexOutOfBoundsException: Index 5 out of bounds for length 5
        ...
        // iterator객체를 통한 반복처리 (Iterable#iterator() -> list, set)
        ...
        // iterator객체는 요소를 소비한다. 모든 소비 후 hasNext()에서는 항상 false를 반환
        while(iter.hasNext()) { // 실행 X
            System.out.println("-----------------");
            String next = iter.next();
            System.out.println(next);
        }
