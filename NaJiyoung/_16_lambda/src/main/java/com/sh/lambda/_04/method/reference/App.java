...
//        Predicate<String> predicate = (txt) -> str.equals(txt);
        Predicate<String> predicate = str::equals; // 특정 객체를 기준으로 호출할 때
...
//        BiFunction<String, Integer, Person> personBiFunction = (name, age) -> new Person(name, age);
        BiFunction<String, Integer, Person> personFunction2 = Person::new;
        System.out.println(personFunction2.apply("홍길동", 33));

        // long, String, int 생성자 호출 - 매개인자 3개 이상은 람다 불가, 함수형 인터페이스 직접 작성
        TriFunction<Long, String, Integer, Person> triFunction = new TriFunction<>() {
            @Override
            public Person apply(long id, String name, int age) {
                return new Person(id, name, age);
            }
        };
        System.out.println(triFunction.apply(2L, "신사임당", 40));
