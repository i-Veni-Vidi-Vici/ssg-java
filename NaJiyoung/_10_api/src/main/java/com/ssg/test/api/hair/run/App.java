package com.ssg.test.api.hair.run;

import com.ssg.test.api.hair.model.dto.User;
...
public class App {
    public static void main(String[] args) {
        ...
        String data2 = data.substring(data.indexOf('1')); // 첫 줄을 제외하고 1부터 시작하는 문자열
        StringTokenizer token = new StringTokenizer(data2, "\n");
        User[] users = new User[token.countTokens()]; // 사용자 배열
        int i = 0; // 인덱스
        String birthday; // '/' 들어간 생일 문자열

        while (token.hasMoreTokens()) {
            String userStr = token.nextToken();
            String[] uStr = userStr.split(",");
            User u = new User();

            u.setId(Integer.parseInt(uStr[0]));
            u.setFirstName(uStr[1]);
            u.setLastName(uStr[2]);
            u.setEmail(uStr[3]);
            u.setGender(uStr[4]);

            // 생일 문자열이 null인지 아닌지 검사
            if (!"".equals(uStr[5])) {
                if(uStr[5].contains("/")) {
                    birthday = uStr[5].replace("/", "-");
                    u.setBirthday(LocalDate.parse(birthday));
                } else
                    u.setBirthday(LocalDate.parse(uStr[5]));
            } else
                u.setBirthday(null);

            u.setReservationDate(LocalDate.parse(uStr[6]));
            u.setReservationTime(LocalTime.parse(uStr[7], DateTimeFormatter.ofPattern("H:mm")));
            u.setCreatedAt(LocalDateTime.parse(uStr[8], DateTimeFormatter.ISO_LOCAL_DATE_TIME));
