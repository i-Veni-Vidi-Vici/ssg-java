 ...
 * 	- TypedQuery : 반환할 타입을 명확하게 지정하는 방식일 때 사용하며 쿼리 객체의 메소드 실행 결과로 지정한 타입이 반환 된다. (자주 사용)
...
public class JpqlTest {
    ...
    void test2() {
        // given
        // where조건절에서도 table이 아닌 entity기준으로 조회한다.
    ...
    void test3() {
        ...
        String jpql = "select m from Menu01 as m";
        // when
        Query query = this.entityManager.createQuery(jpql); // 타입 체크가 안됨
        List<Menu> menus = query.getResultList(); // Unchecked assignment: 'java.util.List' to 'java.util.List<com.sh.app._01.jpql.Menu>'
        /*
            select
                m1_0.menu_code,
                m1_0.category_code,
    ...
    void test4() {
        ...
        // then
        Set<Long> temp = new HashSet<>(categoryCodes); // 중복 제거, null도 하나의 요소로 들어감
