...
/**
 * Logging Framework
 * - System.out.println() 자바 기본 출력문을 사용할 때 문제를 해결하고 더나은 사용성 제공하는 기록용 프레임워크
 * - Log4j, Logback, java.util.logging, commons-logging등
 * - sysout으로 작성한 코드는 선택권이 무조건 출력되면서 성능 저하.
 * - Logging Framework들으 기본적으로 Logging Level을 지정해서 개발/운영모드를 세밀하게 제어할 수 있다.
 * - 출력타겟을 콘솔, 파일등을 다양하게 선택가능
 */
@Slf4j // 전역 변수를 만들 필요 없이 이거 하나 쓰면 로그를 쓸 수 있다
public class App {
    // Logback 타입을 제어
