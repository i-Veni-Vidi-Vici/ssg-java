...
public class App {
    public static void main(String[] args) {
        // new Member(usrname:String, role:MemberRole, gender:Gender)
        Member honggd = new Member("honggd", MemberRole.M, Gender.M);
//        Member honggd = new Member("honggd", Gender.M, MemberRole.M); // 혼용시 컴파일오류 유발 -> 실수하기 어렵다
...
//        MemberRole memberRole = new MemberRole(); // enum은 new 연산자 호출 불가
        System.out.println(memberRole1 == memberRole2); // true
        System.out.println();

        // 동일한 값을 가지고 있어도 enum 네임스페이스(디렉토리와 유사. 그룹핑 목적)가 달라서 구분할 수 있다.
        // 홍길동, 고길동과 같이 MemberRole.M과 Gender.M은 구분이 가능하다.

        // enum toString처리를 통해 실제 이름을 문자열로 반환할 수 있다.
...
//        printRole(Gender.M); // 컴파일 오류
        ...
        // 과목타입, 설명, 배점
        Subject subject1 = new Subject(SubjectType.JAVA, "자바는 어쩌구 저꺼구", 40);
    ...
    public static void printRole(MemberRole memberRole) {
