...
class LegacyMenuRepositoryTest {
     ...
     * - 스프링의 테스트에 @Transactional을 적용하면, 기본적으로 마지막에 rollback처리한다. (변경사항 무효화, update 쿼리 확인 불가)
     * - @Rollback(false)를 통해서 DML을 실제 적용할 수 있다. (rollback되지 않고, 쿼리도 확인 가능)
     * </pre>
     */
    @Transactional
    @Rollback(false) // 데이터 수정 확인 가능
    @Test
