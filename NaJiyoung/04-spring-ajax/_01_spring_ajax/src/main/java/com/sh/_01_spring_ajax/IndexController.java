...
public class IndexController {
    ...
    public String first(@RequestParam String username) {
        ...
        return "🍖🍖🍖 %s, 잘 지내니? 너에게 고기를 보낸다.".formatted(username); // ViewName이 아니라 응답할 데이터 그 자체
    }
    ...
    public String first2(@RequestParam String username) {
        log.debug("username = {}", username);
        return "🥩🥩🥩 %s, 잘 지내니? 저번 고기는 어땠니? 이번엔 소고기야.".formatted(username); // ViewName이 아니라 응답할 데이터 그 자체
    }
}
