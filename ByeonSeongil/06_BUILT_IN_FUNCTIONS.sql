...
# 문자열 연결연산
-- concat(값1, 값2, ...) "값1값2"
-- concat_ws(구분자, 값, 값2, ...) "값1구분자값2"

select concat('사자', '호랑이', '토끼'),
       concat_ws(',', '사자', '호랑이', '토끼');

-- 메뉴 조회 : "1. 떡볶이 (3000원)"
select concat(menu_code, '. ', menu_name, '(', menu_price, '원)') 메뉴
from tbl_menu;
...
select find_in_set('딸기', '사과,딸기,바나나'), -- 2
       find_in_set('아보카드', '사과,딸기,바나나');
-- 0

-- order by에서 특정행만 끌어올리기 위한 용도로 사용가능
select *
from tbl_menu
order by find_in_set(menu_code, '10') desc;
...
-- format(숫자, 소수점이하자리수) : 형식지정된 문자열 반환
-- 출력시 숫자는 오른쪽, 문자는 왼쪽 정렬됨
select format(12345456.1234453245, 3),
       format(12345456.1234453245, 0);

-- 메뉴명 가격조회 (살구쥬스 ￦1,200)
select concat('(', menu_name, ' ￦', format(menu_price, 0), ')') 메뉴
from tbl_menu;
...
-- 파인애플쥬스 -> 파인애플주우스
select insert(menu_name, instr(menu_name, '쥬스'), 2, '쥬우스')
from tbl_menu
where menu_name like '%쥬스%';

# 문자열 잘라내기
-- left(대상문자열, 개수) : 왼쪽부터 지정한 개수로 잘라내 반환
-- right(대상문자열, 개수): 오른족부터 지정한 개수로 잘라내 반환
select left('Hello world!', 3), -- Hel
       right('Hello world!', 3);
-- ld!

-- substring(대상문자열, 시작인덱스, 길이): 잘라낸 문자열 반환
select substring('안녕하세요, 반갑습니다', 8, 2), -- 인덱스 1부터 시작
       substring('안녕하세요, 반갑습니다', 8);
-- 끝까지

-- substring_index(대상문자열, 찾는 문자열, n번째) : 대상문자열에서 n번째 문자열 찾고, 그 이후를 버림처리후 반환
select substring_index('hong.gd@naver.com', '.', 1),
...
-- trim(방향 제거문자 from 대상문자열) : leading(좌)/trailing(우)/both 방향지정후 해당문자 제거
...
select replace(menu_name, '쥬스', '주으스')
from tbl_menu
where menu_name like '%쥬스%';
...
# 정규표현식 관련
select regexp_replace('123,456,789원', '[^0-9]', ''),
       regexp_substr('제 키는 188cm입니다.', '[0-9]+');
-- + 수량자 1개이상
...
select now(),                            -- 2024-04-16 09:50:23
       adddate(now(), 1),-- 2024-04-17 09:50:23
       adddate(now(), interval 1 day),   -- 2024-04-17 09:50:23
       adddate(now(), interval 1 month), -- 2024-05-16 09:50:23
       adddate(now(), interval 1 year); -- 2025-04-16 09:50:23

select adddate('2024-01-31', interval 1 month);-- 2024-02-29

select subdate(now(), 1),              -- 2024-04-15 10:06:35
       subdate(now(), interval 1 day), -- 2024-04-15 10:06:35
       adddate(now(), -1),             -- 2024-04-15 10:06:35
       adddate(now(), interval -1 day);
-- 2024-04-15 10:06:35

-- addtime(날짜시간, 시간)
-- adddate(날짜시간, interval n 단위)
select now(),
       addtime(now(), '1:2:3'),
       subtime(now(), '1:2:3'),
...
# 현재 날짜
-- curdate(), current_date(), current_date 모두 동일
...
# 현재 날짜/시각
-- now(), sysdate(), localtime(), localtime, localtimestamp(), localtimestamp 모두 동일
select now(),           -- 2024-04-16 10:18:27
       sysdate(),       -- 2024-04-16 10:18:27
       localtime,-- 2024-04-16 10:18:27
       localtimestamp(),-- 2024-04-16 10:18:27
       localtimestamp;
-- 2024-04-16 10:18:27
...
-- datediff(미래, 과거날짜)
-- timediff(미래시각, 과거시각)
select datediff('2024=-04=-20', now()), -- 4
       datediff(now(), '2024/04/20'); -- -4

select timediff('12:50:00', '10:30:00');
-- 02:20:00

-- 수료일 d-day 계산쿼리
select datediff('2024/09/04', now());
-- 141

# 날짜정보 관련
-- dayofweek(날짜) : 요일에 해당하는 인덱스 반환 (일요일 -1 ~ 토요일 ~7)
-- monthname(날짜) : 월이름반환
-- dayofyear(날짜) : 해당년도 몇번재 날인지 반환
select dayofweek(now()), -- 3 (화요일)
       monthname(now()), -- April
       dayofyear(now());
-- 107
...
-- 다음달 말일은?
select last_day(now() + interval 1 month), -- 2024-05-31
       last_day('2024-05-31' + interval 1 month); -- 2024-06-30

select '2024-05-31' + interval 1 month;
-- 2024-06-30
...
select period_add(202404, 18), -- 202510
...
-- 날짜시각을 특정형식으로 출력하기
-- date_format(날짜/시각, 형식문자열) : 지정된 형식의 문자열을 반환
-- 형식
-- %Y %y 연도4자리/연도2자리
-- %m %M 월(숫자)/ 월(영단어)
-- %d 일
...
select date_format(now(), '%y/%m/%d'),
       ...
       date_format(now(), '%Y년 %m월 %d일 (%W %a) %p');
...
select '1' + 'zzz'; -- 1 숫자로 변환불가한 문자열은 0으로 변환후 1 + 0 계산
select concat(1, 2, 3, '원'); -- 123원 숫자가 문자열로 자동변환후 연결 '1' '2' '3' '원'
select adddate('2024-04-01', 1) -- 2024-04-02 date형 먼저 변환후 1일 더하기 처리

select '1' + '3만원'; -- 1 + 3 숫자로 시작되는 문자열은 숮자부분만 변환
select '1' + '내가가진3만원'; -- 1 + 0

select 3 > '오'; -- 1(true) '오'는 0으로 암묵적변환후 크기비교, true -->1
select 3 > '5오오오오';
-- 0(false) '5오오오오'는 5로 변환 후 크기 비교 false -> 0

-- ----------------------
-- 기타 함수
-- ----------------------
# null처리함수
-- ifnull(값, null일때값) : 값이 null이 아니면, 값을 사용. 값이 null이면 null일때 값 사용
select ifnull('abc', 'xyz'), -- abc
       ifnull(null, 'xyz');
-- xyz
...
select menu_name, menu_price, if(orderable_status = 'y', '주문가능', '주문불가') 주문가능여부
from tbl_menu;

-- isnull(값) null여부 반환
select isnull('zzz'), -- 0(false)
       isnull(null);
-- 1(true)
...
select menu_name,
       ...
       case
           ...
           when menu_price between 5000 and 10000 then '적당한것'
           when menu_price between 10000 and 20000 then '좀비싼것'
           else '겁나비싼거'
           end menu_label
from tbl_menu;
...
select menu_name,
       menu_price,
       case
           when category_code is null then '없음'
           else category_code
           end category_code,
...
# avg
-- 특정 컬럼의 평균값 반환
select avg(menu_price),             -- 10704.5455
       truncate(avg(menu_price), 1) -- 10704.5
from tbl_menu;

# count
-- 특정컬럼의 값의 개수를 세어 반환. null값은 처리하지 않는다.
select count(*),            -- 22 * 행(모든 컬럼)의 수
       count(category_code) -- 21 찰순대쥬스의 catrgory_code는 null이므로 제외.
from tbl_menu;
...
# min / max
-- 특정 컬럼의 최소/최대값을 반환
-- 숫자/문자(사전등재순맨먼저 ~ 맨뒤)/날짜(과거(작은것) ~ 미래)에 대해서 적용가능
