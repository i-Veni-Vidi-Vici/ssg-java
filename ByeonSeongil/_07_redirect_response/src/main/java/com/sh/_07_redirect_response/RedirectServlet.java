...
//@WebServlet(value = {"/redirect","/redirect2"})
//@WebServlet({"/redirect"})
@WebServlet("/redirect")
...
//        response.sendRedirect("https://naver.com"); // Location 주소값 설정
        ...
        // 매 요청마다 Request, Response객체가 만들어진다. 요청간 Request객체를 공유할 수 없다.
        // 만약 요청간의 데이터등을 공유하려면, Session객체를 이용해야 한다.
        System.out.println(request); // org.apache.catalina.connector.RequestFacade@651ccc33

//        response.sendRedirect("another"); // 상대주소 (/07_redirect_response/redirect -> /07_redirect_response/another)
        response.sendRedirect(request.getContextPath()+"/another"); // 절대주소 (브라우저에서 요청할 주소이므로 application context를 포함)
    }
