 ...
 *
 * - 단점
 * 1. 부모클래스 수정시 자식클래스가 정상적으로 작동하는 예측이 힘들다.
 * 2. 부모클래스 수정이 어렵다. -> 유지보수 악영향
 * 3. 부모클래스의 일부기능만 필요한 경우, 부분상속은 불가능 (불필요한 기능까지 상속)
 *
 *  - 무분별한 상속 지양, Is-a상속관계가 성립할때만 상속을 적용하는 것이 좋다.
 *  - 객체지향에서 객체는 메세지에 대해 응답할 책임을 가진다. 이 책임의 규모가 적절해야한다.
 *  - 상속을 통해서 책임믈 역할의 관점(대체가능성)으로 볼수 있어야 한다.
 *  - 부모클래스의 추상화된 역할은 모든 자식클래스가 동일하게 수행(책임)할수 있다.
 *
 * </pre>
 */
public class App {
