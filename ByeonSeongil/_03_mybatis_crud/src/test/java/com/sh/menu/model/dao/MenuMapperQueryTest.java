...
class MenuMapperQueryTest {
    ...
    void findAll() {
        ...
        assertThat(list)
                ...
                .allSatisfy((menu) -> {
                    ...
                    assertThat(menu.getOrderableStatus().toUpperCase()).containsAnyOf("Y", "N");
                    // nullable한 컬럼은 검증 제외
    ...
    void findByCategoryCode() {
        // given
        // when
        List<MenuDto> list = menuMapper.findByCategoryCode(3);
        // then
        assertThat(list)
                .isNotNull()
                .isNotEmpty()
                .allSatisfy((menu) -> {
                    ...
                    assertThat(menu.getMenuName()).isNotNull();
                    assertThat(menu.getMenuPrice()).isNotZero().isPositive();
                    assertThat(menu.getCategoryCode()).isEqualTo(3);
                    assertThat(menu.getOrderableStatus().toUpperCase()).containsAnyOf("Y", "N");
                    // nullable한 컬럼은 검증 제외
                    assertThat(menu.getOrderableStatus()).satisfiesAnyOf(
                            (status) -> "Y".equals(status),
    ...
    @Test
    void findByNonexistentCategoryCode() {
        // given
        // when
        List<MenuDto> list = menuMapper.findByCategoryCode(3333333);
        // then
        assertThat(list)
                .isNotNull()
                .isEmpty();
