 ...
 * - 현실세계의 모든 사건은 객체와의 상호작용이란 것에서 힌트를 얻어 프로그래밍 세계로 옮겨 놓은 것.
 ...
 * - 송신자객체가 메시지를 보내고(메서드 호출), 수신자 객체는 메세지의 내용(매개인자) 따라 특정작업을 수행
 * - 수행한 작업결과를 답장(리턴값)으로 보낸다.
 *
 * 운전 프로그램
 * 0. 요구사항 정리
 * - 운전프로그램을 만들어주세요.
 * - 운전자는 시동걸기, 악셀을 밝거나, 브레이크를 밟거나, 시동을 끌 수 있어야 합니다.
 * - 자동차는 시동걸기, 가속하거나, 시동이 꺼질 수 있어야 합니다.
 * - 자동차는 처음엔 대기상태로 있습니다.
 ...
 * 3. 클래스 설계
 * - 사용자클래스 (실행클래스)
 * - 운전자클래스 (시동켜기, 시동끄기, 가속, 감속 메서드)
 * - 자동차클래스 (시동켜기, 시동끄기, 가속, 감속| 시동상태, 속도 상태 값)
 *
