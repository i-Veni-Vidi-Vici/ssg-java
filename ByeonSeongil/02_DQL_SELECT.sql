...
-- 조회결과로 0행이상의 ResultSet(결과집합)이라고
...
-- select 출력할 컬럼 명시
-- from 조회대상이 되는 테이블
-- where 결과집합에 포함될 행의 조건 기술. (각행별로 조건검사후 true인 행만 포함)
-- order by 정렬조건 특정 컬럼기준 오름차순/내림차순 지원

select * -- 모든 컬럼
from tbl_menu;

select menu_code, menu_name, menu_name
from tbl_menu;
...
# SELECT 산술연산
-- 부가세 포함 가격
select menu_name, menu_price + (menu_price * 0.1) "부가세 포함 가격"
from tbl_menu;
...
-- 여러 칼럼에 DISTINCT를 적용하면, 컬럼값을 묶어 중복값을 제거한다.
