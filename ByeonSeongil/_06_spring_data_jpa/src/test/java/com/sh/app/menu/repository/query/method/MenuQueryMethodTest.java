package com.sh.app.menu.repository.query.method;
...
//        List<Menu> menus =menuRepository.findAllByOrderByMenuNameAsc();
        ...
        menus.forEach(System.out::println);
        // then
        // Comparator 비교 기준 람다 전달
//        assertThat(menus).isSortedAccordingTo((m1, m2) -> m1.getMenuName().compareTo(m2.getMenuName()));
        assertThat(menus).isSortedAccordingTo((m1, m2) -> m1.getMenuName().compareTo(m1.getMenuName()));
    }
    ...
    void test4() {
        ...
        menus.forEach(System.out::println);
        // then
        assertThat(menus).isSortedAccordingTo((m1, m2) -> m2.getMenuPrice() - m1.getMenuPrice())
     ...
     * - Page<T> 컨텐츠, 총페이지 수, 총 개수, 첫번째/마지막 페이지 여부등
     */
    @Test
    @DisplayName("페이징 처리")
    void test5() {
        // given
        int pageNumber = 0; // 현재페이지(0부터 시작)
        int pageSize = 10; // 한페이지당 노출할 컨텐츠 수
        Pageable pageable = PageRequest.of(pageNumber, pageSize);
        // when
//        Page<Menu> menus = menuRepository.findAll(pageable); // 이미 제공
        Page<Menu> menuPage = menuRepository.findByOrderableStatus(OrderableStatus.Y, pageable); // 이미 제공
        System.out.println(menuPage); // Page 1 of 2 containing com.sh.app.menu.entity.Menu instances
