...
public class CRUDTest {
    ...
    void test3() {
        ...
        try {
            entityManager.persist(menu); // 영속성컨텍스트에 menu 엔티티객체를 저장 (@GeneratedValue 설정시 insert쿼리는 바로 실행, PK를 알아야 영속성 컨텍스트에 저장가능하다.)
    ...
    void test4() {
        ...
        try {
            ...
            System.out.println("영속성컨텍스트 메뉴 객체의 정보가 변경되었습니다.");
            transaction.commit();
        } catch (Exception e) {
            transaction.rollback();
            e.printStackTrace();
        ...
        assertThat(menu).isEqualTo(menu2);
    }
