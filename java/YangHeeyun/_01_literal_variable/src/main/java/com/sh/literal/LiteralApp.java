package com.sh.literal;

public class LiteralApp {
    /**
     * 주석이름: asterisk
     *
     * Literal(값)
     * @param args
     */

    public static void main(String[] args) {
        //1. 정수
        System.out.println(123);
        System.out.println(-123);
        //2. 실수
        System.out.println(12.345);
        //3. 논리값(참/거짓)
        System.out.println(true); //True 안됨. 소문자로
        System.out.println(false);
        //4. 문자(1개) -홑따옴표로 감싸서 표현
        System.out.println('a'); // sout하면 바로 출력문 써짐
        //System.out.println('ab'); // compile error - 자바문법을 제대로 작성하지 못한 경우
        //System.out.println(''); // 문자 0개는 오류
        System.out.println('1'); //숫자여도 ''로 감싸져있어 문자로 처리
        //5. 문자열(n개)
        System.out.println("안녕");
        System.out.println("");// 빈 문자열 표현 가능
        System.out.println("a"); // 문자 1개 표현 가능
    }
}
