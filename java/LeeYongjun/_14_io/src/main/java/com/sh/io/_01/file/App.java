...
public class App {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(file); // LeeYongjun\_14_io\test.txt 파일경로로 toString 오버라이드 되어 있음.
        System.out.println(file.getParent()); // LeeYongjun\_14_io
        System.out.println(file.getName()); // test.txt
        System.out.println(file.exists()); // false -> true
        System.out.println(file.isFile()); // false -> true
        System.out.println(file.isDirectory()); // false
        System.out.println(file.length()); // 0 -> 31 byte
        ...
        try {
            ...
            // 절대경료 : 루트디렉토리부터 시작되는 경로 윈도우 C:/~,D:/~, 맥/리눅스/유닉스 /User/honggd/~
            // 상대경로 : 현재파일 기준으로 작성된 경로 현재디렉토리 ./~, 부모디렉토리 ../~
            System.out.println(file.getAbsolutePath()); // C:\Workspaces\java_workspace\LeeYongjun\_14_io\test.txt
        } catch (IOException e) {
            e.printStackTrace();
        }
