...
/**
 * <pre>
 * - anyMatch(Predicate<T>):boolean 하나라도 Predicate람다식의 결과가 true라면, 최종 true 반환
 * - allMatch(Predicate<T>):boolean 모든요소의 Predicate람다식의 결과가 true라면, 최종 true 반환
 * - noneMatch(Predicate<T>):boolean 모든요소의 Predicate람다식의 결과가 false라면, 최종 true 반환
 * </pre>
 */
public class App4 {
    public static void main(String[] args) {
        ...
        // 이름글자 수가 3글자보다 많은 이웃이 있는지 검사
        System.out.println(supplier.get().allMatch(person -> person.getName().length() > 3));
