...
/**
* <pre>
 *  - 키오스크 의 수신 메세지 목록 (git 허브라면 이슈목록)
 *     메뉴제공하라,
 *          사용자선택에 따라 주문을 생성하라,
 *          생성된 주문을 보여달라,
 *          주문을 바리스타에게 전달하라
...
public class Kiosk {
    ...
    public void menu() {
        ...
        while (true) {
            ...
            String option = null;
            int count = 0;
            Order order = null; // 스위치문안에서 선언된 변수명은 바깥에서 선언
...
//
                    ...
                    option = optionMenu();
                    count = inputCount();
                    order = createOrder(menuName,option,count); // 전달
                    if(order != null){
    ...
    public Order createOrder(String menuName, String option, int count) {
        ...
        return confirmed ? order : null; // 참조형의 기본값 null!
    }
    ...
    public boolean confirmOrder(Order order){
        ...
        return sc.next().equals("1"); // 1이면 true 나머지 false
    }
