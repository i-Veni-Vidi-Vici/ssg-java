 ...
 *
 * - 무분별한 상속을 지양하고, IS-a 상속관계가 성렵될 때만 상속을 적용하는 것이 좋다.
 * - 객체지향에서 객체는 메세지에 대해 응답할 책임을 가진다. 이 책임의 규모가 적절해야한다.
 * - 상속을 통해서 책임을 역할의 관점으로 볼 수 있어야 한다.
 * - 부모클래스의  추상화된 역할은 모든 자식클래스가 동일한 책임을 수행할 수 있다.
 * 추상화 : 핵심적인 개념 또는 기능을 간추려 내는 것
 *
 * 책임은 동일(메소드가 같음) 그 안에 구형방식은 다름
 *
 *
 * </pre>
 *
...
//        car.honk(); // 클락션 못 울림
        ...
        //소방차
//        FireEngine fireEngine = new FireEngine(); // fireEngine의 기본생성자
//        fireEngine.run();
//        fireEngine.honk();
//        fireEngine.stop();
//        fireEngine.honk();
//        fireEngine.extinguish();
        ...
        racingCar.run();
        racingCar.honk(); // 불 필요 하지만 가지고 있는 기능
        racingCar.stop();
        racingCar.honk();
