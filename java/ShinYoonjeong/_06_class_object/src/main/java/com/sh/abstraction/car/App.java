 ...
 * 0. 요구사항 정리
 *  - 운전 프로그램을 만들어주세요.
 *  - 운전자는 시동 걸기, 악셀을 밟거나, 브레이크를 밟거나, 시동을 끌 수 있어야 한다.
 *  - 자동차는 시동 걸기, 가속하거나, 감속을 하거나, 시동이 꺼질 수 있어야 한다.
 *  - 자동차는 처음엔 대기 상태로 있다.
 *  - 운전자에 의해 시동이 걸리고, 이미 시동이 걸려 있다면 다시 걸 수 없어야 한다.
 *  - 운전자가 악셀을 밟으면 시동이 걸린 상태에서만 가속할 수 있다.
 *  악셀을 밟을때마다 10km/h씩 가속되어야 한다
 *  - 운전자가 브레이크를 밟으면, 움직이고 있는 상태에서 10km/h씩 감속된다.
 *  0km/h 정지된 상태에서는 더 이상 감속할 수 없다.
 *  - 운전자가 시동을 끄면 더 이상 차는 가속/감속할 수 없다.
 ...
 *  - 시동켜기 메뉴 선택 -> 운전자 시동켜기 ->자동차 사동켜기
 *  - 가속 메뉴 선택 -> 운전자 가속 -> 자동차 가속(현대 속도를 변경)
 *  - 감속 메뉴 선택 -> 운전자 감속 -> 자동차 감속(현재 속도를 변경)
 *  - 시동끄기 메뉴 -> 운전자 시동끄기 -> 자동차 시동끄기
 * 3. 클래스 설계
 * - 사용자 클래스 (실행 클래스)
 * - 운전자 클래스 (시동켜기, 시동끄기, 가속, 감속 메소드)
 * - 자동차 클래스 (시동켜기, 시동끄기, 가속, 감속 메소드| 시동상태, 속도 상태값)
 */
public class App {
