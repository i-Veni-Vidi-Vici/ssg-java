...
/**
 * 다형성 Polymorphism
 * - 하나의 인스턴스가 여러가지 타입을 가질 수 있다.
 * - 인스턴스 참조 변수의 타입을 부모 타입으로 변환해 제어할 수 있다.
 * - 부모 타입의 참조 변수로 제어할 수 있다.
 * - 제어되는 타입별로 다르게 작동할 수 있다.
 * - 상속을 기반으로 하는 기술 (부모의 타입을 물려받으므로, 해당 타입으로 제어가 가능한 것)
 *
 * - 다형성으로 통해서 여러 객체를 동일한 타입으로 제어할 수 있어 유지보수성, 생산성이 증가한다.
 * - 상속 관계에 있는 여러 객체들은 동이란 메세지를 수신할 수 있는데, 이를 객체별로 다르게 처리할 수 있다.
...
public class App {
     ...
     * 동적바인딩
     * - 상속/다형성이 적용되고, 해당 메소드를 오버라이드했다면, 호출시 실행 시간에 객체 기준으로 메소드를 다시 바인딩하는 것
     *
     *
     */
    private void Test4() {
     ...
     */
    private void Test2() {
        Animal animal1 =(Animal) new Rabbit(); // Rabbit -> Animal upcasting
