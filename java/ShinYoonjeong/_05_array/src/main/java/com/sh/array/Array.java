 ...
 * - 변수를 이름으로 참조할 수 있다.
 * - 변수에 초기값을 제공하지 않는다. 반드시 값 대입후 사용해야 한다.
 * - heap 객체 영역 (new 연산자로 생성된 객체의 저장공간)
 * - 객체를 이름이 아닌 주소값으로 참조할 수 있다.
 * - 변수에 타입별 기본값이 초기값으로 설정한다. (정수 0, 실수 0.0, 논리 false, 문자 null문자(0), 참조형 null)
 * - static 정적 자원 저장공간 (공유 목적)
 */
public class Array {
    public void test(){
        // 1. 선언
    ...
    public void test2()
    {
        ...
        darr = null; // 값 없음을 의미하는 자바 키워드. 참조형변수에만 사용가능
    ...
    public void test5(){
        String[] names = new String[]{"윤정", "민상", "용준", "도환"};
        // 반복문을 통해 출력
        for(int i=0;i<names.length;i++) // magic number 대신 변수를 사용하자. 의도파악이 쉽고 유지보수가 쉽다.
        {
