 ...
 * -객체는 적절한 책임을 가질수 있게 클래스 단위로 잘 분리해 작성해야 한다.
 *
 * 자바 프로그래밍에서의 객체란?
 ...
 * -송신자객체가 메세지를 보내고, 수신자 객체는 메세지의 내용에 따라 특정작업을 수행한다.
 * -수행한 작업 결과를 답장(리턴값) 보낸다.
 * -
 * -운전프로그램
 * 0.요구사항 정리
 * -운전프로그램을 만들어주세요
 * -운전자는 시동걸기. 악셀을 밟거나 .브레이크를 밟거나. 시동을 끌 수 있어야 합니다.
 * 자동차는 시동걸기. 가속하거나 .감속을 하거나. 시동이 꺼질수 있어야 합니다.
 * 자동차는 처음엔 대기상태로 있습니다.
 * 자동차는 운전자에 의해 시동이 걸리고 이미 시동이 걸려 있다면 다시 걸 수 없어야 합니다.
 * 운전자가 악셀을 밟으면 시동이 걸린상태에서만 가속할 수 있습니다.
 * 악셀을 밟을때마다 10km씩 가속되어야합니다.
 * 운전자가 브레이크를 밟으면 움직이고 있는 상태에서 10씩 감속합니다.
 * 정지된상태에서는 더이상 감속할 수 없습니다.
 * 자동차가 달ㄹ리는 중에는 시동을 끌 수 없습니다.
 * 1.객체 도출
 * -자동차
 * -운전자
 * -사용자
 ...
 * 2.객체간의 상호작용
 * -사용자선택 -> 운전자객체 -> 자동차객체
 * -시동켜기메뉴선택 -> 운전자 시동켜기 메소드 -> 자동차에 시동켜기.
 * -가속메뉴선택 -> 운전자 가속 -> 자동차 가속(현재속도를 변경)
 * -감속메뉴선택 -> 운전자 감속 -> 자동차 감속(현재속도를 변경)
 * -시동끄기메뉴선택 -> 운전자 시동끄기 -> 자동차에 시동끄기
 * 3.클래스 설계
 *-사용자클래스(실행클래스)
