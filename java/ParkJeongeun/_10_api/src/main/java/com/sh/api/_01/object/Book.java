...
public class Book {
    ...
    /**
     * 어떤 필드를 기준으로 동등비교할지 정해야 함
     * - id, title, author, price 전부 또는 일부를 대상으로 비교 가능
     * - equals와 hashCode 오버라이드시 동일한 필드 기준으로 작성되어야 함
     * @return
     */
//    @Override
...
//            return false;
//        // 필드 비교
//        Book other = (Book) obj; // 1. down casting
//        // id 필드(기본형) 비교
//        if (this.id != other.id)
//            return false;
//        // title 필드(참조형) 비교
//        if (this.title == null){
//            // 현재객체의 title이 null인데 other객체의 null이 아닌 경우 : 필드값이 같다고 할 수 없기때문에 false
//            if (other.title != null)
//                return false;
//        } else {
//            // 이미 오버라이드된 String#equals 대신 호출
...
//            return false;
//
//        return true; // 위에 모든 if 조건에 부합하지 않으면 문자열이 같음(true)을 반환
//    }
     ...
     * - 필드별로 특정 수를 곱해 고유값을 생성 (31이라는 소수(1과 자기자신만 있는)를 관례적으로 사용)
     * @return
     */
//    @Override
//    public int hashCode(){
