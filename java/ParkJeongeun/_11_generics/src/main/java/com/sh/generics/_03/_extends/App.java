 ...
 * <pre>
 * Generics 상한/하한 제한
 * - A extends B : B를 상속한 A 타입만 가능 (상한 제한)
 * - A super B : B의 부모클래스인 A 타입만 가능 (하한 제한)
 * 주로 상한 제한 많이 씀
 * </pre>
 */
public class App {
    public static void main(String[] args) {
        //RabbitFarm<T> // <T>자리에 <Animal>이 왔기때문에 Animal타입이면 다 가능
...
//
//      // RabbitFarm클래스에서 public class RabbitFarm <T> 이라고 썼을때는 이것도 가능
//        RabbitFarm<LocalDate> localDate = new RabbitFarm<>(LocalDate.now());

        // RabbitFarm<T extends Rabbit>로 변경 후
        ...
        rabbit1.cry(); // bunny의 메소드 실행(상속/다형성 & 오버라이드 -> 동적바인딩)
...
//        AnimalFarm<LocalDate> localDateAnimalFarm = new AnimalFarm<>(); // LocalDate은 Animal을 상속하지 않았기때문에 안됨
    }
}
