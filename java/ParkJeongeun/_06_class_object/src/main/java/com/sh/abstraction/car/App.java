 ...
 * 0. 요구사항 정리
 *   - 운전프로그램을 만들어주세요.
 *   - 운전자는 시동걸기, 악셀을 밟거나, 브레이크를 밟거나, 시동을 끌 수 있어야 함
 *   - 자동차는 시동걸기, 가속하거나, 감속을 하거나, 시동이 꺼질 수 있어야 함
 *   - 자동차는 처음엔 대기상태로 있음
 *   - 운전자에 의해 시동이 걸리고, 이미 시동이 걸려있다면 다시 걸 수 없어야 함
 *   - 운전자가 악셀을 밟으면 시동이 걸린 상태에서만 가속 가능
 *     악셀을 밟을때마다 10km/h씩 가속되어야 함
 *   - 운전자가 브레이크를 밟으면 움직이고 있는 상태에서 10km/h씩 감속됨
 *     정지된 상태(0km/h)에서는 더이상 감속할 수 없음
 *   - 운전자가 시동을 끄면 더이상 차는 가속/감속 할 수 없음
 *   - 자동차가 달리는 중에는 시동을 끌 수 없음
 * 1. 객체 도출
 *   - 자동차
 *   - 운전자
 *   - 사용자(운전자를 제어함)
