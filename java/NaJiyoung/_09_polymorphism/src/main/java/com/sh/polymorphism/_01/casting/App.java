...
//        app.test3();
     ...
     * 동적바인딩
     * - 상속/다형성이 적용되고, 해당 메소드를 오버라이드 했다면, 호출시 실행시간에 객체 기준으로 메소드를 다시 바인딩하는 것
     * </pre>
     */
    private void test4() {
...
//        Tiger tiger = (Tiger) animal;   // ClassCastException 예외 발생!
     ...
     */
    private void test2() {
        Animal animal1 = new Rabbit();  // Rabbit -> Animal upcasting, (Animal) 생략가능
        // 위아래 같은 코드
...
//        Rabbit rabbit = new Animal();
        ...
        ((Rabbit) animal2).jump();  // Animal -> Rabbit downcasting 명시적, 생략불가
        Rabbit rabbit2 = (Rabbit) animal2;

        Animal animal3 = new Tiger();   // up casting
        ((Tiger) animal3).bite();   // down casting
    }
