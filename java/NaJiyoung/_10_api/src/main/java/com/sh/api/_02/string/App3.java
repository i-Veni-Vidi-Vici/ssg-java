...
//        String[] arr3 = data3.split("/");   // 마지막 요소 처리안됨 ["300", "이순신", "경기도"]
        String[] arr3 = data3.split("/", -1);   // 요소개수 제한없음 ["300", "이순신", "경기도", ""]
        System.out.println(Arrays.toString(arr3));

        System.out.println(Arrays.toString(data3.split("/", 2)));   // 요소 개수 2개 ["300", "이순신/경기도/"]
        System.out.println();

        // 2. StringTokenizer
        // - CSV데이터를 토큰으로 잘라내어 관리하는 객체
        StringTokenizer tokenizer = new StringTokenizer(data1, "/");
//        System.out.println(tokenizer);  // java.util.StringTokenizer@36baf30c <-Object#toString, 오버라이드 안됨
        // 반복처리를 통해 토큰을 열람(소비) - 자신의 객체에서 제거
        System.out.println("현재 토큰수 : " + tokenizer.countTokens());  // 4
        ...
        System.out.println("현재 토큰수 : " + tokenizer2.countTokens()); // 3
        ...
        System.out.println("현재 토큰수 : " + tokenizer3.countTokens()); // 3
