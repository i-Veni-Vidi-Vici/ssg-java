 ...
 *  - LinkedList
 *
 *  ArrayList 객체 안에는 데이터를 관리할 배열을 갖고 있다.
 *
 * </pre>
 */
...
//        Collection list3 = new ArrayList(); // 다형성
        ...
        List<String> list = new ArrayList<>(); // 타입추론 (1.7 버전부터)
...
//        list.add(123);   위에 String 넣어서 int는 안되는거!
//        list.add(123);
//        list.add(123.456);
//        list.add(LocalDate.now()); // list는 저장한 순서 그대로!
...
//        System.out.println(list.get(5)); // error
        ...
        for(int i = 0; i < list.size(); i++){
            System.out.println("for문 -> " + i + " - " + list.get(i));
        }

        for(String s : list){
            System.out.println("for each문 -> " + s);
        }
        ...
        // iterator 객체는 요소를 소비한다. 모두 소비후 hasNext()에서는 항상 false 반환
        while(iter.hasNext()){
            System.out.println("--------------------");
            String next = iter.next();
            System.out.println(next);
