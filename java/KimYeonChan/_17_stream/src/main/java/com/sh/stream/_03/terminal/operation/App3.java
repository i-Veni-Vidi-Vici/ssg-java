 ...
 *  - 스트림연산은 원본컬렉션, 배열 등을 수정하지 않으므로, collect 을 통해 결과를 받아야 한다.
 */
public class App3 {
    public static void main(String[] args) {
        ...
        Set<Integer> intSet = List.of(1, 2, 3, 1, 2, 3, 1, 2, 3, 10).stream()
        ...
        // 배열 변환
        Object[] arr = List.of(1, 2, 3, 2, 4, 1, 2, 4).stream()
                .toArray();
        System.out.println(Arrays.toString(arr));

        // 문자열 조합
        String names = List.of("홍길동", "신사임당", "홍길동", "이순신", "홍길동").stream()
                .collect(Collectors.joining(" | ", "<", ">")); // 구분자, 접두사, 접미사
        System.out.println(names);

        // 이웃들의 이름만 출력하세요. (동명은 한번만 출력되고, 오름차순 정렬되어 있어야 한다.)
