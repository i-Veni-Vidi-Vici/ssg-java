...
public class App {
    public static void main(String[] args) {
        ...
        StringTokenizer dataStrToken = new StringTokenizer(data, "\n");
        dataStrToken.nextToken();
        User[] user = new User[dataStrToken.countTokens()];
        for (int i = 0; i < user.length; i++){
            String[] userData = dataStrToken.nextToken().split(",", -1);
            int id = Integer.parseInt(userData[0]);
            String firstName = userData[1];
            String lastName = userData[2];
            String email= userData[3];
            String gender = userData[4];
            LocalDate birthday = userData[5].isEmpty() ? null : LocalDate.parse(userData[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate reservationDate = LocalDate.parse(userData[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime reservationTime = LocalTime.parse(userData[7], DateTimeFormatter.ofPattern("H:mm"));
            LocalDateTime createdAt = LocalDateTime.parse(userData[8], DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));
            user[i] = new User(id, firstName, lastName, email, gender, birthday, reservationDate, reservationTime, createdAt);
