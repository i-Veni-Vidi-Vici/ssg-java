package com.sh.inheritance._02._extends;

public class FireEngine extends Car {

    /**
     * <pre>
     *     상속 시에는 부모 생성자를 자동으로 먼저 호출한다.
     *     - this() 클래스 자신의 다른 생성자 호출
     *     - super() 부모 클래스의 기본 생성자 호출
     *     - 딱 한번, 맨 첫줄에서만 호출 가능
     * </pre>
     */
    public FireEngine() {
        super();            // 샹략 가능, 컴파일러가 자동으로 작성해준다.
        System.out.println("FireEngine 기본생성자 호출!!!");
    }

    @Override
    public void honk() {
//        if(this.isRunning) {  // private 필드는 상속은 되지만, Car 이외의 클래스에서는 직접 접근할 수 없다.
                                // 2. 부모 클래스의 필드 접근 제한자를 protected로 설정
        if(isRunning()) {       // 1. public getter 메소드를 이용해서 접근 가능
            System.out.println("빵빵빵빵빵빵~~~~~~~~~~~");
        }
        else {
            System.out.println("비켜주세요~ 소방차가 지나가야합니다~~~ 빵빵빵빵빵빵~~~~~~~~~~~");

        }
    }

    /**
     * <pre>
     *     확장 기능
     *     - Car에는 없지만 FireEngine에는 제공하는 것
     *
     * </pre>
     */
    public void extinguish() {
        System.out.println("불을 끄는 중입니다................취이이이이이잌~~~~~~~~~");
    }
}
