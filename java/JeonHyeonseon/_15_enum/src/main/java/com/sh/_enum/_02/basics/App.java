...
//        Member honggd = new Member("honggd", Gender.M,  MemberRole.M); // 혼용시 컴파일 오류 유발 -> 실수하기 어렵다.
        ...
        // 동등 비교시 무조건 다르다.
        System.out.println(MemberRole.M.equals(Gender.M)); // 타입이 다르므로 false
        // enum은 단일인스턴스를 사용한다.(싱글턴) -> 동일비교(주소값 비교)가 가능
        MemberRole memberRole1 =  MemberRole.M;
        MemberRole memberRole2 =  MemberRole.M;
//        MemberRole memberRole = new MemberRole(); // new 연산자 호출 불가
        System.out.println(memberRole1 == memberRole2); // true, 주소값이 같다.
        // 동일한 값을 가지고 있어도 enum 네임스페이스(디렉토리와 유사, 그룹핑 목적)가 달라서 구분할 수 있다.
        // 홍길동, 고길동와 같이 MemberRole.M과 Gender.M은 구분이 가능하다. enum클래스가 네임스페이스 역할을 한다.
        System.out.println();
...
//        printRole(Gender.M); // 컴파일 오류
        ...
        // 과목타입, 설명, 배점
        Subject subject1 = new Subject(SubjectType.JAVA, "자바는 어렵다.", 40);
