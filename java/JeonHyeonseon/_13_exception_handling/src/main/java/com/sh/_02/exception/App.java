 ...
 *  - 적절한 예외처리를 통해서 다시 정상흐름으로 복구할 수 있는 미약한 오류이다.
 *
 * 예외흐름
 *  1. 예외 던지기 throw new NullPointerException();
 *  2. 예외 제어
 *   - 예외회피(위임) : 메소드선언에 throws구문을 작성하는 것. ex) public void test() throws SomeException
 *   - 예외처리 : try..catch절을 통해서 예외발생시 처리구문을 작성하는 것. (정상흐름으로 복구 가능)
 *
 * </pre>
 */
public class App {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 오류가 발생했을 때 실행할 코드 - catch절이 실행되면, 예외가 처리되고 정상흐름으로 복구된다.
