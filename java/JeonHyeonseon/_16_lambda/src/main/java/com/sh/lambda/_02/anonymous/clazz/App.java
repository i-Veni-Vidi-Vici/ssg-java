 ...
 *    - 람다식이 메소드 하나처럼 보여도, 무조건 객체는 하나(인터페이스 구현체)이다.
 *    - @FunctionalInterface 유효성검사 어노테이션 제공
 * 2. 추상메소드의 매개변수부를 화살표 좌측에, 메소드 구현부를 오른쪽 블럭에 작성한다.
 *      (int x, int y) -> { .... }
 * 3. 매개변수부의 자료형은 생략이 가능하다.
 *      (x, y) -> { return x * y; }
 * 4. 우츨블럭에 리턴절이 한 줄인 경우, {}과 return키워드를 생략할 수 있다.
 *      (x, y) -> x * y
 * 5. 우측블럭이 실행코드 한 줄만 가진 경우, { }을 생략할 수 있다.
 *      (k) -> { System.out.println(k); }
 *      (k) -> System.out.println(k)
 * 6. 매개변수가 하나뿐인 경우, 소괄호도 생략가능하다.
 *      k -> System.out.println(k)
 * </pre>
 */
public class App {
    public static void main(String[] args) {
        ...
        // 익명클래스 - 클래스 선언과 객체 생성을 동시에 할 수 있다.
        // - 인터페이스, 추상클래스 앞에 new 연산자를 사용하고 몸통부분을 작성(메소드 작성)
        ...
        result = multiplyCalculator.calc(26, 3);
        System.out.println(result); // 78

        // 람다식을 활용해서 나누기계산기 객체 만들기
        Calculator divideCalculator = (int a, int b) -> {
            System.out.println(a);
            System.out.println(b);
            return a / b;
        };
//        Calculator divideCalculator = (a, b) -> a / b; // 리턴구분이 한 가지일 때 중괄호 삭제 가능
        result = divideCalculator.calc(10, 2);
        System.out.println(result); // 5

        // 다음 람다식을 사용하려면? -> 인터페이스가 필요하다.
        Printer printer = (str) -> System.out.println(str); // String 타입 -> void 메소드
        printer.print("안녕");

        Func.IntFunc square = (n) -> n * n;  // (int) -> int 외부클래스 Func(인터페이스), 내부클래스 IntFunc(인터페이스)
        ...
        System.out.println(square2.calc(0.5)); // 0.25  메소드 이름은 calc임을 알 수 있다.
    }
}
