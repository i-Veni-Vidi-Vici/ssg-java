 ...
 * 현실세계의 모든 사건은 객체와 객체와의 상호작용이라는 것에서 힌트를 얻어
 * 프로그래밍 세계로 옮겨놓은것
 * 객체와 상호작용을 코드로 구현하기 위해 각 객체들에 의인화가 적용이된다.
 * 객체는 스스로 책임을 가지고 행동을 수행하게 된다.
 * 객체는 적절한 책임 가질 수 있게 클래스단위로 잘 분리해 작성해야 한다.
 *
 * 자바 프로그래밍에서의 객체란?
 ...
 * - 송신자객체가 메세지를 보내고(메소드 호출), 수신자 객체는 메세지의 내용(매개인자) 에 따라 특정 작업을 수행한다.
 ...
 * - 자동차는 시동걸거나, 가속을 하거나, 감속을 하거나, 시동이 꺼질 수 있어야 합니다.
 * - 자동차는 처음엔 대기상태로 있습니다.
 * - 자동차는 운전자에 의해 시동이 걸리고, 이미 시동이 걸려 있다면 다시 걸 수 없어야 합니다.
 * - 운전자가 악셀을 밟으면, 시동이 걸린 상태에서만 가속할 수 있습니다. 악셀을 밟을때마다 10km/h씩 가속되어야 합니다.
 * - 운전자가 brake를 밟으면 시동이 움직이고 있는 상태에서 10km/h씩 감속됩니다.
 ...
 * 3. 클래스 설계
 * - 사용자 (실행클래스)
 * - 운전자 (시동켜기, 시동끄기, 가속, 감속 메소드 제공)
 * - 자동차 (시동켜기, 시동끄기, 가속, 감속 메소드 제공 | 시동상태, 속도 상태갑)
 *
