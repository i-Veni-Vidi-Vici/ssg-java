 ...
 *      1. key는 중복을 허용하지 않는다.
 *      2. key에 중복된 값을 저장하면, 기존값에 덮어써진다.
 * - value란? -> key 식별자와 연결된 값. key가 다르다면 중복가능
 *
 * - 구현클래스 : HashMap, LinkedHashMap, TreeMap
 * - 레거시클래스 (HashTable, Properties) 중에서는 유일하게 Properties를 지금도 많이 사용하고 있다.
 *      - key/value타입을 모두 String으로 한정함으로써, 설정정보 관리에 최적화 되어 있기 때문이다.
 *
 * </pre>
 */
public class App {
...
//        Map map2 = new HashMap();
        ...
        // put(K, V) 요소저장
        System.out.println(map.put("홍길동", 23)); // null
        map.put("신사임당", 40); // int -> Integer.valueOf(40) auto-boxing 처리됨
//        map.put(50, "이순신");

        // 중복된 key로 저장
        Integer removed = map.put("홍길동", 33);
        System.out.println("removed = " + removed); // removed = 23
