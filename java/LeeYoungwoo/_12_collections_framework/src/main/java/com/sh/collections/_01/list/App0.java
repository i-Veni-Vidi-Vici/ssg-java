 ...
 * - 자바진영에서 대용량 데이터 처리를 위해 만든 자료구조
 *
 * 1. List계열 : 순서가 유지되어야 하는 데이터 처리
 * 2. Set계열 : 중복을 허용해서는 안되는 데이터 처리
 * 3. Map계열 : key/value로 하나의 요소를 구성하는 데이터 처리 (key는 중복을 허용하지 않는다.)
 * </pre>
 */
public class App0 {
    public static void main(String[] args) {
        Book[] books = new Book[] {
                ...
                new Book(300L, "해기계 파헤치기", "장영실", 17000)
        };

        // 2권 추가 (배열은 크기 변경이 불가능하다.)
        Book[] books2 = new Book[500]; // 불필요한 공간을 미리 할당(공간낭비)
        ...
        // 1권 삭제 (중간의 데이터 삭제) // null 뒤를 한칸씩 앞으로 땡겨와야됨
//        books2[2] = null; // 중간에 데이터를 비워두지 않는다.
        books2[2] = books2[3];
        books2[3] = books2[4];
        books2[4] = null;

        // 중간에 1권 추가 // 넣고싶은 인덱스번째를 비우기위해 한칸씩 뒤로 밀고 넣어야됨
        books2[4] = books2[3];
        books2[3] = books2[2];
        books2[2] = books2[1];
