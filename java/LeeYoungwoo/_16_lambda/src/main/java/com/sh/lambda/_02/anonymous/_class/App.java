 ...
 * 3. 람다식을 이요해서 추상메소드를 간결히 작성하는 방식
 *
 * 람다식
 * - 수학람다식을 프로그래밍에 적용한 케이스
 * - 수학람다식 f(x, y) = x * y  / 람다식 (x, y) -> x * y
 *
 * 문법
...
public class App {
    public static void main(String[] args) {
        ...
        // 곱하기 계산기객체 - 익명클래스
        Calculator multipleCalculator = new Calculator() {
            @Override
            public int calc(int a, int b) {
                return a * b;
            }
        };
        result = multipleCalculator.calc(10,20);
        System.out.println(result);

        // 3. 람다식을 활용해서 나누기계산기 객체 만들기
