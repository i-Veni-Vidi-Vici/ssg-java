package com.sh.call.by;

import java.util.Arrays;

/**
 * <pre>
 *  call by value
 *  - 매개인자가 기본형인 경우 해당
 *  - 실제 리터럴ㄹ이 복사되어 처리
 *  call by referencd
 *  - 매개인자가 참조형인 경우 해당
 *  - 객체 주소값이 복사되어 처리
 * </pre>
 */
public class App {

    public static void main(String[] args) {
        //call by value
        int n = 10;
        CallByValue callByValue = new CallByValue();
        callByValue.test(n); // 값 10 출력
        System.out.println(n);

        //call by value에서 작업결과물을 확인하려면 return값을 확인하면된다.
        int m = callByValue.test2(n);
        System.out.println(m);


        //call by reference
        Item item = new Item(1L, "간장떡볶이",7400);
        CallByReference callByReference = new CallByReference();
        callByReference.test(item);
        // Item item = item -> 객체 주소 값 전달되고 그 객체의 item1.setName()
        item.print();

        int[] arr = {1,2,3,4,5};
        callByReference.test2(arr);
        System.out.println(Arrays.toString(arr));

        //String은 참조형이지만, 마치 Call by value처럼 작동한다.
        // - String은 immutable type이다. 값을 변경할 수 없는 타입.
        // - 더하기 연산시 "Hello World" + " ByeBye world" 할 때, 기존 값을 변경하지 않는다.
        // - "Hello world Byebye wolrd"를 새로 만들어 참조한다.
        String str = "Hello";
        callByReference.test3(str);
        System.out.println(str);


        //n 기본형은 기본형 대로 작동
        //item 참조형은 참조형대로 작동
        callByReference.test4(n,item);


        //callbyReference.test(n,item)
        callByReference.test5(item);
    }
}
