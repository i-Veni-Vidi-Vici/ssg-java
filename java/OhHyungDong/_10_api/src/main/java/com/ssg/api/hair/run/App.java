...
import com.ssg.api.hair.run.model.dto.user.User;
...
public class App {
    public static void main(String[] args) {
        ...
        String[] str = data.split("\n"); // 한줄에 하나씩 str에 저장 " " < 형태로 저장

        for(int i = 1; i<str.length; i++){
            String[] information = str[i].split(",");// 한줄에 저장되어 있는 문자들을 ,를 기준으로 나눠서 각각 배열에 저장
            int id = Integer.parseInt(information[0]);
            String firstName = information[1];
            String lastName = information[2];
            String email = information[3];
            String gender = information[4];
            LocalDate birthday = (information[5].isEmpty() ? null : LocalDate.parse(information[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")));
            LocalDate reservationDate = LocalDate.parse(information[6],DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime reservationTime = LocalTime.parse(information[7],DateTimeFormatter.ofPattern("H:mm"));
            LocalDateTime createAt = LocalDateTime.parse(information[8],DateTimeFormatter.ISO_LOCAL_DATE_TIME);

            users[i - 1] = new User(id,firstName,lastName,email,gender,birthday,reservationDate,reservationTime,createAt);
        }
        for(User user : users){
            if(user !=null){
                System.out.printf("User{id=%d, firstname=%s, lastname=%s, email=%s, gender=%s, birthday=%s, reservationDate=%s, reservationTime=%s, createdAt=%s",
                                    user.getId(),user.getFirst_name(),user.getSecond_name(),
                                    user.getEmail(),user.getGender(),user.getBirthday(),user.getReservationDate(),user.getReservationTime(),
                                    user.getCreatedAt());
            }
            System.out.println();
        }
