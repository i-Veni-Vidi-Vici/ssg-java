 ...
 *  - 객체 동일성(Identity)- heap 영역의 같은 객체인가 ? (주소값이 같은가 ? ) == 동등 비교 연산자
 *  - 객체 동등성(Equality) - 객체가 가진 내용이 같은가 ? (특정필드의 값들이 동일한가 ? ) equals 메소드 ( 오버라이드 )
 *
 * 객체 비교 원칙
 * - equals 비교 결과가 true라면, 동일한 hashcode를 반환해야 한다.
 * - 이후 Collections의 HashXXX 클래들은 요소의 동등성을 위해 equals/hashCode를 모두 호출해서 비교를 해야한다.
 * </pre>
 */
public class App2
{
    public static void main(String[] args) {
        ...
        //동등비교 -  특정 필드의 값이 같은지 비교해서 모두 같다면, 두 객체를 동등하다고 판단
