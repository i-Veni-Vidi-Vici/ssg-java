 ...
 * <pre>
 * 인터페이스 interface
 * - 자바 클래스와 유사한 형태지만, 상수와 추상메서드만 가질 수 있다.
 * - jdk8부터는 default/static메서드도 사용할 수 있다.
 * - implements 키워드 사용
 * - 상속은 단일 상속이지만, 구현은 다중구현이 가능. implements IProduct, Discountable
 * - 인터페이스끼리의 상속은 extends키워드를 사용. 자식인터페이스 extends 부모인터페이스1, 부모인터페이스2
 * </pre>
 */
public class App {
    public static void main(String[] args) {
        // 인터페이스도 부모클래스와 같이 사용가능
        // Computer객체는 IProduct타입으로 제어가 가능
...
//            products[i].printInfo();
            ...
            prod.printInfo(); // 정적바인딩  IProduct#printInfo -> 동적바인딩 Computer#printInfo, Moniter#printInfo
        }
