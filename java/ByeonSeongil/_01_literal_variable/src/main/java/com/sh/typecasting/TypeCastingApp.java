 ...
 *
 * 형변환
 * - 다른 타입끼리 연상해야 하는 경우, 필연적으로 형변환이 필요
...
public class TypeCastingApp {
    public static void main(String[] args) {
        // 정수 관련
        byte bnum = 100;
        short snum = bnum; // 대입 연산. byte값을 short공간에 대입. byte가 short로 자동형변환후에 대입
        int inum = snum; // int = short, short가 int로 변환후 대입연산
        long lnum = inum; // long = int, int가 long으로 변환 후 대입연산

//        int result = inum + lnum; // int + long ->  int가 long으로 변환 후 long + long -> long 결과

        // 실수 관련
        float fnum = 123.456f;
        double dnum = fnum; // float이 double로 자동변환 후에 대입 연산

        double result2 = fnum + dnum; // float + double -> float이 double로 변환 후 연산
        ...
        float result3 = inum + fValue; // int + float -> int가 float으로 변환후 float + float -> 리ㅐㅁㅅ
