 ...
 * 변수 명명 규칙
 * - 지키지 않을 경우 컴파일 오류 유발
 * - 지키지 않아도 컴파일 오류는 나지않지만, 자바개발자 사이에 혼동을 줄 수 있는 부분
 * <p>
 * A. 컴파일 오류
 * - 블록안에 동일한 변수이름을 작성할수 없다.
 * - 자바 예약어 사용할 수 없다.
 * - 변수명은 대소문자를 구분한다.
 * - 숫자로 시작할 수 없다.
 * - 특수기호는 _$만 사용가능하다.
 ...
 * - 단어 사이의 연결을 _로 처리하지 않는다.(Snake_Casing 사용금지)
 * - 한글로 변수명을 작성하는 것은 간으하나 자제한다.
 * - 전형적인 변수명이 있다면, 그걸 사용한다.
 * - 명사형으로 작성한다.
 * - boolean형은 논리형을 유츄할 수 있는 변수명을 사용한다. (형용사)
 * - 값의 의미를 명확히 대변해야 한다.
 */
...
//        int true = 100; // 자바 예약어를 사용할 수 없다.
        ...
        int Age = 30; // 대소문자 구분, 그러나 변수명은 관례에 따라 소문자로 시작해야 함.
        boolean True = true; // 위와 동일

//        String 1stName = "길동"; // 숫자로 시작할 수 없다.
        String _1stName = "길동"; // 특수문자 _는 허용한다.
        boolean $harp = false; // 특수문자 $는 허용, 가끔 외국에서 s대신 사용하긴 함
//        String my name = "신사임당"; // 공백 특수문자는 사용할 수 없다.
        String myName = "신사임당"; // 이와같은 카멜케이싱 적용케이스를 추천함.

        // 관례적 명명법(자바)
        String smsafgaslslmsdlsdlsddlmsdlmsdlmsdlsd = "abc";
        int maxAge = 20; // 카멜케이싱
        ...
        double goHome = 3.3; // 변수명은 명사형으로 작성한다. 메소드명을 동사형으로 작성한다.
        double home = 3.3;
