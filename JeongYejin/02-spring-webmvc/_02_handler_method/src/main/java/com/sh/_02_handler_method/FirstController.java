...
@Slf4j
@SessionAttributes({"id", "foo", "bar"}) // id를 모델에 담으면 session scope에 저장해줘
...
//    }
    public void regist() {
    } // /first/regist path에서 viewName을 유추 : /first/regist.html

    /**
     * WebRequest
     * - 스프링 버전의 HttpServletRequest객체이다
    ...
    @GetMapping("/modify") // index에서 get으로 modify.html을 요청
     ...
     * - 모델의 속성명을 변경할 수도 있다. (기본값은 소문자로 시작하는 클래스명)
     */
    @PostMapping("/search")
    ...
    // id가 만약 다른 메소드에 있으면 ?
    // 이미 null인 상태에서는 세션id가 바뀌지 않는다

    /**
     * @return
     * @SessionAttributes로 등록된 session 속성을 폐기할 때는 HttpSession#invalidate()를 사용할 수 없다.
     * SessionStatus객체(@SessionAttributes로 등록된 속성을 관리하는 객체가 따로 있다)의 setComplete()를 호출해 완료시킬 수 있다.
     * - 속성을 개별적으로 삭제시킬 수 없다. (단점)
     */
    @GetMapping("/logout2")
