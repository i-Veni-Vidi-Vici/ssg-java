...
-- 22행 -> 9행 (9개 그룹)
select
#     menu_name, -- SELECT list is not in GROUP BY clause and contains nonaggregated column
    -- group by와 연관이 없는 일반 컬럼은 사용할 수 없다.
    category_code,
    count(*),
    sum(menu_price),
...
select /*distinct*/
    category_code,
    menu_price -- 2개의 컬럼값을 묶어서 중복을 제거하고 유일한 값으로 취급함
from tbl_menu
order by
    1, 2;
...
group by
    category_code
# having  cnt_by_category > 1 -- having절 후에 select절 alias 실행 ... mysql에서만 문법적으로 허용
having count(*)
order by
...
-- 집계데이터는 1을 반환
select
    if(grouping(category_code) = 0, ifnull(category_code, '카테고리없음'), '집계') category,
