...
@Data
//@Getter(AccessLevel.PRIVATE) // 외부에서 Order객체 안에 있는 필드를 마음대로 가져갈 수 없다
@Setter(AccessLevel.PRIVATE) // 외부에서 값을 임의로 변경하는 것을 방지 // 마이바티스를 쓸땐 문제가 된다
@NoArgsConstructor
public class Order {
    ...
    public Order(Long id, Orderer orderer, DeliveryInfo deliveryInfo, List<OrderLine> orderLines, int totalPrice, OrderStatus orderStatus, LocalDateTime orderDate) {
        ...
        // 주문내역 설정과 총 결제금액 계산은 setter에서 일관되게 작업되어야 한다.
        // 외부에서 호출할 수 없게 접근제한자를 private으로 지정해두었다.
    ...
    public void changeDeliveryAddress(Address newAddress) {
        // 발송완료부터는 배송지를 바꿀 수 없다.
        if (this.orderStatus == OrderStatus.주문확인중 || this.orderStatus == OrderStatus.배송준비중) {
            this.deliveryInfo.changeDeliveryAddress(newAddress); // 대부분의 vo들은 setter를 오픈하지 않는다.
    ...
    public void cancelOrder(OrderStatus orderStatus) {
        if (this.orderStatus == OrderStatus.주문확인중 || this.orderStatus == OrderStatus.배송준비중) {
            ...
            throw new IllegalStateException("주문상품이 이미 발송되어서, 주문을 취소할 수 없습니다.");
        }
    }
}