...
@SecondaryTables({
        @SecondaryTable(
                ...
                // name = "tbl_writer_info.pk컬럼명", referencedColumnName = "tbl_writer.id컬럼명"
        ),
        @SecondaryTable(
                name = "tbl_writer_address",
                pkJoinColumns = @PrimaryKeyJoinColumn(name = "writer_id")
                // @PrimaryKeyJoinColumn(referencedColumnName)은 생략가능하다
        )
})
@Builder
public class Writer {
    ...
    @Embedded
    @AttributeOverrides ({
            // @AttributeOverride(name = "Embedded클래스 ❗필드❗명", column = @Column(name = "세컨테이블 컬럼명(덮어씌워질)", table = "세컨테이블")),
            @AttributeOverride(name = "address1", column = @Column(name = "addr11", table = "tbl_writer_address")),
            @AttributeOverride(name = "address2", column = @Column(name = "addr22", table = "tbl_writer_address")),
