...
console.log("a3b4c5".search(re1)); // 1
...
let target = 'javascript jQuery Ajax sass'; // sass css확장판
// $& 매칭된 결과를 가리키는 와일드카드(포인터)
...
console.log(target.replace(/j/gi, '<$&>')); // 대소문자 // <j>avascript <j>Query A<j>ax sass xxx
console.log(target.replace(/^j/gi, '<$&>')); // <j>avascript jQuery Ajax sass xxx
console.log(target.replace(/^j/g, '<$&>')); // <j>avascript jQuery Ajax sass xxx
...
console.log(re.test('acbbbbbbbb')); // ture
...
// 월요일에는 월요병, 화가 부르르르 화요일 홧병, 수수술한잔 하는 <수요일>, 목이 컥~ <목요일>, 금방 오지 않는 <금요일>, 하지만 오늘은 <목요일>
...
target = 'ABCDEFGhijklmn 가나다라마바사ㅋㅋㅎㅎㅓㅓㅏㅏ 1234567890 !@#$%^&*() \t\n';
console.log(target.replace(/[A-Z][a-z]/g, '<$&>')); // ABCDEF<Gh>ijklmn 가나다라마바사ㅋㅋㅎㅎㅓㅓㅏㅏ 1234567890 !@#$%^&*()
...
// 사용자비밀번호 4자리 숫자여부 검사
re = /\d{4}/ ; // true true false false false
re = /[0-9]{4}/ ; // true true false false false
re = /^[\d]{4}$/ ; // true false false false false
re = /^\d{4}$/ ; // true false false false false
