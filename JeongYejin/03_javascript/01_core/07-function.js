 ...
 * 
 * 함수생성
 * 1. 함수선언식(Hoisting처리됨) function foo(){}
 * 2. 함수표현식(Hoisting처리안됨) const foo = function(){};
 * 
 * Hoisting이란?
 * - 실제 실행 전에 자바스크립트 실행엔진에 의해 함수선언식은 먼저 끌어올려서 처리됨
*/

/**
 * 함수 선언식 Function Declaration
 */
foo('홍길동'); // 선언전에 호출이 작동하다니?
function foo(name) {
    console.log(`안녕, ${name}`);
}
...
const bar = function barrrr(name) {
    console.log(`잘가, ${name}`);
};
bar('신사임당');
