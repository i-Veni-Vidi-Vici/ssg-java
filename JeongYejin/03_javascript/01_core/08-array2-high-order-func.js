...
arr.sort((a, b) => a - b); // 숫자 오름차순. 음수면 그대로, 양수면 바꾸기
console.log(arr);
...
vegitables.forEach((item) => {
    console.log(item);
});

/**
 * map(mappingFunction): 세 요소로 구성된 배열반환
...
// const pTags = ['안녕', '잘가', '반가웠어'].map((item) => {return `<p>${item}</p>`})
console.log(pTags);
 ...
 * findIndex(cbFunction): 특정조건을 만족하는 첫번재 요소의 인덱스를 반환
...
// 70점을 넘은 학생이 존재하는가?
console.log(students.findIndex((student) => student.score > 70) + "번지"); // 0
console.log(students.findIndex((student) => student.score > 90)); // -1 존재하지 않음

// 모든 학생이 60점 이상인가요?
console.log(students.every((student) => student.score >= 60)); // false
console.log(students.some((student) => student.score >= 60)); // true
