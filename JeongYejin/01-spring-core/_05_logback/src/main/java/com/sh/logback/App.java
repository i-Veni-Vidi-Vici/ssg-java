...
//    private static org.slf4j.Logger log = LoggerFactory.getLogger(App.class);
    ...
    public static void main(String[] args) {
        ...
        log.error("error"); // 오류/예외발생시 출력
        log.warn("warn"); // 현재 문제는 없지만, 향후 문제가 될 부분에 대해서 작성 (deprecated)
        log.info("info"); // 단순 정보성 메세지 (운영)
        log.debug("debug"); // 개발시에 필요한 로그 작성
        log.trace("trace"); // 디버그를 세분화해서 개발시에 흐름 추적용 작성(debug보다 세밀한 내용)
