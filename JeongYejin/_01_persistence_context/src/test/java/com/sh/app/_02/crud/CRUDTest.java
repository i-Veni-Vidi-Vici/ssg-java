...
public class CRUDTest {
    ...
    void test2() {
        ...
        TypedQuery query = entityManager.createQuery(jpql, Menu.class);
        // when
...
//        menu.setMenuCode(100L);
        ...
        try {
            entityManager.persist(menu); // 영속성 컨텍스트에 menu 엔티티객체를 저장 (@GeneratedValue 설정시 insert쿼리는 바로 실행, PK를 알아야 영속성 컨텍스트에 저장가능하다.)
            System.out.println("menu가 영속성 컨텍스트에 저장되었습니다...");
            Thread.sleep(3000);
            transaction.commit(); // DB에 쿼리 질의 insert into ....
            // 만약에 여러건이라면? GeneratedValue랑 아닌 건들과 석여서 트랜잭션 처리를 할 때는
            // 일부는 바로 db에 접근하고 일부는 커밋할 때 날라간다 ?
        } catch (Exception e) {
