...
- if not exists 옵션을 주지 않을 경우, 이미 존재할 때 오류발생

# 컬럼 선언
- 컬럼명 자료형 [NOT NULL] [DEFAULT 기본값] [AUTO_INCREMENT] [제약조건(컬럼레벨)]
*/
...
-- datetime타입 : 기본값 ecurrent_timestamp, now() 사용
-- 날짜/시간 관련 default값을 설정할 땐 괄호를 꼭 감싸준다.
...
# auto_increment
-- primary key가 지정된 정수형컬럼에서 사용할 수 있다.
-- 중복되지 않는 숫자를 발급
-- 특정숫자값을 직접 지정가능하다. 해당 숫자는 auto_increment는 건너뛰게 된다.
-- tuncate table을 통해서 reset할 수 있다.
...
insert into product2 (name) values ('일양전구');
select *
from product2;
...
# 자료형 기본값 not null comment 순서(first |
...
----------------------------------------------------------------
- db객체를 삭제하기 위한 DDL
- DDL 특성상 rollback불가하므로, 주의해서 실행해야 한다.
- if exists 옵션을 통해 객체가 존재할 때만 삭제할 수 있다.
*/
