 ...
 */
@SpringBootTest
class LegacyLegacyMenuRepositoryTest {
    ...
    void test1() {
         ...
         *     where
         *         m1_0.menu_code=?
         * Menu(menuCode=10, menuName=수박죽, menuPrice=7000, categoryCode=12, orderableStatus=N)
         */
    ...
    void test2() {
        // given
        Long menuCode = 10L;
        int newMenuPrice = 15000;
        // when
        legacyMenuRepository.updateMenuPrice(menuCode, newMenuPrice);

        // then
        Menu menu = legacyMenuRepository.findByMenuCode(menuCode); // 영속성컨텍스트에 있어서 쿼리가 날아가지 않는다.
