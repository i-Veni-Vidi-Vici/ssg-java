 ...
 * 내부클래스
 *  - 클래스 안에 선언된 클래스
 *  - 외부클래스 안에서만 사용 할 목적으로 만들어진 클래스 Map.Entry
 *  - 연관된 클래스를 그룹핑하기 위해서도 사용한다.
 *  - 내부클래스에서는 외부클래스의 private 자원도 접근이 가능하다.
 *
 *  종류
 *  1. static 내부클래스 - 클래스 영역의 static 자원으로 선언
 *  2. non-static 내부클래스 - 클래스 영역의 non - static 자원으로 선언
...
//            System.out.println(n); // static 자원은 non-static 자원에 접근할 수 없다.
    ...
    class Inner2{
        ...
        public void a(){
            System.out.println(s);
            System.out.println(n); // non 스택틱은 둘다 접근 가능
        }
    }
