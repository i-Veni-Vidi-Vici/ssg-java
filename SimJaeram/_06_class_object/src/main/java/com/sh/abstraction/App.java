 ...
 *     0. 요구사항 정리
 *     - 운전 프로그램을 만들어주세요.
 *     - 운전자는 시동걸기, 악셀을 밝거나, 브레이크를 밝거나, 시동을 끌 수 있어야 합니다.
 *     - 자동차는 시동걸기, 가속하거나, 감속을 하거나, 시동이 꺼질 수 있어야 합니다.
 *     - 자동차는 처음엔 대기상태로 있습니다.
 *     - 자동차는 운전자에 의해 시동이 걸리고, 이미 시동이 걸려있다면 다시 걸 수 없어야 합니다.
 *     - 운전자가 악셀을 밟으면, 시동이 걸린 상태에서만 가속할 수 있습니다.
 *       악셀을 밟을때 마다 10km/h씩 가속 되어야 합니다.
 *     - 운전자가 브레이크를 밟으면, 움직이고 있는 상태에서 10km/h씩 감속됩니다.
 *       0km/h로 정지된 상태에서는 더 이상 감속할 수 없습니다.
 ...
 *
 *     각 객체가 수신할 수 있는 메세지 (역할/책임 정의) == 객체가 제공할 수 있는 메소드
 *     - CarMenu
 *      - 사용자 선택할 수 있는 메뉴 제공
 *     - Driver
 *      - 시동 켜기
