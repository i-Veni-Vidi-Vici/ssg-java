...
//        app.test3();
    ...
    private void test4() {
        ...
        Arrays.stream(declaredMethods).forEach(System.out::println); // 모든 메소드를 보여줌
        /*
        ...
        try {
            ...
            // 2. invoke(타겟객체, ... 매개인자):Object 리턴값을 반환
            ...
            returnValue = depositMethod.invoke(account, 500_000);// invoke도 메소드 호출이다
    ...
    private void test3() {
        DefaultAccount account = new DefaultAccount(20, "100-200-300000", 3_000_000);
        Class<DefaultAccount> clazz = (Class<DefaultAccount>) account.getClass(); // account의 클래스객체를 주세요.

        // 필드 객체를 가져오기
        Field[] declaredFields = clazz.getDeclaredFields(); // 가지고 있는 필드 배열 주세요
        System.out.println(Arrays.toString(declaredFields));
        // [private int com.sh.spring.common.account.DefaultAccount.backCode,
        ...
        try {
            ...
            System.out.println(bankCodeValue); // 20 common에서 backCode를 public으로 바꾼 후 가져올 수 있다.
    ...
    private void test2() {
        ...
        // 생성자 가져오기
        Constructor<Account>[] declaredConstructors = (Constructor<Account>[]) clz.getDeclaredConstructors();
        System.out.println(Arrays.toString(declaredConstructors));

        // 객체 만들기
        try {
            Constructor<DefaultAccount> constructor1 = (Constructor<DefaultAccount>) clz.getDeclaredConstructor(null); // null은 파라미터가 없는 것 = 기본생성자
    ...
    private void test1() {
        ...
        System.out.println(clz1); // class com.sh.spring.common.account.DefaultAccount 실제로 풀네임으로 되어있다.

        Class clz2 = new DefaultAccount(10, "110-234-1234567", 0).getClass();
        System.out.println(clz2); // class com.sh.spring.common.account.DefaultAccount
        System.out.println(clz1 == clz2); // 주소값이 같나요?? true 클래스당 하나씩밖에 안 만들어지는 것이 클래스객체이다.

        try {
            Class clz3 = Class.forName("com.sh.spring.common.account.DefaultAccount");
            System.out.println(clz3); // class com.sh.spring.common.account.DefaultAccount
            System.out.println(clz1 == clz3); // true

            // 배열의 클래스객체 가져오기
            Class clz4 = Class.forName("[D"); // D는 Double을 의미
            Class clz5 = double[].class;
            System.out.println(clz4); // class [D
            System.out.println(clz5); // class [D

            // 참조형(String, ...) 클래스 배열의 클래스 객체
            ...
            // 부모클래스의 클래스 객체
            Class clz8 = clz1.getSuperclass(); // 부모클래스객체(extends한 것)
