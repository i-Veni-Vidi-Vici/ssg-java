document.orderCreateFrm.onsubmit = (e) => {
    ...
    // 총 결제금액 확인
    confirm(`총 결제금액은 ${totalOrderPrice}원입니다. 주문할까요?`) || e.preventDefault();
}
...
const addMenu = () => {
    ...
    // 부여된 최대 menuNo 가져오기
    // 유사배열을 진짜 배열로 변환 1. [...$fieldsets] 2. Array.from($fieldsets)
    ...
    // 새 요소를 추가하기
    // before() 이전 형제요소로 추가 = insertAdjacentHTML ('beforebegin', html)
    // prepend() 첫번째 자식요소로 추가 = insertAdjacentHTML ('afterbegin', html)
    // append() 마지막 자식요소로 추가 = insertAdjacentHTML ('beforeend', html)
    // after() 다음 형제요소로 추가 = insertAdjacentHTML ('afterend', html)
