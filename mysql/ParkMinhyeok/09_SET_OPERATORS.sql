...
# minus
-- 차집합. mysql 은 minus 집합연산자 미지원.
-- left join 을 이용해서 우회적으로 구현
select
    a.*
from
    (select *
...
-- 지난 3개월간의 판매데이터 조회
-- 중복제거 기능이 없는 union all 이 속도상 빠르다.
select *
...
-- 지난 3개월간 제품별 판매량을 조회
-- 모든 resultset 은 테이블로써 참조가 가능하다. from (...) t (테이블 별칭 필수)
select
    prod_name 제품명,
...
select
    convert(extract(year_month from sale_date), char),
    prod_name,
    sum(prod_count)
from (
     select *
...
group by prod_name, convert(extract(year_month from sale_date), char);