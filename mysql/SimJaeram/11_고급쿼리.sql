...
select
    menu_name,
    sum(menu_price) over() 'sum', -- menu_price의 전체 합
...
select
    menu_name,
    menu_price,
#     avg(menu_price) over() -- 전체 평균 금액
    round(avg(menu_price) over(partition by category_code)) avg_by_category,
    sum(menu_price) over(partition by category_code order by menu_price asc) sum_by_cateogry -- sum over에 order by하면 누계를 구할 수 있다.
from
    tbl_menu;
...
# 비율함수
-- ratio_to_report
-- 전체 중에 특정 레코드의 비율을 계산해서 소수점 값으로 반환 // 1이 전체 즉, 1 = 100%
...
select
    menu_name,
    menu_price,
    round(menu_price / sum(menu_price) over(), 2) ratio -- 반올림하여 대략적인 비율 구함
from tbl_menu;
