...
select PROFESSOR_NAME 이름,
       truncate(datediff(current_date, concat('19', substring(PROFESSOR_SSN, 1, 6))) / 365, 0) 나이
from tb_professor
...
select right(PROFESSOR_NAME, 2) 이름
from tb_professor
where char_length(PROFESSOR_NAME) = 3;
...
select STUDENT_NO,
       STUDENT_NAME
from tb_student
where truncate(datediff(now(), if(substring(STUDENT_SSN, 8, 1) in (1, 2), concat('19', substring(STUDENT_SSN, 1, 6)), concat('20', substring(STUDENT_SSN, 1, 6)))) / 365, 0) > 19;

select STUDENT_NO,
       STUDENT_NAME
from tb_student
where year(ENTRANCE_DATE) - if(substring(STUDENT_SSN, 8, 1) in (1, 2), concat('19', substring(STUDENT_SSN, 1, 2)), concat('20', substring(STUDENT_SSN, 1, 2))) > 19
order by 1;
...
select distinct
    DEPARTMENT_NO 학과코드명,
    sum(ABSENCE_YN = 'Y') '휴학생 수'
from tb_student
group by DEPARTMENT_NO;

-- 13. 춘 대학교에 다니는 동명이인(同名異人) 학생들의 이름을 찾고자 한다. 어떤 SQL 문장을 사용하면 가능하겠는가?
select s1.STUDENT_NAME 동일이름,
       count(*) '동명인 수'
from tb_student s1 left join tb_student s2 on s1.STUDENT_Name = s2.STUDENT_Name
where  s1.STUDENT_NO != s2.STUDENT_NO and s1.STUDENT_NAME = s2.STUDENT_NAME
