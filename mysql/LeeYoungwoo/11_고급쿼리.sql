...
-- 인자 : 윈도우함수별로 0~n개 작성
-- over절
    -- partiton by : group by와 비슷하게 윈도우 함수내 그룹핑 지정
    -- order by : order by와 비슷하게 윈도우 함수내 정렬
    -- windowing절 : 대상행 지정

# 순위함수
-- rank : 중복된 행이 있는 경우, 갯수만큼 건너뛰고 다음 순위 부여
...
# 집계함수
-- sum, min, max, avg, count와 같은 그룹함수의 원도우함수버전
-- 일반컬럼과 동시에 사용이 가능하다.
-- over(order by)절을 통해 집계순서에 따른 값의 추이를 조회
...
select menu_name,
       ...
       sum(menu_price) over(partition by category_code order by menu_price) summ_by_category
from tbl_menu;
...
-- 전체메뉴 가격중에 현재 메뉴가격의 비율 조회
select menu_name,
       menu_price,
       round(tbl_menu.menu_price / sum(menu_price) over(), 2) ratio
from tbl_menu;
