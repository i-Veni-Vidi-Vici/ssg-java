...
-- SQL문법은 대소문자를 구분하지 않는다. 다만 데이터부분의 대소문자는 정확히 구분함.
show databases;

-- 일반사용자 생성
-- create user '사용자명'@'접속위치' idendified by '비밀번호'
-- - %로 지정하면 외부접속 허용한다. localhost로 지정하면 내 컴퓨터에서만 접근가능하다.
...
-- sh사용자에게 menudb를 사용하라 수 있는 권한을 부여한다.
-- 사용자 user와 database(schema)는 별개의 객체이다.
-- 한 사용자 n개의 db를 사용할수도 있고, 한 db를 여러명의 사용자가 제어할 수도 있다.
