...
insert into tbl_data_types_char values ('1234567890','1234567890'); -- 10글자까지 허용
...
select
    a, length(a), char_length(a),-- 5 영어는 한글자당 1바이트
    b, length(b), char_length(b)-- 9 한글은 한글자당 3바이트
from
    tbl_data_types_char;
...
-- ----------------------------------
-- int(integer) 정수타입
-- float, double 실수타입 (float - 유효자리 6개, double - 유효자리 16개)
-- decimal 고정소수타입 (지정한 소수점이하로 정확한 데이터관리가 필요할때)
-- - decimal(전체자리수, 소수점이하 자리수)
...
values(
       1234.5678, -- 1235
       1234.5678901234567890, -- 1234.57 자리 부족해서 반올림됨
