...
-- 메인쿼리와 서브쿼리를 어떤것으로 할지 잘 구분해야 한다.
-- ----------------------------
-- 일반 서브쿼리
-- ----------------------------
# 단일행 서브쿼리
...
from
    tbl_menu m left join (
        ...
        -- null끼리 동일비교 할 수 없으므로, ifnull함수를 이용해서 특정 값으로 변환 후 비교. 우회해서 비교하기
...
-- 메뉴가 있는 카테고리 조회
-- 행이 존재한지만 체크하면 되므로 서브쿼리에 select는 *말고도 아무거나 쓸 수 있다.
select
    *
from
    tbl_category c
...
#     category_name, -- 인라인뷰에서 별칭을 사영한 경우 별칭으로 참조해야 한다.
    카테고리명,
    ref_category_name
from (
    select
...
select
    ...
    min(메뉴개수)
from (
    select category_code 카테고리명,count(*) 메뉴개수
    from tbl_menu
    group by category_code
    ) t;

# common table expression
-- 인라인뷰에 이름을 저장하고, 이를 참조하는 방식
