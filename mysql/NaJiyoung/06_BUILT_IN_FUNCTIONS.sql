...
-- 문자열길이관련
-- length(문자열) byte수 반환 -> 영문자는 char_length와 같음/한글은 char_length * 3
...
-- substring_index(대상문자열, 찾는문자열, 횟수 n) : 대상문자열에서 n번째 문자열을 찾고, 그 이후를 버림처리 후 반환
select
...
select
    substring('hong.gd@naver.com', 1, instr('hong.gd@naver.com', '@') - 12),
...
-- 내림(rand() * 경우의수 + 최솟값)
select
    floor(rand() * 10 + 1); -- 내림(1.0이상 ~ 11.0미만)
...
select
    ...
    adddate(now(), interval 1 month), -- 2024-05-17 09:47:05
    adddate(now(), interval 1 year); -- 2025-04-17 09:47:05
...
select
    ...
    str_to_date('630909', '%y%m%d'); -- 2063-09-09, 1970 ~ 2069 연도만 가능
...
-- decimal, signed(부호가 있는)/unsigned(부호가 없는) [integer]등을 지원한다.
select
    123, -- 숫자형이라 오른쪽 정렬
    cast(123 as char), -- 문자형이라 왼쪽 정렬
    convert(123, char);
...
-- 싼거 5000원 미만
-- 적당한거 5000원이상 ~ 10000원미만
-- 좀비싼거 10000원이상 ~ 20000원미만
-- 겁나비싼거 20000원이상
