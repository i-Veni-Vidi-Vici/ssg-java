...
# 그룹핑 함수관련
-- group_concat(컬럼) 그룹핑된 레코드의 값을 모아서 출력.모아보기. 윈도우 함수는 아니다.
...
as (
    select
        category_code code,
        category_name name,
        cast(category_code as char(100)) path -- code의 문자형
    from
        tbl_category
    where
...
with
    ...
    b as (select 1989 + row_number() over() from a a1, a a2, a a3) -- 3 * 3 * 3 cross join
select
    *
from
    b;
...
with
    ...
    b as (select 1989 + row_number() over() year from a a1, a a2) -- 6 * 6 cross join
select
    b.year,
    ifnull(e.cnt, 0) cnt
from
