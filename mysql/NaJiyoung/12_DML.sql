...
values
    ('302', '장채현', '901123-1080503', 'jang_ch@ohgiraffers.com', '01033334444', 'D2', 'J7', 'S3', 3500000, '200');
...
set
    SALARY = (
        select
            SALARY
        from (
            select SALARY from employee where EMP_NAME = '유재식'
             ) tmp),
    BONUS = (
        select
            BONUS
        from (
            select BONUS from employee where EMP_NAME = '유재식'
             ) tmp)
where
    EMP_NAME = '빙명수';
...
update
    employee
set
    -- 변경하고자 하는 테이블과 서브쿼리에서 참조하는 테이블이 다르기 때문에 인라인뷰로 감싸지 않아도 가능하다.
...
-- 서브쿼리로 테이블을 생성하면, 테이블 NOT NULL제약조건 외에는 복사되지 않는다. (제약조건, default값 등은 제외)
