...
-- 테이블 행(레코드)을 대상으로 CRUD 명령어를 제공한다.
...
values(
    '육회쥬스'
      ); -- [HY000][1364] Field 'menu_price' doesn't have a default value(기본값이 없다)

-- bulk insert 문법
-- 기본 insert 또는 컬럼지정 문법 사용가능
insert into
    tbl_menu
values
...
insert into
    employee
values ('301', '함지민', '781020-2123453', 'hamham@ohgiraffers.com', '01012343334', 'D1', 'J4', 'S3', 4300000, 0.2, '200', now(), null, default);
insert into
...
desc tbl_coupon;
-- insert시에 생략 가능한 컬럼은? id, discount_price, discount_rate, issued_at(not null이지만 default값 지정으로 생략가능)
...
update
    employee
set
    SALARY = (select SALARY from (select * from employee where EMP_NAME = '유재식') t1),
    BONUS = (select bonus from (select * from employee where EMP_NAME = '유재식' ) t2)
where
    EMP_NAME = '방명수';
...
-- 서브쿼리로 테이블을 생성하면, 테이블의 NOT NULL제약조건외에는 복사되지 않는다. (제약조건, default값 등은 제외)
