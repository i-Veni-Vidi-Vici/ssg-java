...
# SELECT DISTINCT
-- 컬럼에 중복값을 제거하고 한 번씩만 표시되도록 하는 기능
-- 결과집합의 행의 수가 기존 행의 수 대비 줄어들 수 있다.
-- SELECT절에 한 번만 사용이 가능하다.
-- 여러 컬럼에 대해서 DISTINCT를 적용하면, 컬럼값을 묶어 중복값을 제거한다.
select distinct
    category_code
from
    tbl_menu;
...
# SELECT 문자열 연결
-- concat(v1, v2, ...) 함수를 통해서 문자열 연결
select
    menu_name,
--    menu_price + '원' -- (+)더하기는 항상 산술연산을 의미. 숫자로 변경불가한 문자열은 0으로 처리됨.
    menu_price, -- 위치 파악
    concat('￦', menu_price, '원') -- 문자열
from
    tbl_menu;