...
-- JOIN
-- ====================================
-- 두 개 이상의 테이블 레코드를 연결해서 Relation(가상테이블)을 생성할 수 있다.
...
-- 2. OUTER JOIN (외부조인)
--      - LEFT OUTER JOIN - 왼쪽테이블 전체와 오른쪽 테이블에서는 기준컬럼에 부합하는 행들만 조인한 경우
--      - RIGHT OUTER JOIN - 오른쪽테이블 전체와 왼쪽 테이블에서는 기준컬럼에 부합하는 행들만 조인한 경우
-- 3. CROSS JOIN
-- 4. SELF JOIN (자가조인)
...
-- 메뉴/카테고리정보를 모두 조회 (카테고리 없는 메뉴 제외, 메뉴없는 카테고리 제외)
select
    count(*) -- 21행
from
...
-- tbl_menu(22행)에서 category_code가 null인 22번 찰순대쥬스행이 제외되었다. -> 21행이 사용됨
-- tbl_category(12행)에서 tbl_menu.category_code에서 사용되지 않은 4행(1, 2, 3, 7)이 제외되었다. -> 8행이 사용됨
select
    a.menu_code,
    a.menu_name,
    a.menu_price,
...
from
    tbl_menu a inner join tbl_category
#         on a.category_code = b.categor_code;
        using (category_code);
...
from
    tbl_menu m right outer join tbl_category c
        on m.category_code = c.category_code; -- 25행(21행(inner join) + 4행(식사,음료,디저트,퓨전))

# 조인 결과 정리
-- tbl_menu(22행), tbl_category(12행)
-- inner join 21행(join조건에 부합되는 것만 조인됨) : tbl_menu 21행(category_code가 null인 행 제외) + tbl_category 8행(상대테이블에 매칭되는 행이 없는 행 제외)
...
-- 코드 작성 시 확인 순서
-- 1. 두 테이블의 inner join 결과를 확인한다(제외되는 행 확인하기)
-- 2. 문맥에 따라 외부조인 활용한다.
...
-- CROSS JOIN
-- ---------------------------------
-- 좌우측 테이블에서 행과 행이 만나는 모든 경우의 수를 적용한 조인 (카테시안의 곱)
-- 조인조건절을 작성하지 않는다.
...
-- 같은 테이블 안에서 다른 레코드를 참조하고 있을 때 자가조인을 할 수 있다.
select * from tbl_category;
...
from
    tbl_category c1 join tbl_category c2
        on c1.category_code = c2.ref_category_code; -- c1이 상위카테고리 테이블 - c2는 하위카테고리 테이블
...
from
    employee e left join employee m
                         on e.MANAGER_ID = m.EMP_ID; -- 24행 (16행 + 8행(manager_id가 null인 경우))
