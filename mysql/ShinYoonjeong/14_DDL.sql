...
# DB객체
-- table, user, database(schema), view, index등
-- Stored Program : stored_function, stored_procedure, event_scheduler등
...
# 테이블 선언
-- creat table [if not exists] 테이블명 (...) 옵션...;
...
-- NULL 작성시 NOT NULL -> NULLABLE 변경 가능하다.
alter table product3
...
-- auto-increment가 적용되어있을때, pk는 제거할 수 없다.
alter table product3
drop primary key;
...
-- ----------------------------
-- 테이블의 모든 행을 잘라내는 DDL
-- where절 없는 delete(DML)대비 속도 빠르다. (DML의 before image 작업이 없기 때문)
