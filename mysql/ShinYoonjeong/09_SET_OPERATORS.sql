...
select
    month('240101') - 2,
    month('240101' - interval 2 month ),
    extract(year_month from curdate())
from tbl_sales;

-- 두달전 판매 데이터 조회
-- (쿼리는 다음달, 내년, 내후년 등에 실행해도 똑같이 그때의 두달전 데이터가 조회되어야 한다.)
select *
from tbl_sales
#where month(sale_date)=month(curdate()-interval 2 month )
#and year(sale_date) = year(curdate() - interval 2 month );
where
...
union all
select *
from tbl_sales_202403) as product
group by prod_name;

-- 지난 3개월 월별 제품별 판매량 조회
select
    extract(year_month from sale_date),
    prod_name, sum(prod_count)
from (
         select * from tbl_sales
         union all
         select *
