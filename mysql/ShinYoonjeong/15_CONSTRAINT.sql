...
-- CONSTRAINT
-- =======================
-- 제약 조건. 테이블 작성시 각 컬럼에 대해서 값에 대한 제약을 작성할 수 있다.
-- 데이터 무결성 보장을 목적으로 한다.
-- 데이터를 입력/수정시에 자동으로 제약 조건에 부합하는지 검사

# 구분
-- 1. NOT NULL : null을 허용하지 않음
-- 2. UNIQUE : 중복된 값을 허용하지 않음
-- 3. PRIMARY KEY : 기본키, 고유식별 컬럼 (NOT NULL, UNIQUE)
-- 4. FOREIGN KEY : 외래키, 참조테이블의 참조 컬럼값만 작성 가능
...
user_notnull
values
    (1, 'hongdd', '1234', '홍길동', '남', '01012345678', 'hongdd@naver.com'),
    (2, 'sinsa', '1234', '신사임당', '여', '0105678123', 'sinsa@naver.com');
...
insert into
    user_notnull
values
    (3, 'lees', null, '이순신', '남', '01012341234', null);
insert into
user_notnull (user_no, user_id, user_naem, gender, phone, email)
values
    (3, 'lees', '이순신', '남', '01012341234', null);

-- ------------------------
-- UNIQUE
-- ------------------------
-- 중복값을 허용하지 않는 제약 조건
-- n개의 컬럼을 묶어서 복합 unique 제약 조건도 가능하다.

create table if not exists user_unique(
    user_no int not null unique, -- (컬럼레벨) user_no에 중복값을 허용하지 않는다.
    user_id varchar(50) not null ,
    user_pwd varchar(50) not null ,
    user_name varchar(50) not null ,
    gender char(1),
...
insert into
    user_unique
values
    (3, 'gogo', '1234', '고길동', '남', '01012345678', 'gogo@naver.com');
-- [23000][1062] Duplicate entry '1' for key 'user_unique.user_no'
-- [23000][1062] Duplicate entry 'hongdd' for key 'user_unique.uq_user_unique_user_id'
-- [23000][1062] Duplicate entry 'hongdd@naver.com' for key 'user_unique.uq_user_unique_email'
select *
from user_unique;

-- unique 제약조건이 걸린 컬럼은 null을 허용한다.
-- unique 제약 조건 컬럼은 null 값을 여러개 허용한다. (my-sql은 null값도 고유하게 취급해 하나만 허용)
insert into
    user_unique
values
    (4, 'leess', '1234', '이순신', '남', '01012345678', null);
insert into
    user_unique
values
    (5, 'nondog', '1234', '논개', '여', '01012345678', null);
...
create table if not exists user_primarykey(
    ...
    phone char(11) not null,
    email varchar(100),
    primary key(user_no), -- PRIMARY KEY 이름은 지정할 필요 없음
    constraint uq_user_primarykey_id unique(user_id),
...
#    (null, 'hongdd', '1234', '홍길동', '남', '01012345678', 'hongdd@naver.com');
-- [23000][1048] Column 'user_no' cannot be null
(3, 'leess', '1234', '이순신', '남', '01012345678', 'leess@naver.com');
...
#    prod_no = 300
#    and user_id = 'leess'
#    and order_date ='2024-04-23 11:19:27';
    (prod_no, user_id, order_date) = (300, 'leess', '2024-04-23 11:19:27');

-- 복합기본키 대안
create table tbl_order_composite_pk2(
    id int auto_increment primary key , -- 식별 번호
    prod_no int, -- 상품 번호
    user_id varchar(50), -- 회원 아이디
    count int default 1,
    order_date datetime default current_timestamp,
...
user_foreignkey
values
    (1, 'honggd', '1234', '홍길동', 10, '남', '01012345678', 'honggd@naver.com'),
    (2,'sinsa', '1234', '신사임당', 20, '여', '01012345678', 'sinsa@naver.com');

insert into
user_foreignkey
values
    (3, 'leess', '1234', '이순신', 50, '남', '01012345678', 'leess@naver.com');
...
# 참조하는 자식행이있는 경우 부모 컬럼은 삭제/변경할 수 없다.
-- 1. 참조하는 자식행을 먼저 수정/삭제해야 한다.
-- 2. FK제약 조건에 on update set null, on delete set null 옵션을 줄 수 있다.
...
-- 부모 fk컬럼
-- 자기 참조 (같은 테이블 다른 컬럼을 창조, 상위 카테고리의 category_code를 하위카테고리 ref_category_code로 참조)
...
create table if not exists user_check(
    ...
    constraint ck_user_check_gender check (binary gender in ('M' , 'F') ), -- 대소문자 구분 필요한 경우 binary
    constraint uq_user_check_id unique(user_id),
...
user_check
values
    (1, 'honggd', '1234', '홍길동', 'M', 20, '01012345678', 'honggd@naver.com'),
    (2, 'sinsa', '1234', '신사임당', 'F', 40, '01012345678', 'sinsa@naver.com');

insert into
    user_check
values
--     (3, 'leess', '1234', '홍길동', '남', 20, '01012345678', 'honggd@naver.com')
-- [HY000][3819] Check constraint 'ck_user_check_gender' is violated.
(3, 'leess', '1234', '홍길동', 'm', 20, '01012345678', 'honggd@naver.com'); -- 소문자 m 허용

insert into
    user_check
values
    (4, 'nondog', '1234', '논개', 'F', -1, '01012345678', 'nondog@naver.com'); -- 소문자 m 허용
