...
-- commit(변경사항 반영), rollback(변경사항을 취소), savepoint(변경사항을 중간저장) 명령어 지원
...
-- 1. a은행 update문 : balance - money
-- 2. b은행 update문 : balance + money
...
-- 계좌이체 트랜잭션
start transaction; -- 트랜잭션을 수동으로 제어
update
kb_bank
set
...
commit;
-- 트랜잭션 내부 처리가 하나라도 실패한 경우는 rollback
rollback;
