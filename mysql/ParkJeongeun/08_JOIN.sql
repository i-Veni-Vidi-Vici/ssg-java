...
-- JOIN
-- ===================
-- 두개 이상의 테이블 레코드를 연결해서 Relation(가상테이블)을 생성할 수 있음
-- 두 테이블의 특정컬럼 기준으로 행과 행 연결

# 가상테이블 relation 만드는 방법
-- 1. join : 레코드와 레코드를 가로로 연결
-- 2. union(union all) : 레코드와 레코드를 세로로 연결

# JOIN 구분
-- 1. EQUI JOIN(등가조인) : 조인조건에 동일비교 연산자(=)를 사용한 경우
-- 2. NON-EQUI JOIN(비등가조인) : 조인조건에 동일비교 연산자를 사용하지 않은 경우(>, <, <=, >=, between, )

# EQUI JOIN
-- 1. INNER JOIN (내부조인) : 기준컬럼에 부합하는 행들만 조인
-- 1. OUTER JOIN (외부조인)
--   - LEFT OUTER JOIN : 왼쪽 테이블 전체와 오른쪽 테이블의 기준컬럼에 부합하는 행들만 조인
--   - RIGHT OUTER JOIN : 오른쪽 테이블 전체와 왼쪽 테이블의 기준컬럼에 부합하는 행들만 조인
-- 3. CROSS JOIN
-- 4. SELF JOIN (자가조인)
-- 5. MULTIPLE JOIN (다중조인)

# 조인 문법
-- from 좌측 테이블 a join 우측 테이블 b on 조건절(a.컬럼 = b.컬럼)

# 조인
-- 한우딸기국밥의 카테고리명 조회
-- 1. 한우딸기국밥의 tbl_menu.category_code 조회
-- 2. 1번 결과를 통해 tbl_category.category_name 조회

select category_code -- 4
from tbl_menu
where menu_name = '한우딸기국밥';

select category_name -- 한식
from tbl_category
...
-- cross 키워드(생략가능) , on조건절을 생략하면, 내부조인이 아닌 크로스조인으로 처리된다.
