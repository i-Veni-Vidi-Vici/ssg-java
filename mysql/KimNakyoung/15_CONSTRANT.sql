...
#                                                       on update set null -- 부모 참조컬럼이 수정되면, 자식 컬럼을 nll로 변경
...
-- Cannot add or update a child row: a foreign key constraint fails (`empdb`.`user_foreignkey`, CONSTRAINT `fk_user_foreignkey_grade_code` FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`))
...
# 위에  96먼저 해주고 94줄에 # drop table user_foreignkey; 해주고 다시 실행함

-- tbl_menu -> tbl_category
show create table tbl_menu;
# tbl_category 가 부모 왜냐면 tbl_menu 가 tbl_category 참조를 하기때문에 tbl_menu가 부모이다.
...
create table if not exists user_check (
                                               ...
                                               phone char(11) not null,
                                               email varchar(100),
                                               constraint ck_user_check_gender check ( binary gender in ('M', 'F')), -- 대소문자 필요한 경우
...
insert into
    user_check
values
#     (1, 'honggd', '1234', '홍길동', '남', 20, '01012341234', 'honggd@naver.com'),
    (3, 'leess', '1234', '이순신', 'm', 20, '01012341234', 'leess@naver.com');
-- [HY000][3819] Check constraint 'ck_user_check_gender' is violated.

-- 다른 dbms는 대소문자 구별해서 'm' 안되는데 mysql만 가능 이거 구별하려면 binary 써주자!
...
values
#     (1, 'honggd', '1234', '홍길동', '남', 20, '01012341234', 'honggd@naver.com'),
(4, 'leess', '1234', '논개', 'F', -1, '01012341234', 'nondog@naver.com');
