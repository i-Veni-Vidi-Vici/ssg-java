...
# 문법
-- window_function(인자) over([partiom by] [order by] [windowing절])
-- 인자 : 윈도우함수별로 0~n개 작성
-- over절
    -- partiotion by : group by와 비슷하게 윈도우함수내 그룹핑 지정
    -- order by : order by와 비슷하게 윈도우함수내 정렬
    -- windowing절 : 대상행 지정 ( 앞에 5개만 해주세요 이런거)
...
# 비율함수
-- ration_to_report
-- 전체중에 특정레코드 비율을 계산해서 소수점 값을 반환 (1 = 100%)
...
as (
    ...
    from
        tbl_category
    where ref_category_code is null -- 부모 역할
    union all
    select
        tc.category_code,
        tc.category_name,
...
select
    ...
    concat(space((level - 1)),name, '(', id, ')') 조직도,
    level,
    path
from
    employee_hierarchy
