...
-- insert into 테이블 valus (컬럼값1, 컬럼값2, ...)
-- 2. 컬럼작성문법 : 값을 입력할 컬럼을 명시한 후 값을 제공
-- insert into 테이블 (컬럼명1, 컬럼명2,...) values(컬럼값1,컬럼값)
-- bulk insert문법 : 대량의 데이터를 한번에 입력 가능
 ...
 */
 ...
 values
  ('301','함지민','781020-2123453','hamham@ohgiraffers.com','01012343334','D1','J4','S3',4300000,0.2,'200',NULL,NULL,'N'),
  ('302','장채현','901123-1080503','jang_ch@ohgiraffers.com','01033334444','D2','J7','S3',3500000,NULL,'20',NULL,NULL,'N');
...
# delete  from employee where EMP_ID = '301' or EMP_ID = '302';

-- menu.db

-- 쿠폰테이블
create table tbl_coupon (
    id int auto_increment primary key ,-- 쿠폰식별번호 ( 중복되지 않은 식별번호 자동발급)
    coupon_name varchar(1000) not null, -- 쿠폰명
    discount_price int, -- 금액할인인 경우 할인금액
    discount_rate double, -- 비율할인인 경우 할인비율 ( 0.5 -> 50%)
    issued_at datetime not null default now(), -- 발급일 now() == CURRENT_TIMESTAMP notnull이지만 생략가능
    expires_at date not null -- 만료일
);
desc tbl_coupon;
-- insert시에 생략 가능한 컬럼은? id,discount_price, discount_rate, isssued_at(not null이지만,default값 지정)
...
-- 수정하고자 하는 테이블을 서브쿼리에서 접근할 수 없습니다. ( mysql)만 안돼
update
tbl_menu
set
    category_code = (
...
where
    menu_code = 24; -- 이거 했더니 오류남 왜? 수정하고자 하는 테이블을 서브쿼리에서 접근할 수 없습니다. ( mysql)만 안돼

-- 해결법 : 서브쿼리를 인라인뷰(별칭필수)로 감싸서 사용하면 가능하다.
update
    tbl_menu
set
    category_code = (
...
-- 트랜잭션 수동모드에서는 쿼리 실행후, 별도의 commit 명령을 통해 결과를 반영할 수 있다. ( roolback 명령하면 변경사항을 취소할 수 있다.)
...
-- commit 변경사항 적용하기 (취소불가) 이건 undo redo 아님
-- 이거 안 눌러주면 실제로 테이블에서 안 바꼈음
...
SALARY = (select SALARY
              from (select SALARY
                    from employee
                    where EMP_NAME = '유재식') u), # 유재식 사원 급여
BONUS =(
    select BONUS
    from (select BONUS
          from employee
          where EMP_NAME = '유재식') b
    ) # 유재식 보너스 급여
where EMP_NAME = '방명수';
...
select * from employee;
update employee
set JOB_CODE = (select JOB_CODE from job where job_name = '과장'), # 과장의 직급코드
    DEPT_CODE = (Select DEPT_ID from department where DEPT_TITLE = '해외영업3부') # 해외영업3부의 부서코드
where
    emp_name = '임시환';
...
-- --------------------------------------
-- upsert.
-- pk칼럼 기준으로 데이터가 존재하면 update처리, 존재하지 않으면 insert처리.
...
-- ----------------------------------
-- 병합 기능을 지원한다.
-- merge명령은 mysql에서 지원하지 않은다. insert 사용.
-- 서브쿼리를 이용한 대량의 데이터를  insert 할 수 있다.
...
-- 서브쿼리로 테이블을 생성하면, 테이블 NOT NUL 제약조건 외에는 복사되지 않는다. (제약조건, default값 등은 제외)
create table tbl_menu2
as
    select * from tbl_menu where 1 = 2; # 데이터는 포함하고 싶지 않아서
