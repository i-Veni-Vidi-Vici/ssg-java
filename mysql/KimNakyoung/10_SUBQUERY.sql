...
# exist연산자
-- extists (서브쿼리) 서브쿼리의 조회된 행이 존재하면 true, 존재하지 않으면 false
...
#     exists(select * from tbl_menu where category_code = 100); -- 0행이 나와서 false 이다 .
...
select *
    ...
    where exists(
        select
            1 -- 행이 존재하는지 체크라서 내용 중요하지 않음 그래서 1만 적음 원래
        from
            employee
            where
                DEPT_CODE = d.DEPT_ID -- 부서원 코드 같으면 행 존재한다.
    );
    ...
    not exists(
        select 1
            from
                tbl_menu
            where
                menu_price > m.menu_price -- 가격 마다 비교해서 트루이면 최대 아니면 다 false
...
where
    not exists(
        ...
        from
            tbl_menu
        where
            menu_price < m.menu_price -- 가격 마다 비교해서 트루이면 최소 아니면 다 false
...
-- 마치 하나의 테이블인 것처럼 사요할 수 있다.

# 뷰 구분
-- 1. 인라인 뷰 : 1회용 서브쿼리
-- 2. 저장 뷰 : db 객체중의 하나로, 뷰를 테이블처럼 재사용 가능

select
    -- category_code, -- 필드 리스트에 업서요
#     category_name, -- 인라인뷰에 별칭을 사용한 경우 별칭으로 참조해야 한다.
    카테고리명,
    ref_category_code
from (
         select
         ...
         (select category_name from tbl_category where category_code = c.ref_category_code) ref_category_code
         from
             tbl_category c -- 이거 괄호 안에 있는게 테이블입니다.
     ) t;
