...
# auto_increment
-- primary key가 지정된 정수형컬럼에서 사용할 수 있다. (unique도 가능)
-- 중복되지 않는 숫자를 발급.
-- 특정숫자값을 직접 지정가능하다. 해당 숫자응 auto_increment는 건너뛰게 된다.
-- truncate table를 통해서 reset할 수 있다.

create table if not exists product2(
    id int auto_increment primary key comment  '상품식별번호', -- [42000][1075] Incorrect table definition; there can be only one auto column and it must be defined as a key
...
alter table product3
    add price int default 1000 not null comment '가격' after name; -- 자료형 기본값 not null comment 순서
...
-- --------------------------------
-- 테이블의 모든 행을 잘라내는 DDL
-- where절 없는 delete(DML)대비 속도 빠르다, (DML의 before imgae작업이 없기 때문)
