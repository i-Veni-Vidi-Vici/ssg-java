...
select
#     menu_name -- 그룹에 선언된 컬럼외의 일반컬럼은 올 수 없음
    category_code,
       count(*),
       max(menu_price)
from tbl_menu
...
select *
from tbl_menu m
         left join (select category_code,
                      count(*),
                      max(menu_price) max_menu_price
               from tbl_menu
               group by category_code) c
             -- null끼리 동일비교 할 수 없으므로, ifnull함수를 이용해서 특정값으로 변환후 비교
...
-- 메인 쿼리 where절에서 매행마다 다음 쿼리를 수행한다. 이행의 category가 전달된다.

select max(menu_price)
from tbl_menu
where category_code = 10; -- 우항에 메인쿼리 레코드의 category_code가 대입
...
select DEPT_TITLE
from department d
where exists(select EMP_NAME from employee where DEPT_CODE = d.DEPT_ID)
...
select max(메뉴개수) , min(메뉴개수)
    from(select count(*) 메뉴개수
         from tbl_menu
         group by category_code
        ) t;
