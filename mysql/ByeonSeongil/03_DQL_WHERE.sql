...
-- 메뉴가격이 13000원 메뉴 조회(메뉴코드. 메뉴명, 가격)
select menu_code,
       menu_name,
       menu_price
...
# orderable_status <> 'Y';
#     orderable_status= 'N'
# orderable_status = 'y'; -- 비교수행시 대소문자 구분없이 처리
...
menu_name like '_마늘%'; -- 마늘앞에 한글자가 반드시 존재하고, 마늘 다음에는 0개이상의 문자열 존재하는지 조회
...
select *
from tbl_menu
where menu_name like '___쥬스';-- 3글자 이상 ___%쥬스
...
select *
from tbl_menu
where not category_code is null;
...
select *
from tbl_menu
where category_code between 4 and 6;
