...
-- insert into 테이블 (컬럼명1, 컬럼명2, ...) value(컬럼값1, 컬럼값2, ...)
...
use menudb;
desc tbl_menu;
-- auto_increment 자동증가 컬럼(PK 고유식별컬럼에서 주로 사용)은 별도의 값을 작성하지 않아도, 자동으로 고유한 숫자값을 발급.
...
values (301, '함지민', '781020-2123453', 'hamham@kh.or.kr', '01012343334', 'D1', 'J4', 'S3', 4300000, 0.2, 200, now(), null, default);
...
(
    id             int primary key  auto_increment, -- 쿠폰식별번호(중복되지 않는 식별번호 자동발급)
...
-- insert시에 생략 가능한 컬럼은? id, discount_price, discount_rate, issued_at(not null이지만, default값 지정)
...
delete from tbl_coupon where id =1;
truncate table tbl_coupon;

insert into tbl_coupon( coupon_name, discount_rate, issued_at, expires_at)
values      ('건국기념쿠폰', 0.15, '2024-04-22 11:18:47', '2024-05-22');

insert into tbl_coupon(coupon_name, discount_price, issued_at, expires_at)
values ('대한독립기념쿠폰', 5000, '2024-04-22 11:18:46', '2024-05-22'),
...
-- 트랜잭션 수동모드에서는 쿼리 실행후, 별도의 commit 명령을 통해 결과를 반영할 수 있다. (rollback명령하면 변경사항을 취소할 수 있다.)
...
use empdb;
update employee
    set SALARY = (select SALARY from (select SALARY from employee where EMP_NAME = '유재식') t),
...
-- 임시완 사원의 직급을 과장, 부서를 해외영업3부로 변경하세요.
select *
from employee
where EMP_NAME = '임시환';
...
-- 테이블의 행을 삭제하는 명령
-- 실행결과 행수가 줄어든다.
-- WHERE 특정행을 한정해서 사용해야 한다. (WHERE절을 생략하면 모든행이 삭제된다.)
use menudb;
select * from tbl_menu;
