...
- 이중 select는 DQL문으로 하위로 구분

---------------------------------------------------------------------------------
INSERT
---------------------------------------------------------------------------------
- 새로운 행을 추가. 결과적으로 테이블 행수가 늘어난다.
- insert요청시 처리된 행의 수가 반환된다. (intelliJ에서 서비스뷰-Output탭에서 확인이 가능)
...
values
    ('301', '함지민', '787020-2123453', 'hamham@kh.or.kr', '01012343334', 'D1', 'J4', 'S3', '4300000', '0.2', '200'),
    ('302', '장채현', '901123-1080503', 'jang_ch@kh.or.kr', '01033334444', 'D2', 'J7', 'S3', '3500000', null, '20');
-- 생략하면 기본값 없으면 null로 들어감

-- 쿠폰 테이블
create table tbl_coupon (
    id int auto_increment primary key, -- 쿠폰식별번호 (고유한 값 자동 입력)
...
-- insert시에 이 중 생략 가능한 컬럼?
-- id, discount_price, discount_rate, issued_at(not null이지만, default값 지정)
...
-- 수정하고자 하는 테이블을 서브쿼리에서 접근할 수 없다. -> 인라인뷰로 실행
update tbl_menu
set category_code = (select category_code from tbl_menu where menu_name = '우럭스무디')
where menu_code = 24; -- [HY000][1093] You can't specify target table 'tbl_menu' for update in FROM clause

update tbl_menu
set category_code = ( select category_code from (
...
update employee
set salary = (select salary from (select salary from employee where EMP_NAME = '유재식') t1),
    BONUS = (select bonus from (select BONUS from employee where EMP_NAME = '유재식') t2)
where EMP_NAME = '방명수';

-- 임시환 사원의 직급을 과장, 부서를 해외영업3부로 변경하세요.
-- D2, J4
-- 같은테이블일 때만 인라인으로 감싸서 처리한다!
update employee
...
-- limit절을 이용한 삭제
-- 가격이 제일 싼 2개 삭제
delete from tbl_menu order by menu_price limit 2; -- offset지정 안됨
/*
---------------------------------------------------------------------------------
REPLACE
---------------------------------------------------------------------------------
...
-- 서브쿼리로 테이블을 생성하면, 테이블의 NOT NULL 제약조건 외에는 복사되지 않는다. (제약조건, default값들은 제외)
