...
create table if not exists user_unique (
    ...
    user_name varchar(50) not null,
    gender char(1),
    phone char(11) not null, -- unique 걸면 안된다 번호이동이 잦기때문에 새번호를 재등록하기전까지 겹치는 상황이 있다.
    email varchar(100),
    constraint uq_user_unique_user_id unique(user_id), -- (테이블 레벨)
    -- 관례적인 제약조건 이름 uq_{테이블명}_{컬럼명}
...
select *
from information_schema.TABLE_CONSTRAINTS
where CONSTRAINT_SCHEMA = 'menudb' and TABLE_NAME = 'user_unique';
...
-- unique제약조건만 걸린 컬럼은 null은 허용한다.
-- not null이 필요하면 명시할 것
-- null은 중복 가능(여러개 허용)하다 ! -> cf.ms-sql: null값도 고유한 값으로 취급하여 하나만 허용하는 db존재
insert into
    user_unique
values
...
create table tbl_order_composite_pk (
    prod_no int, -- 상품번호가 pk면 한 상품을 한번밖에 못사는 거야
...
#     and order_date = '2024-04-23 11:19:56';
    (prod_no, user_id, order_date) = (300, 'leess', '2024-04-23 11:19:56');
...
select *
from tbl_order_composite_pk2 where id = 1;
...
- 외채키를 통해서 테이블과 테이블간의 관계가 만들어진다. (Relational DBMS)
...
create table if not exists user_foreignkey(
    ...
    phone char(11) not null,
    email varchar(100),
    constraint fk_user_grade_grade_code
        foreign key(grade_code) references user_grade(grade_code)
#             on update set null -- 부모 참조컬럼이 수정되면, 자식fk 컬럼을 null로 변경
...
/*
 CREATE TABLE `user_foreignkey` (
  ...
  `user_pwd` varchar(50) NOT NULL,
  `user_name` varchar(50) NOT NULL,
  `grade_code` int DEFAULT NULL,
  `gender` char(1) DEFAULT NULL,
  `phone` char(11) NOT NULL,
  `email` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`user_no`),
  UNIQUE KEY `uq_user_foreignkey_user_id` (`user_id`),
  UNIQUE KEY `uq_user_foreignkey_email` (`email`),
  KEY `fk_user_grade_grade_code` (`grade_code`),
  CONSTRAINT `fk_user_grade_grade_code` FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`) ON DELETE SET NULL ON UPDATE SET NULL
...
insert into user_foreignkey
values
    (3, 'leess', '1234', '이순신', 50, '남', '01056785678', 'leess@naver.com');
...
-- 부모pk컬럼 PIMARY KEY(`category_code`)
...
-- 제약조건 이름 쓸 때 table자리에 fk의 경우 참조하는 테이블을 적는 건지?
...
insert into user_check
values (3, 'leess', '1234', '이순신', '남', 20, '01012341234', 'leess@naver.com');
-- [HY000][3819] Check constraint 'ck_user_check_gender' is violated.

insert into user_check
values (3, 'leess', '1234', '이순신', 'm', 20, '01012341234', 'leess@naver.com');
-- 다른 dbms는 대소문자 구별해주는데 ,, mysql은 허용 ^^;;
insert into user_check
values (4, 'nondog', '1234', '논개', 'F', -1, '01012341234', 'nondog@naver.com');
-- [HY000][3819] Check constraint 'ck_user_check_age' is violated.
update user_check set age = age - 20 where user_no = 1;
-- [HY000][3819] Check constraint 'ck_user_check_age' is violated.
