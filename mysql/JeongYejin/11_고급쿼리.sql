...
- group by와 같이 데이터를 그룹핑한 결과를 반환하는데,
일반 컬럼과 동시에 표현이 가능하다.
...
-- over절
    ...
    -- order by : order by와 비슷하게 윈도우함수 내 정렬
    -- windowing 절 : 대상행 지정
...
with tbl_sales_3month as (select *
...
select *
from tbl_sales)
select distinct sale_date, prod_name, prod_count 판매량, sum(prod_count) over(partition by prod_name order by sale_date, prod_name) 제품별누계, sum(prod_count) over(order by sale_date, prod_name) 전체제품누계
from tbl_sales_3month
order by sale_date;
...
select category_code, count(*), group_concat(menu_name separator '/')
from tbl_menu
...
as (
    ...
    union all
    select tc.category_code, tc.category_name, concat(ch.path, '>', tc.category_code)
    from category_hierarchy ch join tbl_category tc on ch.code = tc.ref_category_code
    )
select *
...
with recursive emp_hierarchy(id, name, path)
as (
        select
            emp_id id,
            emp_name name,
            cast(EMP_ID as char(100)) path -- 첫줄 시작
        from
            employee
        where
            MANAGER_ID is null
        union all
        select e.EMP_ID, e.EMP_NAME, concat(eh.path, '>', e.EMP_ID)
        from emp_hierarchy eh join employee e on eh.id = e.MANAGER_ID -- 상위 id를 매니저id로 갖고있는 하위 직급들이라면 그 아이디를 패스 뒤에 붙이는 거야
    )
select *
from emp_hierarchy
order by path;
...
#             MANAGER_ID is null
        union all
        select e.EMP_ID, e.EMP_NAME, eh.level + 1, concat(eh.path, '>', e.EMP_ID)
        from emp_hierarchy eh join employee e on eh.id = e.MANAGER_ID -- 상위 id를 매니저id로 갖고있는 하위 직급들이라면 그 아이디를 패스 뒤에 붙이는 거야
    )
select concat(space((level - 1) * 5), name, '(', id, ')') 조직도, level, path
from emp_hierarchy
order by path;
...
with
    ...
    b as (select 1989 + row_number() over() from a a1, a a2, a a3) -- 3 * 3 * 3
select *
from b;
...
select b.year, ifnull(e.cnt, 0) cnt
from b left join (
    select year(HIRE_DATE) hire_year,
           count(*) cnt
    from employee
