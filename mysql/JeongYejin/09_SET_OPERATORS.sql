...
# union all
select menu_code 메뉴코드, menu_name 메뉴명, menu_price 메뉴가격, category_code 카테고리코드
#      , orderable_status 주문가능여부 -- 컬럼수가 다르면 합쳐지지 않는다! The used SELECT statements have a different number of columns
from
    tbl_menu
where
    category_code = 10 -- 6행
union all -- 위의 별칭이 컬럼명으로 사용된다
select
    menu_code, menu_name, menu_price, category_code
from
    tbl_menu
where
    menu_price < 9000 -- 10 행
order by 1; -- 16행
...
-- mysql에서는 지원하지 않는다.
-- 1. inner join으로 우회 구현 (기준 컬럼에 null은 제외, 상대에 없으면 제외)
-- 2. in 연산자 이용해서 우회 구현
...
desc tbl_sales; -- Null YES Null을 포함할 수 있니
...
select *
from tbl_sales
where year(sale_date) = year(curdate() - interval 2 month) -- year까지 고려!
...
select extract(year_month from sale_date), count(*)
from tbl_sales
group by extract(year_month from sale_date) with rollup ;
...
select extract(year_month from sale_date), prod_name, count(*)
from tbl_sales
group by prod_name, extract(year_month from sale_date) with rollup ;
...
select prod_name, sum(prod_count) sum
...
select extract(year_month from t.sale_date) sale_year_month, prod_name, sum(prod_count) sum
from (select * from tbl_sales_202402
      union all
      select * from tbl_sales_202403
      union all
      select * from tbl_sales) t
group by t.prod_name, extract(year_month from t.sale_date);

# select date_format(extract(year_month from '20240418'), '%Y-%m');
