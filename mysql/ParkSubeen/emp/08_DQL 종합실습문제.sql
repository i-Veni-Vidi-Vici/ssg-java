...
where
    DEPT_CODE = 'D8'
order by
    (SALARY + (SALARY * ifnull(BONUS, 0))) * 12 desc
limit
    1;
...
where
  ...
  and
    (SALARY + (SALARY * ifnull(BONUS, 0))) * 12 < (select distinct
                                                        avg((SALARY + (SALARY * ifnull(BONUS, 0))) * 12) over (partition by JOB_CODE)
                                                    from
                                                        employee
                                                    where
                                                        JOB_CODE= e.JOB_CODE)
...
with recursive hierarchy_menu(no, name, level, path)
as (
        select
            no,
            menu_name name,
        ...
        select
            ...
            concat(hm.path, ' > ', m.no)
        from
            hierarchy_menu hm join tbl_hierarchy_menu m
                on hm.no = m.parent_no
    )
select
    concat(space((level - 1) * 5), name)
from
    hierarchy_menu
order by
    path;
