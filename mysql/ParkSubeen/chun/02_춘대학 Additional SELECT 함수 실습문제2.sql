-- 춘대학 Additional SELECT 함수 실습문제2

use chundb;
-- 1. 1. 영어영문학과(학과코드 `002`) 학생들의 학번과 이름, 입학 년도를 입학 년도가 빠른순으로 표시하는 SQL 문장을 작성하시오.
...
select
    PROFESSOR_NAME 교수이름,
    year(now()) - concat('19', substr(PROFESSOR_SSN, 1, 2))
                - if(month(now()) > substr(PROFESSOR_SSN, 3, 2) && day(now()) > substr(PROFESSOR_SSN, 5, 2), 0, 1) 나이
from
    tb_professor
where
