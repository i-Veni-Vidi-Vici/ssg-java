...
-- 트랜잭션 범위의 DML 명령은 모두 성공, 모두 실패 되어야 한다

-- 계좌이체 : a은행 계좌에서 출금, b은행 계좌에 입금하는 행위
-- 1. a 은행 update 문 : balance = balance - money
-- 2. b 은행 update 문 : balance = balance + money
...
select * from kb_bank;
select * from ssg_bank; -- 이렇게 몰아서 처리하는 것을 트랜잭션이랄고 함.
