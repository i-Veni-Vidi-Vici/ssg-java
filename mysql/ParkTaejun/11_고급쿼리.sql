...
# 문법
-- window_function(인자) over(인자) over([partition by] [order by] [windowing절])
-- 인자 : 윈도우함수별로 0개~n개 작성
-- over절
...
select
    ...
    row_number() over (order by menu_price) 'row_bumber'
from
    tbl_menu;
...
-- 조직도, 메뉴, 답변형게시판, 댓글등의 처리에 적합 (fractal 구조)
-- mysql에서는 재귀쿼리로써 처리가 가능하다

-- 카테고리 계층 표현
with recursive category_hierarcy (code, name, path)
as (
    select
        category_code code,
        category_name name,
...
with recursive employee_hierarchy (id, name, level, path) -- 이 재귀에서 사용할 컬럼
    as (
    ...
    from
        employee
    where
         employee.MANAGER_ID is null -- 루트행을 만든다..
#         EMP_ID =200
    union all
    select
        e.EMP_ID,
    ...
    from
        employee_hierarchy eh -- 부모행에 있는 패쓰를 가져와서 지금 붙은 행의 카테고리코드를 붙인다.. 무슨마릴까!!!!!!!!!!!!!!!! 난모르겠소,,
            join employee e
...
#     b as (select 1989 + row_number() over ()from a a1, a a2, ()) -- 3 * 3 * 3
select
    b.year,
    ifnull(e.cnt, 0)cnt
from
