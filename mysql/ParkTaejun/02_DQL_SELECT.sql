...
-- select 출력할 컬럼을 명시
-- from 조회대상이 되는 테이블
-- where 결과집합에 포홤될 행의 조건을 기술. (각 행별로 조건 검사 후 true인 행만 포함)
...
-- 여러 컬럼에 DISTINCT를 적용하면, 컬럼값을 묵어 중복값을 제거한다,=.
...
select
    menu_name,
    --    menu_price + '원' -- + 항상 산술연산을 의미. 숫자로 변경불가한 문자열은 0으로 처리 됨.
    menu_price,
    concat('￦', menu_price, '원') # 개수의 제한 없음
from
    tbl_menu;
