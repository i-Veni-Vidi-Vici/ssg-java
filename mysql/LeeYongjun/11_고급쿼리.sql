...
-- group by와 같이 데이터를 그룹핑한 결과를 반환하는데, 일반컬럼과 동시에 표현 가능하다.
...
#     rank() over (order by menu_price desc) 'rank', -- 오름차순 정렬도 가능
...
-- 일반컬럼과 동시에 사용이 가능하다.
-- over(order by)절을 통해서 집계 순서에 따른 값의 추이를 조회할 수 있다.
select
    menu_name,
...
-- partition by절을 통해 그룹에 대한 집계 조회 가능
select
    menu_name,
    menu_price,
    category_code,
...
select
    category_code,
    group_concat(menu_name separator '/'), -- 모아서 보는 용도로 사용
    count(*)
from
    tbl_menu
group by
...
-- 조직도, 메뉴, 답변형게시판, 댓글 등의 처리에 적합 (fractal구조)
-- mysql에서는 재귀쿼리로써 처리가능
...
with recursive employee_hierarchy (id, name, level, path)
        ...
        from
            employee
        where
            EMP_ID = 200 -- 관리자 아이디가 없는 사람들을 제외하는 식
#         MANAGER_ID is null -- 관리자 아이디가 없는 사람들까지 포함되는 식
        union all
        select
            e.EMP_ID,
            e.EMP_NAME,
            eh.level + 1, -- (부모에서 level을 1씩 더하기 위한 식)
...
select
    -- space((level - 1) * 5)는 들여쓰기를 하기 위한 식
