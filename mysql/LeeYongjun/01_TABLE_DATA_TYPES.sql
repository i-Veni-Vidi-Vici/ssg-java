...
-- --------------------------------------
-- int(Integer) 정수타입
-- float, double 실수타입 (float - 유효자리수 6개, double은 유효자리수 16개)
-- decimal 고정소수타입 (지정한 소숫점이하로 정확한 데이터관리가 필요할때)
...
values (
           ...
           1234.5678, -- 1234.57
           1234.5678, -- 1234.5678
           1234.5678 -- 1234.5678 00000 00000 00000
       );
...
values (
        1234.5678, -- 1235
        1234.5678156165561654, -- 1234.57
        1234.5678123584645644, -- 1234.5678
        1234.5678564645645456 -- 1234.5678 00000 00000 00000
       );
...
-- TIMESTAMP YYYY-MM-DD HH:MM:SS 형식으로 관리.
    -- 지역대정보를 제거하고 UTC로 관리. 불러올때는 지역대정보를 적용해서 보내줌.
    -- 국제화가 필요한 앱에 적합하다.
-- YEAR YYYY 형식으로 관리
...
-- 지역대 정보가 변경되면, timestamp타입은 그에 알맞게 변화해서 불러온다.
...
-- 결과집합의 컬럼명을 변경
-- 컬럼 AS "별칭" ('', `` 허용)
-- as 생략가능, 별칭이 공백/특수문자를 포함하지 않았다면 "" 생략가능
select
    menu_name as "메뉴명",
    menu_price as "가격",
...
-- 여러 컬렘에 DISTINCT를 적용하면, 컬럼값을 묶어 중복값을 제거한다.
select distinct
    category_code
from
    tbl_menu;
