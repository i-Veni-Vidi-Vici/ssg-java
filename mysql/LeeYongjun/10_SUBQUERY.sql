...
-- 메인쿼리 수행중에 서브쿼리를 만나면, 서브쿼리 우선 실행하고 그 결과를 메인쿼리에 반환하는 구조
...
-- 서브쿼리의 실행결과(행/컬럼) 수에 따랄 사용해야하는 연산자가 다르다.
...
-- inner join (category_code가 null인 그룹제외)
select
    m.*
from
    tbl_menu m join(
...
from
    tbl_menu m left join(
        ...
        -- null끼리 동일비교 할 수 없으므로, ifnull함수를 이용해서 특정 값으로 변환후 비교
...
from
    tbl_menu
where
    category_code = 4; -- 4 = 15000.0000, 5 = 22000.0000, ...
...
where
    exists(
        select
#             *,
1 -- 내용이 중요하지 않고 행이 존재하는지 체크하기 위해서 1을 사용한다.
        from
            employee
        where
            DEPT_CODE = d.DEPT_ID
