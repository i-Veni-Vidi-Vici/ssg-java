...
from
    tbl_menu
where
#    orderable_status = 'N'; -- 비교수행시 대소문자 구분없이
...
-- 카테고리코드가 4번 또는 가격이 9000원 이면서 메뉴번호가 10번보다 큰 메뉴 조회
...
from tbl_menu
where
    menu_price < 10000 || menu_price > 25000;

#LIKE 문자열 패턴 연산자
-- 비교하려는 컬럼값이 특정패턴을 만족하면 ture를 반환하는 연산자.
