...
select *
from tbl_menu
where category_code = 10 && orderable_status = 'Y'; -- and, && 둘 다 사용 가능

-- 카테고리코드가 10번 또는 주문가능한 메뉴 조회
select *
from tbl_menu
where category_code = 10 || orderable_status = 'Y'; -- or, || 둘 다 사용 가능

-- 카테고리코드가 4번 또는 가격이 9000원이면서 메뉴번호가 10번보다 큰 메뉴 조회
-- and, or 를 나열할 때, and연산자가 우선순위가 높다.
select *
from tbl_menu
where category_code = 4
          ||
      (menu_price = 9000 && menu_code > 10);

# between 연산자
-- between 값1 and 값2 : 값1 이상 값2 이하
-- 숫자/문자/날짜시간에 대해 모두 사용가능
-- 숫자, 사전등재순, 과거-미래 순서에 따라 범위 지정
...
# where not menu_price between 10000 and 25000;
# where !(menu_price between 10000 and 25000);
where menu_price < 10000 or menu_price > 25000;
...
#     menu_name like '%마늘'; -- 마늘로 끝나는지 조회
    menu_name like  '_마늘%'; -- 마늘 앞에 한글자가 반드시 존재하고, 마늘 다음에는 0개 이상의 문자열 존재하는지
...
select *
from tbl_menu
where category_code is null; -- =null은 없음
...
select *
from tbl_menu
where ifnull(category_code, 0) = 0;
...
select *
from tbl_menu
where category_code != 4 and
    category_code != 5 and
    category_code != 6;

select *
from tbl_menu
where not(category_code = 4 or
    category_code = 5 or
    category_code = 6);
