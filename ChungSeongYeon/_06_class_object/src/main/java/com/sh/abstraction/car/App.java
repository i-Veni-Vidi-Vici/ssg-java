 ...
 * - 현실세계의 모든 사건은 객체와 객체와의 상호작용이란 것에서 힌트를 얻어 프로그래밍 세계로 옮겨놓은 것.
 * - 객체와 상호작용을 코드로 구현하기 위해 각 객체들에 의인화가 적용된다.
 * - 객체는 스스로 책임을 갖고 행동을 수행하게 된다.
 * - 객체는 적절한 책임을 가질 수 있게 클래스 단위로 잘 분리해 작성해야 한다.(SOLID 객체지향 설계원칙 중 Single Responsibility Principle 단일책임원칙)
 *
 * 자바 프로그래밍에서의 객체란?
 ...
 *  - 자동차는 시동걸기, 가속하거나, 감속하거나, 시동이 꺼질 수 있어야 합니다.
 *  - 자동차는 처음엔 대기 상태로 있습니다.
 ...
 *      0km/h 정지된 상태에서는 더 이상 감속할 수 없습니다.
 *  - 운전자가 시동을 끄면 더이상 자동차는 가속/감속할 수 없습니다.
