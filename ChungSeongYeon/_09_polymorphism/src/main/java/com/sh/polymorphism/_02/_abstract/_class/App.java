 ...
 * <pre>
 * 추상클래스 | 인터페이스
 * - 상속(구현)을 통해 클래스 계층구조를 만들면서, 추상 메소드를 통해 구현을 강제화할 수 있다는 특징.
 * - 둘 다 부모 클래스 역할을 할 수 있음.
 * - 추상 메소드란? 미완성 메소드로 몸통이 없다. 대신 자식클래스에서 반드시 구현해야 한다.
 * - 둘 다 객체화할 수 없다. (new 연산자 사용 불가)
 *
 * 추상클래스
 * - abstract class 미완성클래스
 * - 추상메소드를 통해서 자식클래스의 기능을 강제구현하게 한다.
 * - 일반 필드/메소드를 가질 수 있다. (인터페이스와 다른 점)
...
//        Product product = new Product(); // abstract 로 인해 new 연산자 사용이 막힘.
