...
@Slf4j
@SessionAttributes({"id"}) // id를 session Scope에 담기
...
//    }
    public void regist(){} // /first/regist path에서 viewName을 유추
     ...
     * - 모델 속성명을 변경할수도 있다. (기본값은 소문자로 시작하는 클래스명)
     *
     * @param menuDto
     * @return
     */
    ...
    public String logout(HttpSession session){
        session.invalidate(); // 기존 세션 폐기 (안에 있던 속성값 모두 날아감)
        return "first/loginResult";
    }
     ...
     * 왜 세션 사용? 첫 요청을 받고 난 후에도 계속 값을 저장해서 다음 요청에도 같은 값을 주기 위해서
     *
     * @param model
     * @param id
     * @param pwd
    ...
    public String login(Model model, @RequestParam String id, @RequestParam String pwd){
        log.debug("id/pwd = {}/{}", id, pwd);
        // 클래스레벨 @SessionAttributes("id") 등록이 되어 있으므로, 아래는 session scope 관리된다.
        model.addAttribute("id", id);
        return "first/loginResult";
    }

    /**
     * @SessionAttributes로 등록된 session속성을 폐기할때는 HttpSession#invalidate()를 사용할 수 없다.
     * SessionStatus객체(@SessionAttributes로 등록된 속성을 관리하는 객체)의 setComplete()를 호출해 만료시킬수 있다.
     * - 속성을 개별적으로 삭제시킬수 없다.
     *
     * @return
     */
    ...
    public String etc(@RequestHeader("user-agent") String userAgent, @CookieValue("JSESSIONID") String jSessionId){
        log.info("userAgent = {}", userAgent); // userAgent = Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36
        log.info("JSESSIONID = {}", jSessionId); // JSESSIONID = 396847FD5B47F3F814E338904A8083F2
