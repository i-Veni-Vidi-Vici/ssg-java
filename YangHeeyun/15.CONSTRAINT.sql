...
-- ============================
-- 제약조건
-- 테이블 작성시 각 컬럼에 대해서 값에 대한 제약을 작성할 수 있다.
-- 데이터무결성 보장을 목적으로 한다.
...
-- 5. CHECK : 지정한 도메인(컬럼값의 범위, 값목록)에서만 값 사용 가능

# 제약조건 조회 (CONSTRAINT_TYPE)
select *
from
...
CREATE TABLE `employee` (
  `EMP_ID` varchar(3) NOT NULL, -- 컬럼레벨 (컬럼 선언과 동시에 작성하면 컬럼레벨에 작성했다고 함)
  ...
  PRIMARY KEY (`EMP_ID`), -- 테이블레벨 (컬럼 선언 끝나고 작성했으면 테이블레벨에 작성했다고 함)
...
values
    ...
    (2,'singsa','1234','신사임당','여','01056785678','sinsa@naver.com');
...
-- Duplicate entry 'honggd' for key 'user_unique.uq_user_unique_user_id'
-- [23000][1062] Duplicate entry 'honggd@naver.com' for key 'user_unique.uq_user_unique_email'

-- unique제약조건이 걸린 컬럼은 null을 허용한다.
-- unique 제약조건 컬럼은 null값을 여러개 허용한다. (예외 ms-sql은 null값도 고유하게 취급해 하나만 허용)
insert into
    user_unique
values
...
create table if not exists user_primarykey (
    ...
    phone char(11) not null,
    email varchar(100),
    constraint pk_user_primarykey_user_no primary key(user_no),
...
create table tbl_order_composite_pk(
    ...
    count int default 1, -- 가격
    order_date datetime default current_timestamp, -- 주문일자
...
show create table tbl_order_com
...
#     prod_no = 300
#     and user_id = 'leess'
#   and order_date = '2024-04-23 11:20:35';
    (prod_no,user_id,order_date) = (300,'leess','2024-04-23 11:20:35');
...
values
    ...
    (2,'singsa','1234','신사임당',20,'여','01056785678','sinsa@naver.com');
...
# 부모데이터가 변경되면?
# 참조하는 자식행이 있는 경우 부모컬럼은 삭제 또는 변경할 수 없다. (참조무결성 위반)
...
-- 자기 참조 (같은 테이블 다른 컬럼), 상위 카테고리의 category_code를 하위카테고리 ref_category_code로 참조
...
create table if not exists user_check (
    ...
    phone char(11) not null,
    email varchar(100),
    constraint ck_user_check_gender check(binary gender in ('M','F')), -- 대소문자 구분이 필요한 경우 binary키워드 사용(mysql의 경우)
    constraint ck_user_check_age check(age > 0),
    constraint uq_user_check_user_id unique(user_id),
    constraint uq_user_check__email unique(email)
);
...
values
    ...
    (2,'singsa','1234','신사임당','F', 40,'01056785678','sinsa@naver.com');

insert into
    user_check
values
#     (3,'leess','1234','이순신','남', 20,'01012341234','leess@naver.com'); -- [HY000][3819] Check constraint 'ck_user_check_gender' is violated.
    (3,'leess','1234','이순신','m', 20,'01012341234','leess@naver.com'); -- 소문자 m 허용 (binary키워드를 넣지 않은 경우, 넣으면 허용불가)
