...
public class RestaurantEntityTest {
    ...
    void test2() {
        ...
        // then
        assertThat(restaurant.getCreatedAt()).isNotNull(); // insert성공 시 자동생성되므로 notnull 검증
//        entityManager.detach(restaurant); // select문 보고싶어서 영속성 컨텍스트에서 제거

        Restaurant restaurant2 = entityManager.find(Restaurant.class, restaurant.getId());
        assertThat(restaurant2).isSameAs(restaurant); // detach하면 객체가 하나뿐이므로 오류가 나게됨
    }
