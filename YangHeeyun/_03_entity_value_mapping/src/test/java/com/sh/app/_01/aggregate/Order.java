 ...
 * <pre>
 * 1. 엔티티(Entity)
 * - 식별자(Identifier): 엔티티는 고유한 식별자(@Id, @EmbeddedId)를 가진다. 이 식별자를 통해 엔티티를 식별하고 구분할 수 있다.
...
@Data
//@Getter(AccessLevel.PRIVATE) // 외부에서 값을 가져가는 것을 방지
@Setter(AccessLevel.PRIVATE) // 외부에서 값을 임의로 변경하는 것을 방지
@NoArgsConstructor
public class Order { // 엔티티라 생각
    ...
    public Order(Long id, Orderer orderer, DeliveryInfo deliveryInfo, List<OrderLine> orderLines, int totalPrice, OrderStatus orderStatus, LocalDateTime localDateTime) {
        this.id = id;
        this.orderer = orderer;
        this.deliveryInfo = deliveryInfo;
        // 주문내역 설정과 총결제금액 계산은 setter에서 일관되게 작업되어야 한다.
        // 외부에서 호출할 수 없게 접근제한자를 private로 지정해두었다.
    ...
    public void changeDeliveryAddress(Address newAddress) {
        if(this.orderStatus == OrderStatus.배송준비중 || this.orderStatus == OrderStatus.배송준비중) {
