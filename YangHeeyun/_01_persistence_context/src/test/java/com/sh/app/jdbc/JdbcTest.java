 ...
 * 2. SQL 의존 개발(요구사항이 변하면 반드시 sql을 수정해야한다)
 * 3. 패러다임 불일치 (연관관계, 자바진영의 상속, 객체 그래프 탐색 등에 구현에 있어 DB Table과 매칭되지 않음)
 * 4. 동일성 보장 문제 : PK로 조회한 두 객체가 다를 수 있다.
 *
 * MyBatis가 1번은 해결했지만 직접 sql문을 작성하기때문에 2를 해결할 수 없다. 3,4도 마찬가지
 * JPA를 쓰면 4개를 모두 해결보장한다.
 * </pre>
 */
public class JdbcTest  {
    ...
    void test2() throws SQLException {
        ...
        // then
        assertThat(result).isPositive(); // DML이니까 성공하면 1을 반환하므로 양수여야 한다.
    }

    /**
     * <pre>
     * DB관점에서는 상속이 존재하지 않는다.
     * DB관점의 상속은 서브타입 엔티티를 의미한다.
     *
     * - tbl_member: id, name
     * - tbl_admin_member: member_id, admin_role
     * - tbl_shop_member: member_id, address
     *
     * AdminMember 조회 시 tbl_member & tbl_admin_member 조인쿼리
     * ShopMember 조회 시 tbl_member & tbl_shop_member 조인쿼리
     *
