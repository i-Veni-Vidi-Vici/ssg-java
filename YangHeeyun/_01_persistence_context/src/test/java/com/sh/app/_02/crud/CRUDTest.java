...
public class CRUDTest {
    ...
    /**
     * <pre>
     * DML은 EntityTransaction객체를 통해서 트랜잭션 범위 및 commit/rollback처리를 해줘야 한다.
     * </pre>
     */
    @Test
    @DisplayName("메뉴 신규 둥록")
    void test3() {
        // given
        Menu menu = new Menu();
        ...
        try{
            entityManager.persist(menu); // 메뉴를 db에 저장하는 코드가 아닌 영속성컨텍스트에 menu 엔티티객체를 저장.
            // (@GeneratedValue 설정시 insert쿼리는 바로 실행. PK를 알아야 영속성컨텍스트에 저장가능하다.)
            System.out.println("menu가 영속성 컨텍스트에 저장되었습니다...");
            transaction.commit(); // DB에 쿼리 질의 insert into... 문제가 없다면 트랜잭션 커밋
        }catch(Exception e){
            transaction.rollback();
            e.printStackTrace();
        }
        // then
        assertThat(menu.getMenuCode()).isNotZero(); // auto-increment로 id값을 할당받으므로 zero여서는 안된다
