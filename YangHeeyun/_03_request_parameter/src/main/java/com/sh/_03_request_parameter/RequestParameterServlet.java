...
public class RequestParameterServlet extends HttpServlet {
     ...
     * - http://localhost:8080/03_request_parameter/test?name=홍길동&age=33&married=true (?가 구분자로 연결된다)
     * - name=홍길동&age=33&married=true Query String부분은 request객체 안에서 HashMap으로 관리한다.
...
//        String age = req.getParameter("age");
        int age = Integer.parseInt(req.getParameter("age")); // 굳이 int로 쓰고 싶다면 형변환을 해준다
        LocalDate birthday =
            LocalDate.parse(req.getParameter("birthday"),
                            DateTimeFormatter.ISO_DATE); // yyyy-MM-dd
        String gender = req.getParameter("gender");
        String national = req.getParameter("national");
        // hobby는 복수입력이 가능하다. 복수입력은 getParameterValues를 사용
