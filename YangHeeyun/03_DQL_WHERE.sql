...
#    orderable_status = 'n'; -- 비교수행 시 대소문자 구분없이 처리
    binary orderable_status = 'n'; -- binary추가하면 비교수행 시 대소문자 구분없이 처리
...
-- 카테고리코드가 10번이면서 주문가능한 메뉴 조회(조건 2개)
select *
from tbl_menu
where
    category_code = 10
...
# between 연산자
-- betwenn 값1 and 값2 : 값1 이상 값2 이하
-- 숫자/문자/날짜시간에 대해 모두 사용가능
-- 숫자 범위, 사전등재 순, 과거~미래 순서에 따라 범위를 지정
select
    *
from
    tbl_menu
...
#    menu_name like '%마늘' -- 마늘로 끝나는지 조회
    menu_name like '_마늘%'; -- 마늘앞에 한글자가 반드시 존재하고, 마을 다음에는 0개 이상의 문자열 존재하는지
...
from
    tbl_menu
where
#     category_code = null; -- null값 연산 불가(비교,산술)
...
-- null처리함수(null은 동등비교할 수 없으므로, 다른값으로 대체후에 비교)
-- ifnull(nullableValue, value if null)
-- ifnull(null이 아니면 그대로 출력, null이면 출력할 값)
