...
-- Stored Program : stored_function,stored_procedure, event_scheduler 등의 사용자 함수
...
create table tbl_rent(
    ...
    car_name varchar(50),
    rent_date date default (current_date), -- 날짜/시간 관련 default는 괄호를 꼭 감싸준다.
...
-- truncate table을 통해 reset할 수 있다.
...
delete from product2 where id = 3; -- 3번을 삭제하면, 다시는 3번을 발행하지 않는다. 다음 insert시 4 발행
...
-- --------------------------
-- 테이블의 모든 행을 잘라내는 DDL
-- where절 없는 delete(DML)대비 속도 빠름. (DML의 before image작업이 없기때문)
