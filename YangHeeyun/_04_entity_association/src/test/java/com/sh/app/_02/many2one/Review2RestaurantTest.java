...
public class Review2RestaurantTest {
    ...
    void test2() {
         ...
         */
        // then
        System.out.println(review); // Review(id=1, restaurant=Restaurant(id=1, name=길동이네 떡뾱기), grade=STAR5, comment=존맛탱~~~~~~~~~~~~~~)
    ...
    void test3() {
        ...
        TypedQuery<Review> query = this.entityManager.createQuery(jpql, Review.class)
                ...
                // query.setParameter(parameterName, value)이 형식이고, parameterName은 JPQL쿼리에서 ':' 기호 뒤에 있는 이름과 일치해야 한다.
        ...
        /**
         * Review(id=1, restaurant=Restaurant(id=1, name=족발장수), grade=STAR5, comment=와우~ 족발이다~)
         * Review(id=2, restaurant=Restaurant(id=1, name=족발장수), grade=STAR4, comment=건강한 맛입니다.)
         * Review(id=3, restaurant=Restaurant(id=1, name=족발장수), grade=STAR5, comment=족발은 이집이다~)
         * Review(id=4, restaurant=Restaurant(id=1, name=족발장수), grade=STAR3, comment=여느 동네의 평범한 족발입니다.)
         * Review(id=5, restaurant=Restaurant(id=1, name=족발장수), grade=STAR2, comment=족발이 다이어트가 너무 심해요ㅠㅜ)
         */
        // then
