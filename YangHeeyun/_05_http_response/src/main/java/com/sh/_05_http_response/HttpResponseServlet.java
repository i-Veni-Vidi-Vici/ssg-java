...
/*
Servlet이 하는 일
1. 요청 핸들링 : 사용자입력값을 request객체로부터 꺼내서 자바변수에 대입
2. 업무로직 (비지니스 로직) : DB데이터 등록, 조회, 수정, 삭제 (CRUD) , Service를 선정해서 업무위임
...
//        response.setContentType("text/html"); // 응답 컨탠트 타입(MIME) 지정. MIME 뜻 : Multipurpose Internet Mail Extensions)는 전자 우편을 위한 인터넷 표준 포맷
        response.setContentType("text/html; charset=UTF-8"); // MIME + encoding 동시작성 법

        // 응답바디 (헤더설정 이후 작성할 것)
        // 응답본문 작성을 위한 스트림객체가 이미 만들어져있어 가져와서 사용하면 된다.
