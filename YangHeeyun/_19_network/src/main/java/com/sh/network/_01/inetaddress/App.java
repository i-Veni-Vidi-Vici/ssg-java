...
public class App {
    public static void main(String[] args) {
        try {
            ...
            System.out.println(google.getHostAddress()); // 142.250.76.132

            // n개의 ip주소 확인(google은 1개, naver는 여러개)
            InetAddress[] navers = InetAddress.getAllByName("www.naver.com");
            for(InetAddress naver: navers)
                System.out.println(naver.getHostAddress());

            // 내 컴퓨터 ip확인 localhost 127.0.0.1
            InetAddress localHost = InetAddress.getLocalHost();
            System.out.println(localHost.getHostAddress()); // 192.168.0.30
