 ...
 * 0. 요구사항 정리
 * - 운전프로그램을 만들어주세요.
 * - 운전자는 시동걸기, 악셀 밟기, 브레이크 밟기,시동을 끌 수 있어야 한다.
 * - 자동차는 시동걸기,가속하거나, 감속하거나, 시동이 꺼질 수 있어야 합니다.
 * - 자동차는 처음엔 대기상태로 있습니다.
 * - 운전자에 으해 시동이 걸리고, 이미 시동이 걸려있다면 다시 걸 수 없어야 합니다.
 ...
 * - 시동켜기메뉴 선택 -> 운전자 시동켜기 -> 자동차 시동켜기
 * - 가속메뉴 선택 -> 운전자 가속 -> 자동차 가속(현재속도를 변경)
 * - 감속메뉴 선택 -> 운전자 감속 -> 자동차 감속(현재속도를 변경)
 * - 시동끄기메뉴 선택 -> 운전자 시동끄기 -> 자동차 시동끄기
 * 3. 클래스 설계
 * - 사용자 클래스(실행클래스)
 * - 운전자 클래스(시동켜기,시동끄기,가속,감속 메소드)
 * - 자동차 클래스(시동켜기,시동끄기,가속,감속 메소드 | 시동상태, 속도값)
 *
