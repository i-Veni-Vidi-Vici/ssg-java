...
select
    ...
    sum(prod_count) over(order by sale_date) 전체제품누계
...
as (
    ...
    from
        tbl_category
    where
        ref_category_code is null -- root
    union all
    select
        tc.category_code,
        tc.category_name,
...
as (select
        ...
        emp_name,
        1 level,
        cast(emp_id as char(100)) path -- cast char 문자
    from
        employee
    where
        emp_id = 200
...
-- 사원이 입사하지 않은 날이 0명으로 표시가 되지 않고 이다.
select
...
with
    ...
    b as (select 1989 + row_number() over() from a a1, a a2, a a3) -- 3*3*3으로 행이 증가
select * from b;

with
    a as (select 1 union all select 1 union all select 1 union all select 1 union all select 1 union all select 1),
    b as (select 1989 + row_number() over() year from a a1, a a2) -- 6*6 = 36
select
    b.year,
    ifnull(e.cnt,0) cnt
from
