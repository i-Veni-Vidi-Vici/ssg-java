...
from
    tbl_menu
order by
    1; -- 컬럼순서. 첫번째 컬럼을 기준으로 하겠다(여기서는 menu_code)
#     2; -- 두번째 컬럼을 기준으로 하겠다(여기서는 menu_name)select
...
select field('X','A','B','C'); -- 0 (mysql에서는 존재하지 않는 인덱스)
...
from
    tbl_menu
order by
   field(orderable_status, 'Y','N'); -- 1~2 정렬
...
-- is null. null이면 true 1, null이 아니면  false 0
select
    *
from
    tbl_menu
order by
    category_code is null, -- is null asc에서는 null값이 맨 밑에 옴
    category_code;
...
from
    tbl_menu
order by
    category_code is null desc, -- is null desc에서는 null값이 맨 앞에 옴
    category_code desc;
