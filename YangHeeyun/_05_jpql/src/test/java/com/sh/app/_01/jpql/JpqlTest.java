 ...
 * 2. 쿼리 객체는 `TypedQuery`, `Query` 두 가지가 있다. (타입이 명시됐으면 typedQuery, 명시되지 않았으면 그냥 Query)
...
public class JpqlTest {
    ...
    void test1() {
        // given
//        String jpql = "select * from tbl_menu";
        // 테이블 별칭을 선언하고, select절에서 사용해야 한다. *를 사용할 수 없다.
        String jpql = "select m from Menu01 as m"; // Entity(name = "Menu01")
        // when
        TypedQuery<Menu> query = this.entityManager.createQuery(jpql, Menu.class);
        List<Menu> menus = query.getResultList(); // 결과가 여러건이니까 getResultList
//        List<java.awt.Menu> menus = query.getResultList(); // Typed이므로 반환자료형이 List<Menu>가 아니면 오류가난다
        /**
         * Hibernate:
         *     select
         *         m1_0.menu_code,
    ...
    void test2() {
        ...
        String jpql = "select m.menuName from Menu01 as m where m.menuCode = 7";
        // when
        TypedQuery<String> query = this.entityManager.createQuery(jpql, String.class); // m.menuName의 type
        String menuName = query.getSingleResult(); // 결과가 한 행이므로 .getSingleResult
        /**
         * Hibernate:
         *     select
         *         m1_0.menu_name
         ...
         *         m1_0.menu_code=7
         */
        System.out.println(menuName); // 민트미역국
        // then
    ...
    void test3() {
        ...
        // when
        // 타입지정 없는 쿼리 - TypedQuery와는 다르게 반환타입을 컴파일타임에 체크할 수 없다.
        Query query = this.entityManager.createQuery(jpql);
        List<Menu> menus = query.getResultList(); // 타입이 정해지지 않았으므로 query결과가 Menu엔티티인지 알 수 없으므로 주의만 줌
