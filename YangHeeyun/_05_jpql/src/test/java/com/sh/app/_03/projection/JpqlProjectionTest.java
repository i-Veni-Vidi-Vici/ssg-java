 ...
 *
 * 2. 임베디드(VO) 타입 프로젝션(임베디드 타입에 대한 설명은 MenuInfo 클래스에서 설명)
...
public class JpqlProjectionTest {
    ...
    void test2() {
        ...
        // then
        assertThat(menuVos).isNotNull() // menuVos 리스트가 null이 아닌지 확인합니다.
//                .allMatch((menuVo) -> menuVo instanceof MenuVo); // menuVo`가 `MenuVo`의 인스턴스이면 `true`를 반환하고, 그렇지 않으면 `false`를 반환
                .allSatisfy(menuVo -> assertThat(menuVo).isInstanceOf(MenuVo.class)); // MenuVo 클래스의 인스턴스인지 확인합니다.
    }

    @Test
    @DisplayName("3. 스칼라 프로젝션") // 값이 하나여서 스칼라라함
    void test3() {
        // given
        String jpql = """
                select
    ...
    void test4_0() {
        ...
        String jpql = """
                ...
                """;
        // when
        Query query = this.entityManager.createQuery(jpql); // 반환타입을 지정할 수 없으므로 그냥 Query
        List<Object[]> rows = query.getResultList(); // select결과 자료형이 2개 이상일때 반환타입을 지정하지 않으면 디폴트로 List<Object[]>가 자료형이 됨
        // [{1L, 식사, 123}, {2L, 디저트, 123}, .... ] // Object[] 안은 어떤 모습? (한 행을 들고와 그대로 object배열에 넣음)
        /*
            select
                c1_0.category_code,
                c1_0.category_name,
    ...
    void test4() {
        ...
        // DTO 수정은 영속성컨텍스트 또는 DB에 어떤영향도 줄수 없다. 영속객체가 아니기 때문에
