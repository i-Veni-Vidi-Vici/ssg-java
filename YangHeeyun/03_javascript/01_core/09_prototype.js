...
function Car(name){
    // this는 현재 객체를 가리킨다.
    this.name = name;
    // 공통(중복)코드를 prototype(내장객체)에 작성한다.
    // this.run = function(){
    //     console.log(`${this.name} 이/가 부릉부릉 달린다.`);
    // }
}
...
car2.forEach((car) => car.run());

/*
Car 생성자함수
- prototype속성으로 실제 프로토타입객체를 참조한다.
Car.prototype 프로토타입객체 (Car함수 생성시 프로토타입 객체도 자동으로 생성된다)
- constructor속성으로 생성자함수를 참조한다.
생성된 car (new Car()) 
- [[Prototype]]속성으로 프로토타입객체를 참조한다.
- 프로토타입은 자바스크립트 객체의 부모객체이다. (상속개념과 비슷)
...
function A(){
    // 현재겍체 this는 new연산자 호출결과로 만들어질 객체
    // this.id = '가나다';
    // this.hello = function(){
    //     console.log('안녕');
    // };
...
a.hello(); // 안녕
// A객체에 아무것도 적혀있지 않으면(주석처리한 부분) 부모인 prototype의 'ABC'와 'Hi'를 출력한다.\

console.log(A.id); // 123 static(생성자함수)속성
A.hello(); // 456