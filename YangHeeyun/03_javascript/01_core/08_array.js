...
.includes(요소) : 존재하는 요소가 있으면 true, 없으면 false 반환
*/
console.log(`foods.indexOf('물회') : ${foods.indexOf('물회')}`); // 0
console.log(`foods.lastIndexOf('물회') : ${foods.lastIndexOf('물회')}`); // 4
console.log(`foods.indexOf('막국수') : ${foods.indexOf('막국수')}`); // -1
console.log(`foods.includes('물회') : ${foods.includes('물회')}`); // true
console.log(`foods.includes('막국수') : ${foods.includes('막국수')}`); // false

/*
unshift(E) 요소를 0번지에 추가
shift(): E 0번지 요소를 제거후 반환
push(E) 요소를 맨 마지막에 추가
pop(): E 맨 마지막요소를 제거후 반환
*/
const stations = ['교대','강남','역삼'];

stations.unshift('서초');
stations.push('선릉');
console.log(stations); // [ '서초', '교대', '강남', '역삼', '선릉' ]

console.log(stations.shift()); // 서초
console.log(stations.pop()); // 선릉

// 교대 요소를 제거한 뒤 맨 앞에 다시 넣고 싶다
stations.unshift(stations.pop());

console.log(stations); // [ '교대', '강남', '역삼' ]

/**
 * arr1.concat(arr2) : 두 배열의 요소를 합친 새 배열을 반환 (immutab;e)
 */
const chars1 = ['가','나','다']; 
const chars2 = ['a','b','c'];

// const chars = chars1.concat(chars2);
// concat대신 spread를 쓸 수 있다.
const chars = [...chars1, ...chars2];
console.log(chars); // [ '가', '나', '다', 'a', 'b', 'c' ]
console.log(chars1,chars2); // [ '가', '나', '다' ] [ 'a', 'b', 'c' ]

/**
 * slice(startIndex, endIndex) : startIndex부터 endIndex전까지의 요소를 새 배열에 반환
 */
const origin = ['a','b','c','d','e'];
const copied1 = origin.slice(2,4); // [ 'c', 'd' ]
console.log(copied1);
const copied2 = origin.slice(2); // [ 'c', 'd', 'e' ] 2번째 인자를 적지 않으면 끝까지 slice한다
console.log(copied2);
console.log(origin); // [ 'a', 'b', 'c', 'd', 'e' ] slice해도 값은 변경되지 않는다

/*
splice(startIndex, delCount, newItem1, newItem2, ...) : 삭제와 요소추가를 동시에 하는 메소드
삭제된 요소가 반환된다.
*/
...
console.log(cars); // cars = [ '카니발', '그랜져', '소나타' ]
console.log(cars2); // cars2 = [ '카니발', '그랜져', '소나타' ] cars = ['소나타','그랜져','카니발'];
