...
두가지 키워드를 통해 변수 선언
1. const (상수선언)
2. let (변수선언) 진짜 변경되어야 할 값만 사용
var도 있었는데 이제 거의 사용하지 않음(직접 사용할 필요없다. 레거시브라우져 호환용)
*/
const n = 123;
console.log(n);
...
/**
 * 1. undefined
 * - 변수 공간은 있는데 아직 값이 할당되지 않은 상태를 가리키는 타입이자 값
 */
let a;
...
let c = 123.456;
console.log(c,typeof c); // 123.456 number 실수도 number

// 정수,실수 간의 연산은 모두 실수로써 처리된다.
console.log(10/4); // 2.5   자바의 경우, int형으로 계산해서 2가 나옴
