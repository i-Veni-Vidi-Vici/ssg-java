 ...
 * - noneMatch(Predicate<T>): boolean 모든 요소의 Predicate 람다식의 결과가 false라면, 최종 true반환
 * </pre>
 */
...
//        supplier.get().forEach(System.out::println);
        ...
        // 이름 글자수가 3글자보다 많은 이웃이 있는지 -> true
        System.out.println(supplier.get().anyMatch((person -> person.getName().length() > 3)));

        // 모든 이웃이 성인인지 검사 -> true
        System.out.println(supplier.get().allMatch((person -> person.getAge() > 19)));
        System.out.println(supplier.get().noneMatch((person -> person.getAge() < 19)));
    }
}
