 ...
 * 3. Procy.newProxyInstance(클래스로더, 클래스객체, InvocationHandler):
 *
 * </pre>
 */
...
public class App {
    public static void main(String[] args) {
        ...
        // 2. InvocationHandler객체 (target 메소드 대리 호출, 부가기능) 생성
        StudentHandler studentHandler = new StudentHandler(student); //스튜던트 핸들러를 이때 전달
        // 3 . 프록시 생성
        Student proxy = (Student) Proxy.newProxyInstance(
                student.getClass().getClassLoader(),
                new Class[]{Student.class},
                studentHandler); // 제네릭 안되있어서 다운캐스팅 해줘야된다. ? 이게 뭔소리?

        // 스프링 IocContainer는 위와같이 Proxy빈을 생성후 의존 주입한다.
        proxy.study(8);
        System.out.println();
