 ...
 * Generics
 * - 사전적의미는 일반화이다.
 * - 자바에서 제네릿은 타입을 일반화한다는 의미, 타입을 제한하는 용도로 사용한다.
 *
 * - 클래스내부의 특정필드,매개변수,리턴타입들을 제한할 목적으로 사용한다.
...
public class App {
    public static void main(String[] args) {
     ...
     intBox.setValue(123);
//     intBox.setValue("ㅋㅋㅋㅋ"); // Interger가 아닌 타입은 사용할 수 없다.
     Integer value = intBox.getValue(); // Integer외의 타입을 반환할 수 없다.
        System.out.println(value);


        //jdk7부터 타입추론을 통해 생성자쪽 타입은 생략가능하다.
        Box<String> strBox = new Box<String>(); // String만 사용할 수있는 박스 객체
        // 스트링으로 제한을 걸어버리면
...
//        strBox.setValue(123); // String이 아닌 타입을 사용할 수 없다.
        ...
        box.setValue(true);
        box.setValue("ㅎㅎㅎㅎㅎ"); // 오만게 ㄴ다옴
        box.setValue(LocalDateTime.now());
        // 리턴받은 값의 타입이 Object이므로 다운캐스팅이후 사용할수 있다.

        Object value3 = box.getValue();
        LocalDateTime _values3 = (LocalDateTime) value3; // 형변환 해줘야함 이거 안해주면 포맷사용못함
        System.out.println(_values3.format((DateTimeFormatter.BASIC_ISO_DATE)));
