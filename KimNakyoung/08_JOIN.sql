...
select
    b.category_code -- 테이블 별칭을 생략가능하나 되도록이면 작성하자
from
...
-- ** 기준컬럼이 null이거나, 상대테이블에 매칭괴는 행이 없다면, relation에서 제외된다. **
-- inner 키워드는 생략이 가능하다.
...
from
    ...
    on m.category_code = c.category_code; -- 이거쓰면 카테고리코드 뒤에 있음
#     using (category_code); -- 22행 -- 이거쓰면 카테고리 코드가 맨처음으로 튀어나온다.

# RIGHT OUTER JOIN
-- 오른쪽 테이블 기준으로 조인
-- tbl_category.category_code가 상대테이블 tbl_menu에서 사용되지 않아 제외됐던 4행이 추가되었다.
-- tbl_menu.category.code가 null인 찰순대쥬스행은 제외되었다.
...
select distinct
    d.DEPT_ID,
    d.DEPT_TITLE
    from employee e join empdb.department d
        on  e.DEPT_CODE = d.DEPT_ID;
...
-- SELF JOIN
-- ------------------------------
-- 자가조인, 같은 테이블 좌우측에 두고 다른테이블인양 조인.
-- 같은 테이블 안에서 다른 레코드를 참조하고 있을때 자가조인 할 수 있다.

select * from tbl_category; #  상위 카테고리 참조
...
select
    ...
    *
from
    tbl_category c1 inner join tbl_category c2
...
select * from employee; -- MANAGER_ID EMP_NAME

-- 사번 사원명 매니져사번 매니져명 조회
-- 내부조인
-- 1. e테이블에서 manner_id가 null인 행 제외
-- 2. m 테이블에서는 m.emp_id가 상응하는 e.manager_id가 없는 경우 제외 (관리하는 부하직원이 없는경우)
...
from
    employee e join employee m
                    on e.MANAGER_ID = m.emp_id; -- 24행 (16행 + 8행 ) : manager_id null


-- 우측외부조인
-- 매니저-사원 조회( 관리하는 직원이 없는 매니져도 조회)
select
    m.EMP_id,
    m.EMP_NAME,
#     e.MANAGER_ID,
...
from
    employee e right join employee m
        on e.MANAGER_ID = m.emp_id; -- 33행 (16행 + 17행 ) : manager_id null 포함
