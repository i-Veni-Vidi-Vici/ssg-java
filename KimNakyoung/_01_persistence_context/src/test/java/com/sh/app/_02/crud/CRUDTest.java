...
public class CRUDTest {
    ...
    void test1() {
        ...
        assertThat(menu).isNotNull()
                .satisfies(
                        ...
                        (_menu) -> assertThat(_menu.getMenuPrice()).isNotNull(),
     ...
     *
     * <pre>
     * EntityTransaction객체를 통해서 트랜잭션 범위 및 commit/rollback처리를 해줘야된다.
     * </pre>
     *
     */
    ...
    void test3() {
        ...
        try {
            ...
            // 근데 여기서는 바로 쿼리 날라감 왜? MENU에서 (@GeneratedValue 설정함.
            System.out.println("menu가 영속성 컨텍스트에 저장되었습니다.");
            transaction.commit(); // 오류가 없다면 commit // DB에 쿼리 질의 insert into.. 날라감!
        } catch (Exception e) {
        ...
        assertThat(entityManager.contains(menu)).isTrue(); // 메뉴객체에 menu가 포함되어있니? yes
    }
    ...
    void test4() {
        ...
        Menu menu = entityManager.find(Menu.class, 1L); // 조회하는 쿼리 바로 날라감
    ...
    void test5() {
        ...
        try {
            //entity 등록/수정/삭제 코드
            entityManager.remove(menu); // 엔티티매니저야 가지고 있는 객체 지워
            System.out.println("영속성컨텍스트 메뉴 객체의 정보가 변경되었습니다." + menu);
            transaction.commit();
        } catch (Exception e) {
            transaction.rollback();
