package com.sh.custom.type.encapsulation;



/**
 * <pre>
 *  객체지향의 3대원칙 !!외우세요!!
 *  1. 캡슐화
 *  2. 상속
 *  3. 다형성
 *
 *  캡슐화 Encapsulation
 *  - 필드/메소드를 클래스 내부에 함께 작성하는 것.
 *  - private 필드의 직접접근을 방지하고, public 메소드를 톻해 제어하는 것.
 *  - 은닉화 ( 중요한 정보를 외부에 숨기는 것)
 * </pre>
 *
 */
public class App {
    public static void main(String[] args) {

        Account account = new Account();
//        account.name = "홍길동";
//        account.balance = 1_000_000L;

        account.init("홍길동", 1_000_000);


        account.printInfo();

//        System.out.println(account.name + "님의 잔액은 " + account.balance + "원 입니다.");

        // 입금
//        account.balance += 1_000_000L;

        // 변수를 직접 제어해서 생기는 문제들 -> 메소드를 통해 제어하자! (입금메소드 deposit)
//        account.balance += -1_000_000L; // 음수 입금
//        account.balance *= 1_000_000L; // 입금 연산 잘못하기



        account.deposit(1_000_000);
        account.deposit(-1_000_000); // 입금처리 안됨.
//        System.out.println(account.name + "님의 잔액은 " + account.balance + "원 입니다.");
        account.printInfo(); // private로 변수를 선언해서 메소드로 호출하기 직접 호출 x

        // 출금
//        account.balance -= 500_000;
        // 변수를 직접 제어해서 생기는 문제들 -> 메소드를 통해 제어하자! (출금메소드)
//        account.balance -= -500_000; // 음수 출금 하면 더하기 처리됨
//        account.balance -= 5_000_000; // 잔액보다 많은 금액 출금

        account.withdraw(500_000);
        account.withdraw(-500_000);
        account.withdraw(10_000_000);
//        System.out.println(account.name + "님의 잔액은 " + account.balance + "원 입니다.");
        account.printInfo();
    }
}
