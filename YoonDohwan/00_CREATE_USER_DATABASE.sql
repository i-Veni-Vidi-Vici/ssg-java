...
-- 일반사용자 생성
-- '사용자명'@'접속위치'%로 지정하면 외부접속 허용한다. localhost로 지정하면 내 컴퓨터에서만 접근 가능하다.
...
-- 데이터베이스(테이블등의 객체가 보관되는 장소) 추가
-- mysql에서는 schema와 database가 같은의미로 사용된다.
create database menudb;
create schema menudb;

-- sh사용자에게 menudb를 사용할 수 있는 권한을 부여한다.
-- 사용가 user와 database(schema)는 별개의 객체이다.
