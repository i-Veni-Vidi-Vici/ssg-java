...
# 문법
-- window b_function(인자) over([partition by] [order by] [windowing절])
-- 인자 : 윈도우함수별로 0~n개 작성
-- over절
...
select concat(space((level - 1) * 5), name, '(', id, ')') 조직도,
       level,
       path
...
-- 특정수의 거듭제곱식으로 행추가
with a as (select 1 union all select 1 union all select 1 union all select 1 union all select 1 union all select 1),
     b as (select 1989 + row_number() over () year
           from a a1,
                a a2) -- 6 * 6
select b.year,
       ifnull(e.cnt, 0) cnt
from b
         left join (select year(hire_date) hire_year,
                           count(*)        cnt
                    from employee
                    group by year(hire_date)) e
