...
// var (직접 사용할 필요없다. 레거시브라우져 호환용으로만 생각)
...
let x = 123;
x = null;
console.log(x, typeof x); // null object (js 초기부터 object타입에 대한 type tag와 실제값 0이 동일하게 생긴 이슈)
...
console.log(obj); // { id: 12345, type: '객체' }
...
foo(); // 함수 호출 실행 foooooooooooooooooooo
console.log(foo, typeof foo); // [Function: foo] function => object 하위 객체(function)

/**
 * 자바스크립트는 동적타입언어이다.
 * - 정적타입언어 (c, c++, java, ...) 변수에 자료형이 고정된 언어. 컴파일타임에 타입체크가 가능하므로, 런타임오류 줄일 수 있다.
 * - 동적타입언어 (js, python, ...) 
