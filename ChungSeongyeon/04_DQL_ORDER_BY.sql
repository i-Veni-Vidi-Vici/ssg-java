...
select *
from tbl_menu
# order by menu_code asc; 오름차순 기본값
# order by menu_code desc; 내림차순
# order by menu_name asc;
order by menu_name desc;

-- 두 개 이상의 컬럼에 대해서 정렬 지원
-- 카테고리코드 오름차순, 가격 내림차순
-- 우선 카테고리 오름차순 적용, 카테고리코드가 같은 경우 두번째 가격 내림차순
select *
from tbl_menu
...
order by category_code is null, -- is null asc에서는 null값이 가장 밑에 온다
         category_code;

-- 카테고리 내림차순, null값을 맨 위로 보내기
select *
from tbl_menu
order by category_code is null desc, -- is null desc에서는 null값이 가장 먼저 나온다
         category_code desc;
