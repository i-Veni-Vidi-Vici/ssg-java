...
-- order by 절은 마지막 테이블에서 한번만 사용 가능.
-- 컬럼명이 다르다면, 첫번째 테이블의 컬럼명을 사용한다.
-- 컬럼의 타입이 다른 경우, 문자열타입으로 변환되어 처리된다.

# 구분
-- union all 합집합 (연결만 지원)
-- union 합집합 (연결 후 중복 제거 처리)
...
SELECT *
FROM tbl_sales
WHERE YEAR(sale_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 2 MONTH))
  AND MONTH(sale_date) = MONTH(DATE_SUB(CURDATE(), INTERVAL 2 MONTH));
-- 한 줄 짜리 WHERE절
#  extract(year_month from sale_date) = extract(year_month from curdate() - interval 1 month); -- 지난달 데이터
#  extract(year_month from sale_date) = extract(year_month from curdate()); -- 이번달 데이터
