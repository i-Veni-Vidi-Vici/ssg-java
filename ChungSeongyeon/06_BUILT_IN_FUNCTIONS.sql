...
select concat('사자', '호랑이', '토끼'), -- 사자호랑이토끼
...
select now(), -- 2024-04-16 09:49:29
       adddate(now(), 1), -- 2024-04-17 09:49:29
       adddate(now(), interval 1 day), -- 2024-04-17 09:49:29
       adddate(now(), interval 1 month), -- 2024-05-16 09:49:29
       adddate(now(), interval 1 year); -- 2025-04-16 09:49:29

select adddate('2024-01-31', interval 1 month); -- 2024-02-29

select subdate(now(), 1), -- 2024-04-15 10:06:31
       subdate(now(), interval 1 day), -- 2024-04-15 10:06:31
       adddate(now(), -1), -- 2024-04-15 10:06:31
       adddate(now(), interval -1 day); -- 2024-04-15 10:06:31
...
select now(), -- 2024-04-16 10:18:25
       sysdate(), -- 2024-04-16 10:18:25
       localtime(), -- 2024-04-16 10:18:25
       localtime, -- 2024-04-16 10:18:25
       localtimestamp(), -- 2024-04-16 10:18:25
       localtimestamp; -- 2024-04-16 10:18:25
...
select
    ...
    datediff(now(), '2024$04$20'); -- -4    $ 대신 / 도 OK
...
-- 다음달 말일은?
select
    last_day(now() + interval 1 month), -- + interval로 5월 말일 구하기
    last_day('2024-05-31' + interval 1 month); -- 2024-06-30 말일이 29,30,31 뭐든 말일을 출력해줌

select '2024-05-31' + interval 1 month; -- 2024-06-30
...
select
    ...
    date_format(now(), '%H:%i:%s'), -- 11:16:00
    date_format(now(), '%Y년/%m월/%d일 (%W)'); -- 2024년/04월/16일 (화요일)

-- 지역 변경
select @@lc_time_names; -- en_US
set @@lc_time_names = 'ko_KR'; -- 한국으로 변경
...
select
    123,
    cast(123 as char), -- 문자열로 바뀜(왼쪽 정렬됨.)
    convert(123, char); -- 문자열로 바뀜(왼쪽 정렬됨.)

select
    123.456,
    cast(123.456 as signed integer),
    convert(123.456, signed integer); -- signed(부호를 갖는 값)/unsinged(부호를 가지지 않는 값. 오직 양수)
#     convert(-123, unsigned integer); 이렇게 쓰지 마!!!

# 암묵적 형변환
select '1' + '2'; -- 3 숫자 변환후 1+2=3 처리됨.
select '1' + 'ㅋㅋㅋ'; -- 1 숫자로 변환불가한 문자열은 0으로 변환후 1+0=1 처리
select concat(1, 2, 3, '원'); -- 123원.. 숫자가 문자열로 자동변환후 연결 '1', '2', '3', '원'
select adddate('2024-04-01', 1); -- 1이 자동으로 date형으로 변환 후 + 1일 됨.

select '1' + '3만원'; -- 1+3 숫자로 시작되는 문자열은 숫자부분만 변환되서 1+3 = 4(결과)
select '1' + '내가가진3만원'; -- 1 + 0 = 1 숫자로 시작한 게 아니라 문자로 시작해서 0임.

select 3 > '오'; -- 1(true) 3>0 문자'오'->0으로 암묵적 형변환 후 크기 비교
select 3 > '5오오오홍'; -- 3>5 0(false)
...
from tbl_menu;
-- ifnull구문 추가하는 순간 문자형으로 바뀜.
-- 결과집합에서 하나의 컬럼은 모두 동일한 자료형이어야 하기 때문에! 숫자 -> 문자
...
select
    menu_name 메뉴명,
    menu_price 가격,
    if(orderable_status = 'Y', '주문가능', '주문불가') 주문가능여부 -- 주문가능을 ''으로 여백만들면 주문불가가 눈에 더 띄어요
from tbl_menu;

# isnull(값) null여부 반환
select
    isnull('ㅋㅋㅋㅋㅋ'), -- 0(false)
    isnull(null); -- 1(true)
...
select
    ...
    menu_price,
    case
        when menu_price < 5000 then '싼 것'
        when menu_price between 5000 and 10000 then '적당한 것'
        when menu_price between 10000 and 20000 then '쪼금 비싼 것'
        else '겁나 비싼 것' -- 한줄로 쭉 써도 된다~
    end as 'menu_label' -- as '~' 별칭
from tbl_menu;
...
#     menu_name, -- [42000][1140] In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'menudb.tbl_menu.menu_name'; this is incompatible with sql_mode=only_full_group_by
    sum(menu_price) 전체메뉴가격 -- 235500
from tbl_menu;

# avg
-- 특정 컬럼의 평균값 반환
select avg(menu_price) 평균가격, -- 10704.5455
       truncate(avg(menu_price), 1) -- 10704.5
from tbl_menu;

# count
-- 특정 컬럼의 값의 개수를 세어 반환. null 값은 처리하지 않는다.
select count(*), -- 22 * 행(모든 컬럼)의 수
       count(category_code) -- 21 -> 찰순대쥬스의 category_code가 null이기 때문에
from tbl_menu;
...
-- 결과는 21로 같지만 처리과정에는 차이가 있다.
-- 22개 일단 다 갖고와서 하나씩 비교 후 없는 거 제외하고 21 반환.
-- where 절이 있는 경우 찰순대쥬스 아예 제외하고 행의 개수 세서 21 반환
