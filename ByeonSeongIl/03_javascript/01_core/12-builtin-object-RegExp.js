 ...
 * - 조건/반복처리에 비해 간결한 코딩 가능
 * - 학습비용이 요구되는 편.
 * - 언어독립적 (타 프로그래밍언에서 사용가능)
 */
...
console.log("a1b2c3".search(re1)); // 1 매칭된 문자열의 인덱스반환
...
console.log(re.test('acbbbbbbbbb')); // true
...
console.log(target.replace(/[^A-Z]/g, '<$&>'));                                               // ABCDEFGhijklmn< ><가><나><다><라><마><바><사><ㅋ><ㅋ><ㅎ><ㅎ><ㅓ><ㅓ><ㅏ><ㅏ>< ><1><2><3><4><5><6><7><8><9><0>< ><!><@><#><$><%><^><&><*><(><)>< ><	>< >
...
/**
 * Escaping 
 * . ^ $ | [] 정규식에서 특수한 문자를 찾고 싶은경우 escaping이 필요
