 ...
 * - 부모타입의 참조변수의 제어할 수 있다.
 * - 제어되는 타입별로 다르게 작동할 수 있다.
 * - 상속을 기반으로 하는 기술 (부모의 타입 물려받으므로, 해당 타입으로 제어가 가능하다.)
 *
...
public class App {
     ...
     *
     * 동적바인딩
     * - 상속/다형성이 적용되고, 해당 메소드를 오버라이드 했다면, 호출 시 실행시간에 객체 기준으로 메소드를 다시 바인딩하는 것
     *
     * </pre>
     */
    private void test4() {
    ...
    private void tese2() {
        ...
        Animal animal2 = rabbit; // upcasting 위 코드와 동일한 과정
        ...
        Rabbit rabbit2 = (Rabbit) animal2; // 위와 동일한 과정
...
//        object1.eat(); // 자식 타입에 선언된 메소드는 호출불가
//        object1.run(); // 자식 타입에 선언된 메소드는 호출불가
//        object1.cry(); // 자식 타입에 선언된 메소드는 호출불가
