...
public class JpqlProjectionTest {
    ...
    @Test
    @DisplayName("1.Entity 프로젝션 - Category")
    void test1() {
        // given
        String jpql = """
                select
    ...
    @Test
    @DisplayName("2. Embedded(VO) 프로젝션")
    void test2() {
        // given
        String jpql = """
                select
                ...
                from
                    Menu0302 as m
                """;
        // jpql은 엔티티를 대상으로 하기 때문에 MenuVo는 Vo이기 때문에 찾지 못한다.
...
//                .allMatch((menuVo) -> menuVo instanceof MenuVo);
    ...
    @Test
    @DisplayName("3. 스칼라 프로젝션 (값1개 가져오기)")
    void test3() {
        // given
        String jpql = """
                select
    ...
    void test4() {
        ...
        // DTO 수정은 영속성 컨텍스트 또는 DB에 어떤 영향도 줄 수 없다. (영속객체가 아니기 때문)
