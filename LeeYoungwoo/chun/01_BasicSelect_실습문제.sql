...
-- 3. "국어국문학과" 에 다니는 여학생 중 현재 휴학중인 여학생을 찾아달라는 요청이들어왔다. 누구인가?
-- (국문학과의 '학과코드'는 학과 테이블(TB_DEPARTMENT)을 조회해서찾아 내도록 하자)
select s.STUDENT_NAME
from tb_student s join tb_department d on s.DEPARTMENT_NO = d.DEPARTMENT_NO
where d.DEPARTMENT_NAME = '국어국문학과' and ABSENCE_YN = 'Y' and substring(s.STUDENT_SSN, 8, 1) in (2, 4);
...
select CLASS_NO
from tb_class
where not isnull(PREATTENDING_CLASS_NO);
...
select STUDENT_NO,
       ...
       STUDENT_SSN
from tb_student
where year(ENTRANCE_DATE) = 2019 and ABSENCE_YN = 'N' and STUDENT_ADDRESS like '%전주시%'
order by 2;