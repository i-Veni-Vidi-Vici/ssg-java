...
// var (옛날거/ 직접 사용할 필요없다. 레거시브라우져 호환용)
const n = 123;
console.log(n);
...
const str1 = "홍길동";
const str2 = '신사임당';
const str3 = `이순신`; // ES6(ES2015) 버전에 추가된 Template String 문법 : ``안에서 개행/탭문자를 인식. ${}표현식을 통해 다른 변수 출력가능
...
let x = 123;
x = null;
console.log(x, typeof x); // null object (js 초기부터 object타입에 대한 type tag와 실제 값 0이 동일하게 사용해 생긴 이슈)

/**
 * 6. bigint
 *  - 길이 제약없이 정수를 제어하는 타입
 */
const bigNum = 12345678909876543210147258369n; // 접미사 n을 통해 bigint를 명시
console.log(bigNum, typeof bigNum); // 12345678909876543210147258369n bigint
...
/**
 * 8. object
 *  - js 자료형 중 유일한 객체형 (나머지 7가지는 모두 원시형)
 *  - 속성/메소드를 가진 자료형
 * 
 * object의 하위타입
 *  - object
 ...
 * 자바 스크립트는 동적타입 언어이다.
 * - 정적타입언어 (c, c++, java...) 
 *      변수에 자료형이 고정된 언어. 컴파일타임에 타입체크가 가능하므로, 런타임 오류를 줄일 수 있다.
 * - 동적타입언어 (js, python...) 
