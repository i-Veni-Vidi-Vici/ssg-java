 ...
 *  - UrlResource 웹상의 리소스
 *  - ClassResource 클래스패스상의 리소스
 *  - FileSystemResource 서버컴퓨터상의 리소스
 *  - ServletContextResource 웹 루트 디렉토리에서 접근 가능한 리소스...
 * </pre>
 */
@Controller
@Slf4j
public class FileUploadController {
    ...
    public String singleFile(
        ...
        String originalFilename = singleFile.getOriginalFilename(); // abc.png -> 마지막 .의 인덱스는 3,
        String ext = originalFilename.substring(originalFilename.lastIndexOf(".")); // 3번지 이후를 잘라내라 -> .png가 담긴다.
        String saveName = UUID.randomUUID() + ext; // abds1234-2352-63er.png
        //  3. 저장
        singleFile.transferTo(new File(dir, saveName)); // 실제 서버컴퓨터에 저장

        // 결과 피드백
        model.addAttribute("message", "파일 업로드 성공 👍");
        model.addAttribute("img", "upload/single/" + saveName); // 클라이언트 호출할 경로
        return "result";
    }
    ...
    public String multiFile(
        ...
        for (MultipartFile multipartFile : multiFiles) {
            //  2. 저장할 이름 새로 부여(인코딩이슈/보안이슈를 대비한 것)
            String originalFilename = multipartFile.getOriginalFilename(); // abc.png -> 마지막 .의 인덱스는 3,
            String ext = originalFilename.substring(originalFilename.lastIndexOf("."));// 3번지 이후를 잘라내라 -> .png가 담긴다.
            String savedName = UUID.randomUUID() + ext; // abds1234-2352-63er.png
            //  3. 저장
