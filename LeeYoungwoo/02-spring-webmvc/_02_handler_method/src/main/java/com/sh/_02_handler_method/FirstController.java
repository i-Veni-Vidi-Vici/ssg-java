...
@Slf4j
@SessionAttributes({"id"}) // 모델에 id속성값이 저장되면 세션으로 관리해줘
...
//    }
    public void regist() {
    } // /first/regist path에서 viewName을 유추하기 때문에 return 안해줘도 돌아간다.
    ...
    public void modify(Model model) {
        log.info("GET /first/modiry 요청!");
        // 기존데이터
        MenuDto menuDto = new MenuDto(1L, "차돌짬뽕", 30000, 1, "Y");
        // view단에 데이터 전달
     ...
     *  - name값과 변수명이 다르다면, name값을 직접 명시할 수 있다. -> @RequestParam("name")
     ...
     *  - 모델의 속성명을 변경할수도 있다. (기본값은 소문자로 시작하는 클래스명)
     */
    @PostMapping("/search")
    ...
    /**
     * Model
     *  - view단에 데이터를 전달하기 위한 목적(모델의 속성은 기본적으로 request scope를 가진다)
     *  - 만약 session scope에 속성값을 저장하려면, 클래스 레벨에 @SessionAttributes에 먼저 속성명을 등록해야 한다.
     */
    @PostMapping("/login2")
    ...
    /**
     * @SessionAttributes로 등록된 session 속성을 폐기할때는 HttpSession#invalidate() 사용할 수 없다.
     * SessionStatus 객체(@SessionAttributes로 등록된 session속성을 관리하는 객체)의 setComplete()를 호출해 만료시킬 수 있다.
     * - 속성을 개별적으로 삭제할 수 없다.
     */
    @GetMapping("/logout2")
