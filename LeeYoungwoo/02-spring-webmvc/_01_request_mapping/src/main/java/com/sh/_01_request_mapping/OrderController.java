...
/**
 * 클래스 레벨의 @RequestMapping과 메소드레벨의 @RequestMapping의 path값을 연결해 url을 매칭시킨다
 * 중복된 url의 관리가 수월하다.
 * 클래스레벨의 *(와일트카드)와 메소드레벨의 path없는 @RequestMapping을 사용하면, 기본 url처럼 처리할 수 있다.
 * @RequestMapping 안에 Get/Post 둘다 포함된다.
 */
@Controller
@RequestMapping("/order/*") // 모든 url이 @RequestMapping에 적힌 order + @Get(POST)Mapping밑에 적힌 주소로 나옴
...
//    }
    ...
    /**
     * 하나의 핸들러에서 여러 url을 처리할 수 있다. (반대는 불가)
     * @param model
     * @return
     */
    ...
    /**
     * - @RequestMapping의 path 속성 중 {변수명]에 해당하는 부분을 핸들러의 매개변수로 사용할 수 있다.
     * - 경로변수와 매개변수명이 같아야 한다.
    ...
    @RequestMapping // 준비되지 않은 url이 호출될때 이게 나옴
