...
-- INSERT
-- ------------------------------------------
-- 새로운 행을 추가. 결과적으로 테이블의 행 수가 늘어난다.
-- insert 요청시 처리된 행의 수가 반환된다. (Intellij에서는 Service뷰 - Output탭에서 확인 가능)
...
-- 3. bulk insert문법 : 대량의 데이터를 한번에 입력 가능
    -- insert into 테이블 values(컬럼값, 컬럼값2, ...), (컬럼값, 컬럼값2, ...), (컬럼값, 컬럼값2, ...)
 ...
 */
desc tbl_menu;
-- auto_increment 자동증가 컬럼(PK 고유식별컬럼)은 별도의 값을 작성하지 않아도 자동으로 고유한 숫자값을 발급해준다.
...
values (
        301, '함지민', '781020-2123453', 'hamham@ohgiraffers.com', '0102343334', 'D1', 'J4',
        'S3', 4300000, 0.2, '200'
       );
...
insert into
    employee(emp_id, emp_name, emp_no, email, phone, dept_code, job_code, sal_level, salary, bonus, manager_id)
values (
           302, '장채현', '901123-1080503', 'jang_ch@ohgiraffers.com', '01033334444', 'D2', 'J7',
           'S3', 3500000, null, '20'
       );

-- 쿠폰테이블
create table tbl_coupon (
    id int auto_increment primary key , -- 쿠폰식별번호 (중복되지 않는 식별분호를 자동으로 발급)
    coupon_name varchar(1000) not null , -- 쿠폰명
    discount_price int, -- 금액할인의 경우 할인금액
    discount_rate double, -- 비율할인의 경우 할인비율
...
desc tbl_coupon;
-- insert시에 생략 가능한 컬럼은?
-- -> id, discount_price, discount_rate, issued_at(not null 이지만 default값이 지정되어 있어 생략가능)
...
insert into tbl_coupon(coupon_name, discount_price,expires_at)
values
    ('대한독립기념쿠폰', 5000,'2024-05-22');

insert into tbl_coupon(coupon_name, discount_rate,expires_at)
values ('건국기념쿠폰',0.15,'2024-05-22');

insert into tbl_coupon(coupon_name, discount_price, issued_at,expires_at)
values
...
-- where절을 통해 대상 행을 찾지 못한 경우, 오류는 발생하지 않는다. (아무것도 수정안됨)
update
tbl_menu
set
    category_code = 7
where
#     menu_name = '파인애플탕';
    menu_code = 24; -- 중복값이 없는 식별컬럼(PK)을 통해 행을 찾는것이 좋다.

-- 서브쿼리를 통한 update
-- 우럭스무디와 같은 카테고리로 파인애플탕의 카테고리를 수정하세요.
-- 수정하고자 하는 테이블을 서브쿼리에서 접근할 수 없다. (mysql에서는)
update
    tbl_menu
set
...
-- DML은 트랜잭션 모드를 수동(Manual)으로 지정해서 쿼리실행, db반영을 구분할 수 있다.
-- 트랜잭션 수동모드에서는 쿼리 실행 후, 별도의 commit 명령을 통해 결과를 반영할 수 있다.
-- (rollback 명령을 통해 변경사항을 취소할 수 있다.)
use menudb;

update tbl_menu
set
    tbl_menu.menu_price = menu_price + 1000
...
set
    SALARY = (
        select SALARY
        from (select SALARY from employee where EMP_NAME = '유재식') s
        ),
    BONUS = (
        select BONUS
        from (select BONUS from employee where EMP_NAME = '유재식') b
        )
where
    EMP_NAME = '방명수';
...
-- 서브쿼리로 테이블을 생성하면, 테이블 NOT NULL 제약조건외에는 복사되지 않는다.(제약조건, default값등은 제외)
