...
public class RequestParameterServlet extends HttpServlet {
     ...
     *  - http://localhost:8080/03_request_parameter/test?name=홍길동&age=27&married=true
     *  - name=홍길동&age=27&married=true -> Query String 부분은 request 객체안에서 HashMap으로 관리된다.
     *  - HttpServletRequest#getParameter(name):String으로 처리할 수 있다.
     *  - 모든 사용자 입력값은 문자열로써 처리된다.
     *  - 단순조회요청은 전송방식 GET을 사용한다.
     *
     * @param req
     * @param resp
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 사용자입력값 QueryString 가져오기 (무조건 String이기 때문에 다르게 쓸거면 형변환 해줘야한다.)
    ...
    /**
     * POST 요청에서 사용자입력값은 http요청 메세지의 본문영역에 작성된다. (url에 노출되지 않는다.)
