...
-- 카테고리코드가 4번 또는 가격이 9000원 이면서 메뉴번호가 10번보다 큰 메뉴조회
-- and, or를 나열할 때 and연산자가 우선순위가 높다. -> ()사용해서 우선순위 나타내주기
select *
from
    tbl_menu
where
...
# LIKE 문자열 패턴 연산자
-- 비교하려는 컬럼값이 특정패턴을 만족시키면 true를 반환하는 연산자.
...
#     menu_name like '%마늘'; -- 마늘로 끝나는지 조회
    menu_name like '_마늘%'; -- 마늘앞에 한글자가 반드시 존재하고, 마늘 다음에는 0개 이상의 문자열이 존재하는지 체크
...
from
    tbl_menu
where
#     category_code = null; -- null값은 연산 불가
...
from tbl_menu
where
    category_code < 4 or category_code > 6;