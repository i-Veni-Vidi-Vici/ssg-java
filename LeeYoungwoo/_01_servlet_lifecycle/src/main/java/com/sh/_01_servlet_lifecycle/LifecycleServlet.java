 ...
 *  - Tomcat은 Servlet 객체를 Singletone 패턴으로 관리한다. (객체를 한번만 만들고 재사용한다.)
 *  - url별 첫번째 요청에 Tomcat은 Servlet 객체를 생성하고, 그 init(ServletConfig)메소드를 호출한다.
 *  - 실제 요청을 처리하는 것은 service 메소드이다.
 *      1. 요청이 오면, service(ServletRequest, ServletResponse)가 호출된다.
 *      2. 전속방식별로 그에 상응하는 메소드 호출 doGet, doPost 등이 있다.
 *  - Tomcat 종료시에 destroy메소드 호출 후 모든 Servlet 객체를 반환한다.
 */
