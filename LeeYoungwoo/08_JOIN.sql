...
-- 1. join : 레코드(행)와 레코드를 가로로 연결
-- 2. union(union all) : 레코드와 레코드를 세로로 연결

# JOIN 구분
-- 1. EQUI JOIN(등가조인) : 조인 조건에 = 동일비교 연산자를 사용한 경우
-- 2. NON-EQUI JOIN(비등가조인) : 조인 조건에 = 동일비교 연산자를 사용하지 않은 경우 (>, <, >=, <=, between, like, in)

# EQUI JOIN 구분
-- 1. INNER JOIN (내부조인) : 기준컬럼에 부합하는 행들만 조인하는 경우
-- 2. OUTER JOIN (외부조인)     
        -- LEFT OUTER JOIN  : 왼쪽 테이블 전체와 오른쪽 테이블의 기준컬럼에 부합하는 행들만 조인한 경우
        -- RIGHT OUTER JOIN : 오른쪽 테이블 전체와 왼쪽 테이블의 기준컬럼에 부합하는 행들만 조인한 경우
-- 3. CROSS JOIN
-- 4. SELF JOIN (자가조인)
...
select b.category_name -- 테이블 별칭을 생략가능하나 되도록 작성한다.
...
-- 메뉴/카테고리정보를 모두 조회 (카테고리없는 메뉴 제외, 메뉴 없는 카테고리 제외)
select
    count(*)
...
-- 기준이 된 테이블의 모든 행과 상대 테이블에서 조인조건에 부합되는 행이 사용된다.

# LEFT OUTER JOIN
-- 왼쪽 테이블 기준으로 조인
-- tbl_menu.category_code가 null인 찰순대쥬스행이 추가되었다.
-- tbl_category.category_code가 상대 테이블 tbl_menu에서 사용되지 않은 4행은 제외되었다.

-- 메뉴 조회 (카테고리가 없는 메뉴도 포함)
select *
...
# RIGHT OUTER JOIN
-- 오른쪽 테이블 기준으로 조인
-- tbl_category.category_code가 상대 테이블 tbl_menu에서 사용되지 않아 제외됬던 4행이 추가되었다.
-- tbl_menu.category_code가 null인 찰순대쥬스행은 제외되었다.
select *
from tbl_menu m right outer join tbl_category c
    on m.category_code = c.category_code; -- 25행(21행 + 4행:식사, 음료, 디저트, 퓨전(1,2,3,7) )

# 조인 결과 정리
-- tbl_menu(22행) , tbl_category(12행)
-- inner join 21행 -> tbl_menu 21행(category_code가 null인 행(찰순대쥬스) 제외) + tbl_category 8행(상대테이블에 매칭되는 행이 없는(1,2,3,7)행 제외)
-- left outer join 22행 -> tbl_menu 22행(*) + tbl_category 8행(상대테이블에 매칭되는 행이 없는(1,2,3,7)행 제외)
-- right outer join 25행 -> tbl_menu 21행(category_code가 null인 행(찰순대쥬스) 제외) +  tbl_category 12행(*)

-- 두 테이블의 inner join 결과를 확인(제외되는 행 확인)
-- 문맥에 따라 외부조인을 활용

-- left, right는 작성된 위치 기준이다.
select *
...
-- 내부조인 (22행)
-- employee (22행) -> dept_code가 null인 사원 2명 제외
...
-- 좌측외부조인 (24행) : 22행 + 2행 (dept_code가 null인 행 추가)
-- 사원조회(부서지정 안된 사원도 포함)
select *
from employee e left join department d
    on e.DEPT_CODE = d.DEPT_ID;

-- 우측외부조인 (25행) : 22행 + 3행 (imployee에 dept_id와 매칭되는 행이 없든 3행 추가 )
-- 부서조회(유령부서도 포함)
select *
...
-- 조인 조건절을 작성하지 않는다.
-- relation행의수는 좌축테이블수 * 우측테이블행수와 같다.
...
select *
from menudb.tbl_menu m cross join menudb.tbl_category c;

-- --------------------------------
-- SELF JOIN
-- --------------------------------
-- 자가조인 : 같은 테이블 좌우측에 두고 다른 테이블인양 조인
-- 같은 테이블안에서 다른 레코드를 참조하고 있을 때 자가조인 할 수 있다.

select * from menudb.tbl_category;

-- 카테고리 조회 (상위 - 하위)
-- SELF 내부조인
-- 카테고리 조회(상위카테고리가 없거나, 하위카테고리가 없으면 제외)
select concat(c1.category_name, ' > ', c2.category_name)
from menudb.tbl_category c1 join menudb.tbl_category c2
    on c1.category_code = c2.ref_category_code; -- c1 상위카테고리 - c2 하위카테고리

-- SELF 왼쪽 외부조인
-- 카테고리 조회(상위카테고리는 모두 조회)
select *
from menudb.tbl_category c1 LEFT JOIN  menudb.tbl_category c2
     on c1.category_code = c2.ref_category_code; -- 18행 (9 + 9)

-- SELF 오른쪽 외부조인
-- 카테고리 조회(하위카테고리는 모두 조회)
select *
from menudb.tbl_category c1 RIGHT JOIN  menudb.tbl_category c2
   on c1.category_code = c2.ref_category_code; -- 12행 (9 + 3)
...
-- 내부조인
-- 1. e1 테이블에서 manaver_id가 null인 행이 제외
-- 2. e2


-- 좌측외부조인
-- 사원-메니져 조회(메니져 없는 사원도 조회)
select e.EMP_ID,
       e.EMP_NAME,
       m.EMP_ID,
       m.EMP_NAME
...
select e2.EMP_ID 사번,
       ...
       e2.MANAGER_ID 매니져사번,
       e1.EMP_NAME 매니져명
from empdb.employee e1 right join empdb.employee e2
      on e1.EMP_ID = e2.MANAGER_ID;

-- 우측외부조인
-- 메니져-사원 조회(관리하는 직원이 없는 매니져도 조회)
select e.EMP_ID,
       e.EMP_NAME,
       m.EMP_ID,
       m.EMP_NAME
