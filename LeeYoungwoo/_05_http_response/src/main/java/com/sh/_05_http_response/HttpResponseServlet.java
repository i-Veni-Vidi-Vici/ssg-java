...
/**
 * Servlet이 하는 일
 *  1. 요청핸들링 : 사용자 입력값을 request 객체로부터 꺼내서 자바변수에 대입
 *  2. 업무로직(비즈니스로직) : DB데이터 등록, 조회, 수정, 삭제(CRUD), Service를 선정해서 업무위임
 *  3. 응답메세지 작성 (헤더값, 본문내용)
 *
 */
@WebServlet("/response")
...
//        response.setContentType("text/html"); // 응답 컨텐트 타입(MIME) 지정
        response.setContentType("text/html; charset=UTF-8"); // MIME + encoding 동시에 작성하기

        // 응답바디 (헤더설정 이후 작성할 것)
        // 응답본문 작성을 위한 스트림 객체가 이미 만들어져 있다.
