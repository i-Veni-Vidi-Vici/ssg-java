 ...
 * - 요리사, 손님 : 쓰레드
 * - 접시 : 공유자원
 * - 동기화 처리를 하지 않는다면, 제공하지 않은 빵을 먹거나, 빵을 두개이상 준비하는 등의 오류 발생
 *
 * 쓰레드 상태관리 메소드
 * - wait() WAITING상태로 변환
 ...
 *
 * 구현해야할 동기화 기능
 * - 손님은 접시가 비어있을때 요리가사 빵을 만들길(notify) 기다려야 한다(waiting).
 * - 요리사는 접시가 비어있지 않을때 손님이 빵을 먹길(notify) 기다려야 한다(waiting).
...
public class App {
    public static void main(String[] args) {
        ...
        Thread backer = new Thread(new Baker(plate), "요리사");
        consumer.start();
        backer.start();
