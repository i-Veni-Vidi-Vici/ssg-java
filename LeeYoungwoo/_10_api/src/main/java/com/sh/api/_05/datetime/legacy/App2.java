 ...
 * - Calendar 객체는 불변객체가 아니여서 값 수정이 가능하다.
 * - Calendar 월을 표현할때 0 ~ 11로 처리한다.
 * - 윤초(세슘원자시계와 태양시 사이의 오차)를 반영하지 못한다.
 * </pre>
 */
public class App2 {
...
//        Calendar cal = Calendar.getInstance();
        ...
        int datOfMonth = sometime.get(Calendar.DAY_OF_MONTH);
        ...
        System.out.println("datOfMonth = " + datOfMonth);
        ...
        Calendar thisWeekend = new GregorianCalendar(2024, 2, 30);

        // (미래long - 과거long) / 1000 / 60 / 60 / 24-> 일 단위
        double diff = thisWeekend.getTimeInMillis() - today.getTimeInMillis();
        System.out.println(diff); // 두 날짜 사이의 차를 밀리초로 바꾼거
        diff = diff / 1000 / 60 / 60 / 24;
        System.out.println(diff); // 2.495925520833333 일 단위로 바꾸기
        diff = Math.ceil(diff); // 3 위의 값 올림으로 깔끔하게 바꾸기
