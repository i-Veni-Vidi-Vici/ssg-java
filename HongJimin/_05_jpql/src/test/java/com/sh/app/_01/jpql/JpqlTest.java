 ...
 * 	</pre>
 */
public class JpqlTest { // Department2Student도 가능함!
    ...
    void test1() {
        // given
        // 테이블 별칭을 선언하고, select 절에서 사용해야 한다. *을 사용할 수 없다.
        String jpql = "select m from Menu01 as m"; // Menu에서 지정한 @Entity(name = "Menu01")명으로 써줘야 함
        // when
    ...
    /**
     * TypedQuery와 다르게 반환 타입을 컴파일 타임에 체크할 수 없다.
     */
    @Test
    @DisplayName("Query - Menu 테스트")
    void test3() {
        // given
        // select절, where 조건절에서도 table이 아닌 entity기준으로 조회한다.
    ...
    @Test
    @DisplayName("distinct 키워드") // 중복이 제거됨
    void test4() {
        // given
        String jpql = """
                select distinct
