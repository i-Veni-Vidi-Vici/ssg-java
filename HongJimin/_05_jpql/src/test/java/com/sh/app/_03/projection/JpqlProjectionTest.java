...
public class JpqlProjectionTest {
    ...
    void test1() {
        ...
        // 반환받은 Category 객체(영속객체) 수정한다면,
        categories.get(0).changeCategoryName("식사");
        /**
         * Hibernate:
         *     update
         *         tbl_category
         ...
         *     where
         *         category_code=?
         *  영속성 컨텍스트 안에 포함이 되어있으므로 update 쿼리를 날림
         */
...
//                .allMatch((menuVo) -> menuVo instanceof MenuVo);
    ...
    @Test
    @DisplayName("여러 컬럼 조회 - Query-Object[] 사용하기(반환 타입을 지정할 수 없는 경우)")
    void test4_0() {
        // given
        // 세 가지의 값을 담아낼 수 잇는 클래스가 없움
        String jpql = """
                select
                    c.categoryCode,
                    c.categoryName,
    ...
    /**
     * 반환된 DTO 객체는 영속되지 않는다. (엔티티 객체가 아니므로)
     */
    @Test
    @DisplayName("4. DTO 프로젝션 ")
    void test4() {
        // given
        // 세 가지의 값을 담아낼 수 잇는 클래스가 없움 -> ✨이 방식 많이 씀!
        String jpql = """
                select
        ...
        // DTO 수정 - 영속성 컨텍스트 또는 DB에 어떤 영향도 줄 수 없다.
//        categoryResponseDtos.get(0).setCategoryCode(100L); 영속 컨텍스트가 아니기 때문에 update 쿼리가 날라가지 않음
