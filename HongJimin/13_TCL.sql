...
-- commit(변경사항 반영), rollback(변경사항 취소), savepoint(변경사항을 중간 저장) 명령어 지원


# 트랜잭션이란
    -- 한꺼번에 처리되어야 할 최소한의 작업 단위
    -- 트랜잭션 범위의 DML명령은 모두 성공, 모두 실패되어야 한다.
...
-- 계좌 이체 : A은행 계좌에서 출금, B은행 계좌에 입금하는 행위
-- 1.  A 은행 update문 : balance = balance - money
-- 2.  B 은행 update문 : balance = balance + money
...
-- 계좌이체 트랜잭션
start transaction; -- 트랜잭션을 수동으로 제어, 위에가 오토여도 수동으로 작동됨!
update
    kb_bank
set
