...
// - 조건/반복처리 대비에서 굉장히 간결하게 코드 작성이 가능
// - 학습 비용이 요구되는 편.
// - 언어 독립적. 타 프로그래밍 언어에서 사용 가능
...
console.log(re1.test("a1b2c3")); // true -> 숫자가 있어서 true로 반환됨
console.log(re1.test("abc")); // false -> 숫자가 없어서 flase로 반환됨
...
/**
 * flag 옵션
 * - g global 전역 비교 수행, 이게 없으면 매칭된 첫 문자열만 처리
 * - i ignore 대소문자 구분 안함
 * - m multiline 여러줄인 데이터에 한해 행단위 검색
 */
let target = 'Javascript jQuery Ajax sass';

console.log(target.replace(/a/, '<$&>')); // J<a>vascript jQuery Ajax sass -> 매칭된 결괄르 가리키는 와일드 카드
...
console.log(target.replace(/x/g, '<$&>')); // <J>avascript jQuery Ajax sass xxx
console.log(target.replace(/x$/g, '<$&>')); // <J>avascript jQuery Ajax sass xxx
...
console.log(re.test('acbbbbb')); // true
...
console.log(target.replace(/[A-Z^E]/g, '<$&>')); // E가 아닌 영문자 찾기 실패 ^은 맨 앞에 사용할 때만 반전의 의미를 가짐
