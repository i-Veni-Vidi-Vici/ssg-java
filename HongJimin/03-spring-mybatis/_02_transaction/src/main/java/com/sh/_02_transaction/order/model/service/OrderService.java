 ...
 *
 * - @Transactional 메소드 안에서 던져진 예외가 RunTimeException후손인 경우만, rollback 처리한다.(기본값)
 * -> 모든 예외에 대해 rollback 처리하도록 예외 클래스를 Exception.class로 확장한다.
 *
 */
@Transactional(
...
public class OrderService {
    ...
    public int createOrder(OrderDto orderDto) {
        ...
        for (OrderMenuDto orderMenuDto : orderMenuDtos) {
            ...
            result = orderMapper.insertOrderMenu(orderMenuDto); // 람다 안에서는 외부의 변수를 참조할 때, final이어야 함
        }
        return result; // result가 0만 아니면 성공!
    }
}
