...
/**
 ...
 - 실제 요청의 설정 정보가 담겨 있음
 - 4가지 헤더 카테고리
 1. General Header : 요청/응답 모두 적용되는 내용
 2. Request Header : 응답할 리소스나 요청 클라이언트에 대한 상세 정보
 3. Response Header : 응답 위치/서버 자체와 같은 응답 관련 상세 정보
 4. Entity Header : 본문의 응답 리소스에 대한 상세 정보 (리소스크기, 타입.. 등을 가짐)

 ### 헤더 내용
 - accept : 요청을 보낼 때 서버에게 요청할 응답 타입 명시
    - 이런 타입의 리소스를 줬으면 좋겠어~~ 하는 것(xml, json이 될 수 있음)
 ...
 - referer : 이 페이지 이전에 대한 주소 → 어디 페이지에서 요청을 보냈는지 알려주는 것
...
public class HttpRequestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        Enumeration<String> headerNames = req.getHeaderNames(); // 기존 enum이랑 다름
        while(headerNames.hasMoreElements()){
            String heaederName = headerNames.nextElement();
            String headerValue = req.getHeader(heaederName);
            System.out.println(heaederName + " = " + headerValue);
        }
