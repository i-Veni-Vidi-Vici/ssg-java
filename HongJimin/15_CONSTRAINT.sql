...
-- - 데이터 무결정 보장을 목적으로 한다.
-- - 데이터를 입력/수정 시에 자동으로 제약 조건에 부합하는지 검사
-- - 구분
--   1. NOT NULL : null을 허용하지 않음
--   2. UNIQUE : 중복된 값을 허용하지 않음
--   3. PRIMARY KEY : 기본키, 고유 식별 컬럼(NOT NULL, UNIQUE)
--   4. FOREIGN KEY : 외래키, 참조 테이블의 참조 컬럼 값만 작성 가능
...
/*
  ...
  `EMP_ID` varchar(3) NOT NULL, -- ⛳컬럼 레벨에 작성 : 제약 조건을 옆에 작성하는 것
  ...
  PRIMARY KEY (`EMP_ID`), -- ⛳테이블 레벨 : 컬럼 선언이 끝난 이후에 작성하는 것
...
# drop table user_notnull; -> 테이블 삭제시 사용!
...
-- -------------------------------
-- 중복값을 허용하지 않는 제약조건
-- n개의 컬럼을 묶어서 복합 unique 제약 조건도 가능
...
create table tbl_order_composite_pk (
    ...
    primary key (prod_no, user_id, order_date) -- 세 개의 값이 세트
);
...
where
    prod_no = 300
    and user_id = 'leess'
    and order_date = '2024-04-23 11:19:26';
...
#                on delete set null,-- 부모행이 삭제되면, 자식fk컬럼을 null로 변경
               on update cascade -- 부모행 참조컬럼이 수정되면, 자식 fk 컬럼을 수정
...
-- 2. FK제약조건에 on update set null, on delete set null 옵션을 줄수 있다.
delete from
    user_grade
where
    grade_code = 10;
...
-- CHECK
-- --------------------------------
-- 컬럼 값을 도메인(값 목록/범위)으로 제한하는 제약 조건
...
create table if not exists user_check (
       ...
       phone char(11) not null,
       email varchar(100),
       constraint ck_user_check_gender check (gender in ('M', 'F')), -- M 또는 F만 들어갈 수 있음,
       constraint ck_user_check_age check ( age > 0 ), -- age는 0보다 커야 함
...
insert into
    user_check
values
#     (3, 'leess', '1234', '이순신', '남', 20, '01012341234', 'leess@naver.com'), -- [HY000][3819] Check constraint 'ck_user_check_gender' is violated. -> ⛳'남' 자리에 M or F만 와야해서 오류가 남
    (3, 'leess', '1234', '이순신', 'm', 20, '01012341234', 'leess@naver.com'); -- ⛳ 소문자 m허용
