...
//        ResultView.displayResult("카테고리 & 메뉴 등록", result);
    ...
    public void findByOrderableStatusY() {
        List<MenuDto> list = menuService.findByOrderableStatusY();
        ResultView.displayOrderStatusY(list);
    }

    public void findByCategoryCodeStatus(int categoryCode) {
        List<MenuDto> list = menuService.findByCategoryCodeStatus(categoryCode);
        ResultView.displayCategoryListToOrderStatusY(list);
    }
}
