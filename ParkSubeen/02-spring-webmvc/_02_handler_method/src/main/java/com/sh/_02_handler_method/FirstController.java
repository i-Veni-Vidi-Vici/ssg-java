...
@Slf4j
@SessionAttributes({"id", "foo", "bar"}) // 속성명을 기록, session scope로 관리해줘
...
//    }
     ...
     */
//    @PostMapping("/modify")
//    public String modify(@RequestParam("name") String menuName, @RequestParam int price, Model model){
...
//    }
    ...
    @PostMapping("/login")
    // @RequestParam은 필수값을 불러올 때 사용하면 좋다.
    ...
    public void requestBody(@RequestBody String queryString){
        log.debug("querySting = {}", queryString); // name=%EC%82%BC%EA%B0%81%EA%B9%80%EB%B0%A5&price=3000&categoryCode=1&orderableStatus=Y
    }

    @GetMapping("/etc")
    public String etc(@RequestHeader("user-agent") String userAgent, @CookieValue("JSESSIONID") String jSessionId){
        log.info("userAgent = {}", userAgent); // Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36
