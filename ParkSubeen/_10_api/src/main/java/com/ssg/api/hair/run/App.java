...
public class App {
    public static void main(String[] args) {
        ...
        LocalDate birthday, reservationDate;
        ...
        for(int i = 0; i < user.length; i++) {
            if(i == 0) {
                ...
                email = userInfo[3];
                gender = userInfo[4];
                birthday = !"".equals(userInfo[5]) ? LocalDate.parse(userInfo[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")) : null;
                reservationDate = LocalDate.parse(userInfo[6]);
                reservationTime = LocalTime.parse(userInfo[7], DateTimeFormatter.ofPattern("H:m"));
                createdAt = LocalDateTime.parse(userInfo[8]);
