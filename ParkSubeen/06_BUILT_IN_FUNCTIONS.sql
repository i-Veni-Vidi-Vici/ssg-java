...
from
    tbl_menu
order by
    -- find_in_set도 하나씩 값 비교하여 false면 0, true면 1의 값을 반환하기 때문에 위로 올리기 위해선 desc를 붙여야한다.
...
-- trim(공백포함문자열) : 양쪽 공백ㅁ문자 제거 후 반환
-- trim(방향 문자 from 대상문자열) : leading/trailing/both 방향지정 후 해당문자 제거
select
#     concat('(', '       안녕       ', ')'),
    concat('(', ltrim('       안녕'), ')'),
    concat('(', rtrim('안녕       '), ')'),
...
select
    ...
    regexp_substr('제 키는 188cm입니다.', '[0-9]+'); -- 188 : + => 수량자. 1개이상
...
-- 거듭제곱, 제곱근
-- pow(밑수, 지수) : 2의 3제곱에서 2가 밑수 3이 지수
-- sqrt(숫자)
select
    pow(2, 3), -- 2^3
    pow(3, 3); -- 3^3
...
select
    rand(),
    rand() * 10 + 1, -- 1.0 ~ 11.0미만
    floor(rand() * 10 + 1); -- 1 ~ 10
...
select
    now(), -- 2024-04-16 09:46:35
    adddate(now(), 1), -- 2024-04-17 09:46:35
    adddate(now(), interval 1 day), -- 2024-04-17 09:46:35
    adddate(now(), interval 1 month), -- 2024-05-16 09:46:35
    adddate(now(), interval 1 year); -- 2025-04-16 09:46:35

select
    adddate('2024-01-31', interval 1 month);

select
    subdate(now(), 1), -- 2024-04-15 10:05:57
    subdate(now(), interval  1 day), -- 2024-04-15 10:05:57
    adddate(now(), -1), -- 2024-04-15 10:05:57
    adddate(now(), interval  -1 day); -- 2024-04-15 10:05:57
...
select
    curtime(), -- 10:16:10
    current_time(), -- 10:16:10
    current_time; -- 10:16:10

-- 현재 날짜/시각
-- now(), sysdate(), localtime(), localtime, localtimestamp(), localtimestamp
select
    now(), -- 2024-04-16 10:17:52
    sysdate(), -- 2024-04-16 10:17:52
    localtime(), -- 2024-04-16 10:17:52
    localtime, -- 2024-04-16 10:17:52
    localtimestamp(), -- 2024-04-16 10:17:52
    localtimestamp; -- 2024-04-16 10:17:52
...
select
    timediff('12:50:00', '10:30:30'); -- 02:19:30
...
-- 날짜시각을 특정형식으로 출력하기
-- date_format(날짜/시각, 형식문자열) : 지정된 형식의 문자여 반환
-- 형식
-- %Y/%y 연도4자리/연도2자리
-- %m 월
-- %d 일
...
select
    ...
    date_format(now(), '%H/%i/%s'), -- 11/14/10
    date_format(now(), '%Y년 %m월 %d일 (%W)'); -- 2024년 04월 16일
...
select '1' + 'ㅋㅋㅋ'; -- 1 숫자로 변환불가한 문자열은 0으로 변환 후 1 + 0 계산
...
select 3> '5오오오오'; -- 0(false) '5오오오오'는 5로 변환 후 크기비교. false -> 0
...
select
    ...
    count(category_code) -- 21 찰순대쥬스의 category_code는 null임므로 제외.
from
    tbl_menu;
