 ...
 *
 * - 다형성을 통해서 여러객체를 동일한 타입으로 제어할 수 있어 유지보수성, 생산성이 증가한다.
...
//        app.test3();
    ...
    private void test4() {
        ...
        rabbit.eat(); // Animal#eat으로 정적 바인딩 -> 실행시간(runtime)에 Rabbit#eat으로 동적바인딩
        rabbit.run();
        rabbit.cry();
    }
...
//        Tiger tiger = (Tiger) animal; // ClassCastException 예외 발생!
     ...
     */
    private void test2() {
        Animal animal1 = new Rabbit(); // Rabbit -> Animal up casting, new 앞에 형변환으로 (Animal)을 작성해주어야 하지만 생략 가능.
//        Rabbit rabbit = new Animal(); // error
        ...
        Rabbit rabbit2 = (Rabbit) animal2; // down casting

        Animal animal3 = new Tiger();
        ((Tiger) animal3).bite();
//        Tiger tiger3 = (Tiger) animal3;
    }
