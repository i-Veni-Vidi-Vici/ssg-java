...
//        menu.setMenuCode(100L);
        ...
        try {
            // 영속성 컨텍스트에서 관리하라고 먼저 저장시키고 DB에 쿼리를 실행
            entityManager.persist(menu); // 영속성컨텍스트에 menu 엔티티객체를 저장 (@GeneratedValue 설정 시 insert쿼리는 바로 실행. PK를 알아야 영속성컨텍스트에 저장가능하다. (영속성컨텍스트는 key,value로 되어있음.))
