...
-- inner join (catefgory_code가 null인 그룹 제외)
select
    *
from
    tbl_menu m join (
...
#         on ifnull(m.category_code, 0) = ifnull(c.category_code, 0)
        on ifnull(m.category_code, 1000) = ifnull(c.category_code, 1000) -- 특정 값으로 변환해서 비교하기 때문에 0이 아닌 값도 가능
where
    m.menu_price = c.max_menu_price
order by
    m.category_code;
...
where
    exists(
        select
            1 -- 내용은 중요하지 않다, 행이 존재하는가의 여부가 중요
        from
            tbl_menu
        where
...
-- not exists(서브쿼리) 서브쿼리 행이 존재하면 참!
select
    *
from
    department d
...
-- 다른 테이블의 여러개의 값을 조회하는게 아니면 join 대신 사용하는 것이 좋을 것 같다.
