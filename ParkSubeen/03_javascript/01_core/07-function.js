...
/**
 * ...
 * 나머지파라미터 Rest Parameter : 매개변수부에서 복수개의 값을 처리할 수 있는 변수, 맨 마지막에 한번만 사용하기
 * 전개연산자 Spread Operator : 배열의 요소를 뿌려서 전달(매개인자/값)
 * 파라미터 기본값 : 매개인자가 존재하지 않는 경우(undefined) 지정한 기본값 사용가능
 */
function test1(id, ...chars) { // ...chars를 먼저 사용하는 것은 X
    // chars 배열로써 처리
...
// 2. 몸통부에 return절을 생략 (return구문 한줄만 존재할때)
...
func();
const func2 = func; // func의 주소값을 func2에 저장
func2();
const f = func;
f();
...
console.log(carFactory); // [Function (anonymous)] : 이름없는 함수가 저장되어 있다는걸 보여줌
carFactory(); // 🚙
...
// 경찰차 factory를 지어서(함수) 10대를 출력하세요.
const policecarFactoryBuilder = (car) => {
    return (n) => {
        for(let i = 0; i < n; i++)
            console.log(car);
    }
...
const policecarFactory = policecarFactoryBuilder('🚔');
policecarFactory(10);