...
-- tbl_category(12행)에서 tbl_ menu.category_code에서 사용되지 않은 4행 (1, 2, 3, 7)이 제외되었다. -> 8행 사용됨.
...
select
    *
from
    tbl_category c left join menudb.tbl_menu m
...
select
    *
from
    employee e left join empdb.department d
...
select
    *
from
    employee e right join empdb.department d
        on d.DEPT_ID = e.DEPT_CODE;
...
select
    *
from
#     tbl_menu cross join tbl_category c;
    tbl_menu join tbl_category c;
...
select
#     concat(e.EMP_NAME, '의 매니져는 ', m.EMP_NAME)
    *
from
...
-- 2. m테이블에서는 m.empid가 상응하는 e.manager_id가 없는 경우 제외 (관리하는 부하직원이 없는 경우)
select
    e.EMP_ID,
    e.EMP_NAME,
#     e.MANAGER_ID,
