...
public class Controller {
        ...
        while(true){
            ...
            switch (choice){
                ...
                case "APPLY":
                    ...
                    else if(applyResult.equals("NOPREREQUISITE")){
                        System.out.println("[실패] 선수과목을 이수하지 않았습니다.");
                    }
                    else if (applyResult.equals("OVERCAPACITY")) {
                        System.out.println("[실패] 수강신청 정원을 초과했습니다.");
                    }
                    else if(applyResult.equals("OVERCREDIT")){
                        System.out.println("[실패] 신청 가능한 학점이 부족합니다.");
                    }
                    else if(applyResult.equals("NOMAJOR")){
                        System.out.println("[실패] 신청 가능한 전공이 아닙니다.");
                    }
                    else if(applyResult.equals("NOPERIOD")){
                        System.out.println("[실패] 수강신청 기간이 아닙니다.");
                    }
                    else if(applyResult.equals("ALREADYAPPLIED")){
                        System.out.println("[실패] 이미 신청한 수업입니다.");
                    }
                    else{
    ...
    private void printLISTUPCourses(String[] courses){
        System.out.println("--------------------전체수업 목록--------------------");
        for(String course : courses){
            String[] fields = course.split(",");
            String courseCode = fields[0];
            String courseName = fields[1];
            String courseCredit = fields[2];
            String currentCapa = fields[3];
            String Capacity = fields[4];
            String professor = fields[5];
            String courseMajor = fields[6];
            System.out.println("수업코드: "+ courseCode + "/ 수업명: "+courseName+"/ 학점:" +courseCredit+"/ 현재신청인원: "+currentCapa+"/ 정원: "+Capacity+"/ 교수: "+professor+
                    "/ 전공: "+courseMajor);
        }
        System.out.println("---------------------------------------------------");
    }
