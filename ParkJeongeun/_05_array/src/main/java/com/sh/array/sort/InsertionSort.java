...
public class InsertionSort {
    public void test() {
        ...
        for (int i = 1; i < arr.length; i++){
            ...
            // i=3, j=3,2,1
            // i=4, j=4,3,2,1
            int j; // 안쪽 for문이 끝난 후 변수에 접근하기 위해 미리 선언.
                   // 반복문 탈출할때 증감변수의 범위 -1한 값 사용
            for(j = i; j > 0; j--){
                // j-1 인덱스값이 temp보다 크면 j-1인덱스(왼쪽) 값을 j인덱스(오른쪽)에 덮어씀
                if (arr[j -1] > temp){
                    arr[j] = arr[j - 1];
                    print(arr, i);
                }
