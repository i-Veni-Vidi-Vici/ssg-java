...
public class App {
    public static void main(String[] args) {
        ...
        Product carpBread = context.getBean("carpBread", Product.class); // Product.class 대신 Bread 가능
        Product milk = context.getBean("milk", Product.class);
        Product water = context.getBean("water", Product.class);

        // scope속성이 prototype인 경우, 해당 빈은 ApplicationContext 초기화시 만들지 않고, getBean 메소드가 호출될때마다 생성됨
        ...
        // bean의 destroyMethod를 명시적으로 호출하기위해 process 종료전에 context 종료하면 됨
        // stream 계열의 빈에서 자원반환을 위해 destroyMethod 사용
