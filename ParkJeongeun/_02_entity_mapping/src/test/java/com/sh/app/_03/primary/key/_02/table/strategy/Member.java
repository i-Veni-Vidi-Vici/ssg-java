...
@Table(name = "tbl_member0302") // 실제 db테이블과 매핑되는 이름
@TableGenerator(
        name = "table_sequence_generator",
        table = "tbl_sequence",
        allocationSize = 50, // 기본값 50, 한번 읽어서 메모리(db가 아닌 앱)에서 배포할 크기, 매번 조회X
        pkColumnName = "table_name" // 테이블별 식별자, next_val 컬럼을 통해 채번할 숫자 관리
)
@Data
@NoArgsConstructor
@AllArgsConstructor
