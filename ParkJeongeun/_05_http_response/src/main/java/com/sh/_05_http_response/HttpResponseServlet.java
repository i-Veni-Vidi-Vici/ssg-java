...
public class HttpResponseServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 응답헤더
        // 인코딩 설정
//        response.setCharacterEncoding("UTF-8"); // 기본값이어서 생략 가능
//        response.setContentType("text/html"); // 응답 컨텐트의 타입(MIME) 지정
        response.setContentType("text/html; charset=UTF-8"); // MIME + encoding 동시 작성

        // 응답바디 (헤더 설정 이후 작성)
        // 응답본문 작성을 위한 스트림객체가 이미 만들어져 있음
//        ServletOutputStream outputStream = response.getOutputStream(); // 바이트기반 출력 스트림
        PrintWriter out = response.getWriter(); // 문자기반 출력 스트림
