 ...
 *                    따라서 오염된 값(수정/변경된 값)을 읽거나, 재현 불가능한 읽기, 허상 읽기 등의 문제가 발생할 수 있다.
 ...
 *
 * @Transactional : 메소드 안에서 던져진 예외가 RuntimeException 후손인 경우만 rollback 처리(기본값)
 * -> 모든 예외에 대해 rollback 처리하도록 처리할 예외클래스를 Exception.class로 확장함
 */
@Transactional(
        propagation = Propagation.REQUIRED, // 작성 안해도 기본값
        isolation = Isolation.READ_COMMITTED, // 작성 안해도 기본값
        rollbackFor = Exception.class)
@Service
public class OrderService {
    /**
     * @Mappder 구현클래스 빈이 의존주입되어 처리됨
     * - SqlSessionTemplate#getMapper는 더이상 작성하지 않아도 됨
     */
