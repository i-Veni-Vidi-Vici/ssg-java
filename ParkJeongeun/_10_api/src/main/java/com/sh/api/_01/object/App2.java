 ...
 * - 객체 동일성(Identity) : heap 영역의 같은 객체인가? (주소값이 같은가?) -> == 동등비교 연산자 사용
 * - 객체 동등성(Equality) : 객체가 가진 내용이 같은가? (특정 필드의 값이 동일한가?) -> 오버라이드된 equals 메소드 사용
 *
 * 객체비교 원칙
 * - equals 비교 결과가 true라면 동일한 hashCode를 반환해야 함
 * - 이후 Collections의 HashXXX 클래스들은 요소의 동등성을 위해 equals/hashCode를 모두 호출해서 비교함
 * </pre>
 */
public class App2 {
    public static void main(String[] args) {
        ...
        // hashCode
        System.out.println(book2.equals(book3)); // true
        System.out.println(book2.hashCode()); // 1554547125 -> hashCode 오버라이드 후 -1740523887
        System.out.println(book3.hashCode()); // 617901222 -> hashCode 오버라이드 후 -1740523887
    }
}
