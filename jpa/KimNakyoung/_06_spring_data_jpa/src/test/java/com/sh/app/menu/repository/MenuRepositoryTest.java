...
//        Menu menu = optMenu.orElseThrow(); // optMenu안에 값이 존재하면 반환하고, 없으면 예외 던지기
    ...
    void test5() {
        ...
        // when
        String newMenuName = "수밧" + menu.getMenuName();
        menu.changeMenuName(newMenuName); // 메뉴야 이 이름으로 바꿔줘
        /*
            Hibernate:
                insert
                into
...
//        Optional<Menu> optMenu = menuRepository.findById(menuCode);
//        Menu menu = optMenu.orElse(null);
//        menuRepository.delete(menu); // 반환 업승ㅁ

        menuRepository.deleteById(menuCode); // 먼저 엔티티를 조회 할 필요없이 바로 삭제요청 (내부적으로 조회요청)
        /*
            Hibernate:
                insert
                into
