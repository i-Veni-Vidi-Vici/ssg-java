 ...
 *  (mysql의 enum타입을 사용하면, 노출되는 값은 name값이지만, 내부적으로는 숫자값으로 관리하기때문에 효율성 역시 좋다)
 * </pre>
 */
...
public class Member {
    ...
    @Enumerated(EnumType.ORDINAL) // enum name의 선언 인덱스를 db에서 관리하게 된다.
    private Gender gender;

    @Column(name = "member_role")
    @Enumerated(EnumType.STRING) // enum name값은 문자열로 db에서 관리하게 된다.
