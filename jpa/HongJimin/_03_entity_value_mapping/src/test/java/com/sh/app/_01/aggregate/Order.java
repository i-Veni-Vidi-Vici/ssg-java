...
@Data
@Setter(AccessLevel.PRIVATE) // 게터 세터를 프라이빗으로 만들면 외부에서 값을 임의로 변경하는 것을 방지함
@NoArgsConstructor
//@AllArgsConstructor // 직접 만들 예정!
public class Order { // Order는 하나만 만들고, 그 밑의 Orderer, DeliveriInfo, List<OrderLine>들이 value가 되는 것
    private Long id; // ✨OrderEntity의 식별자
    private Orderer orderer; // ⛔직접 수정하지 않음
    private DeliveryInfo deliveryInfo; // ⛔직접 수정하지 않음
    private List<OrderLine> orderLines;  // 주문 내역, ⛔직접 수정하지 않음 /
    private int totalPrice;
    private OrderStatus orderStatus;
    private LocalDateTime orderDate;
    // ⛔ 직접 수정하지 않는 것은 Order 객체를 통해 수정함!
    ...
    public Order(Long id, Orderer orderer, DeliveryInfo deliveryInfo, List<OrderLine> orderLines, int totalPrice, OrderStatus orderStatus, LocalDateTime orderDate) {
        ...
        // 주문 내역 설정과 총 결제금액 계산은 setter에서 일관되게 작업되어야한다.
        // 외부에서 호출할 수 없게 접근 제한자를 private로 지정해둠
    ...
    private void setOrderLines(List<OrderLine> orderLines) {
        ...
        this.totalPrice = orderLines.stream()
                .map((orderLine) -> orderLine.getPrice() * orderLine.getCount()) // 각 상품별 금액 계산
                .reduce(0, Integer::sum); // 바뀐 값들을 차곡차곡 누적함
    }
    ...
    public void changeDeliveryAddress(Address newAddress) {
        ...
        } else throw new IllegalStateException("주문상품이 이미 발송되어서, 배송지 정보를 수정할 수 없습니다.");
    }
}
