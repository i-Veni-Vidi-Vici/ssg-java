...
public class JpqlNativeQueryTest {
    ...
    @Test
    @DisplayName("결과 타입을 지정하지 않고 Native Query 사용하기 (엔티티 클래스 외에는 지정불가)")
    void test2() {
        // given
        Long menuCode = 15L;
        // when
        ...
        // 엔티티 타입이 아니어서 검사자체가 되지 않는다.
        // 엔티티 타입이 아닌경우 EntityManger#contains()의 인자로 전달할 수 없다. IllegalArgumentException
//        assertThat(this.entityManager.contains(row)).isFalse();
    }

    @Test
    @DisplayName("NamedNativeQuery 사용하기 - 엔테테")
    void test3() {
        // given
        Long menuCode = 15L;
        // when
    ...
    void test4() {
        ...
        Query query = this.entityManager.createNamedQuery("_09.nativequery.Menu.findMenuAndCategory")
                .setParameter(1, menuCode);
        MenuCategoryResponseDto dto = (MenuCategoryResponseDto) query.getSingleResult();
