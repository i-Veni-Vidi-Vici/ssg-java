...
public class CRUDTest {
    ...
    void test2() {
        // given
        String jpql = "select m from Menu m"; // jpa 안에서만 사용가능한 sql. entity 객체에 대한 쿼리
    ...
    void test3() {
        ...
        // then
        assertThat(menu.getMenuCode()).isNotZero(); // auto_increment로 id 값을 할당받음
    ...
    void test4() {
        ...
        try{
            ...
            System.out.println("메뉴 정보 변경되었습니다." + menu);
            transaction.commit();
        }catch (Exception e){
            transaction.rollback();
            e.printStackTrace();
    ...
    void test5() {
        ...
        try{
            ...
            System.out.println("영속성컨텍스트 메뉴 객체의 정보가 변경." + menu);
            transaction.commit();
        }catch (Exception e){
            transaction.rollback();
            e.printStackTrace();
