...
-- 21행 ->21그룹
select
    # menu_name -- grouping과 상관없는 컬럼으로는
    category_code,
    menu_price,
    count(*)
from tbl_menu
...
-- 카테고리별 개수 조회
select category_code,count(*),sum(menu_price)
from tbl_menu
group by category_code,menu_price with rollup
order by 1,2;
-- group by전 모든 행을 하나로 봤을때 count(*)를 추가로 제공.

-- 카테고리, 가격으로 그룹조회
-- with rollup : group by category행 추가, 모든행 한그룹으로 행 추가
-- with rollup은 group by에 대해 단반향 소계(앞선 컬럼기준)을 제공한다.
...
from
    tbl_menu
group by
    menu_price, category_code with rollup;

# 그루핑함수
-- 실제데이터는 0을 반환
-- 집계데이터는 1을 반환
select
    if(grouping(category_code) = 0, ifnull(category_code,'카테고리 없음'),null),
    count(*)
from tbl_menu
