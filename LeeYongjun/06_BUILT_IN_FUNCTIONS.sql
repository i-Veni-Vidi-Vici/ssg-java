...
-- 숫자포메팅(세자리콤마찍기, 소수점이하 자리수 보전)
-- format(숫자, 소주점이하자리수) : 형식지정된 문자열 반환
select
...
-- 메뉴명 가격조회 (살구쥬스 ￦1,200)
select
    concat(menu_name,' ￦',format(menu_price,0),'원')
from tbl_menu;
...
-- 파인애플쥬스 -> 파인애플 쥬우스
SELECT
    INSERT(menu_name, instr(menu_name,'주스'),2,'주우스')
from
    tbl_menu
where
    menu_name like '%쥬스%';
...
select
    ...
    regexp_substr('제 키는 188cm 입니다.','[0-9]+');
