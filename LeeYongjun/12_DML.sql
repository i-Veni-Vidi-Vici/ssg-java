...
-- insert시에 이중 생략 가능한 컬럼은? id, discount_rate,discount_price,issued_at(not null이지만, default값 지정)
...
insert into
    tbl_coupon
    values
    (null,'대한독립기념쿠폰',5000,null,now(),'2024-05-22'),
    (null,'건국기념쿠폰',null,0.15,now(),'2024-05-22'),
    ...
    (null,'겨울맞이쿠폰',3000,null,'2024-11-15 00:00:00','2024-03-01');
...
-- 트랜잭션 수동모드에서는 쿼리 실행 후, 별도의 commit 명령을 통해 결과를 반영할 수 있다.(rollback 명령하면 변경사항을 취소할 수 있다.)
update
    tbl_menu
set
...
set
    ...
    DEPT_CODE = (select DEPT_ID from (select DEPT_ID from department where dept_title = '해외영업3부') d)
where
    EMP_NAME = '임시환';
...
-- 테이블의 행을 삭제하는 명령
-- 실행결과 행수가 줄어든다.
-- WHERE로 특정행을 한정해서 사용해야한다.(WHERE절을 생략하면 모든행이 삭제된다.)
select * from tbl_menu;
...
-- --------------------------------
-- 병합 기능을 지원한다.
-- merge명령은 mysql에서 지원하지 않는다. insert사용.
-- 서브쿼리를 이용한 대량의 데이터를 insert할 수 있다.

-- 테이블의 구조만 복사 (1=2 조건떄문에 데이터는 복사되지 않는다.)
-- 서브쿼리로 테이블을 생성하면, 테이블 NOT NULL 제약조건 외에는 복사되지 않는다.
