...
-- create, alter , drop, truncate 등의 명령어 제공한다.
...
-- if not exists 존재하지 않을떄만 create table을 수행하는 옵션
...
-- 중복되지 않는 숫자를 발급.
-- 특정 숫자값을 직접 지정가능하다. 해당 숫자는 auto_increment는 건너뛰게 된다
...
add price int default 1000 not null comment '가격' after name; -- 자료형 기본값 not null comment순서
...
-- 컬럼명 변경 (자료형 필수)
alter table product3
change name prodname varchar(10);

-- 컬럼 자료형/기본값/NOT NULL변경
-- NULL 작성시 NOT NULL -> NULLABLE 변경가능하다.
alter table product3
modify prodname varchar(100) default '상품명 없음' not null after price;
...
-- --------------------------------
-- 테이블의 모든 행을 잘라내는 DDL
-- where절 없는 delete(DML) 대비 속도 빠르다.
-- auto_increment컬럼도 초기화 된다.
