...
public class MenuService {
    ...
    public List<MenuDto> findAll() {
        // 매퍼(메퍼 인터페이스를 구현한 객체) 가져오기
        MenuMapper menuMapper = sqlSession.getMapper(MenuMapper.class);
        return menuMapper.findAll();
    }

    public MenuDto findByMenuCode(Long menuCode) {
        return sqlSession.getMapper(MenuMapper.class).findByMenuCode();
    }

    public List<MenuDto> findByCategoryCode() {
        MenuMapper menuMapper = sqlSession.getMapper(MenuMapper.class);
        return menuMapper.findByCategoryCode();
    }
