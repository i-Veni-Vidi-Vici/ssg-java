...
class MenuMapperCommandTest {
    // Fixture : 테스트에 도움이 되는 대상, 테스트 대상
    SqlSession sqlSession; // 테스트에 도움이 되는 대상
    MenuMapper menuMapper; // 테스트 대상

    @BeforeEach // 매 테스트 시 반복되는 코드
    void setUp() {
        this.sqlSession = getSqlSession();
        this.menuMapper = this.sqlSession.getMapper(MenuMapper.class);
    }

    @AfterEach // 매 테스트 시 반복되는 마지막 코드
    void tearDown() {
//        this.sqlSession.commit(); // 생략해도 sqlSession 해제시 자동 커밋됨 (테스트 환경 제외)
    ...
    void insertMenu() {
        ...
        System.out.println(menuCode); // 방금 insert된 menuCode 출력
