 ...
 * - 두 결과가 동일하면 테스트 성공!
 * - junit 단정문, hamcrest, truth 등 중에서 assertj를 가장 많이 사용함
 * </pre>
 */
class AppTest {
    App app;
    ...
    @DisplayName("메소드가 양수를 반환한다.") //메소드 명보다 우선해서 표현됨
    @Test
    void 메소드가_양수를_반환한다() {
        int n = app.getNumber();
    ...
    @DisplayName("getNumber는 1 ~ 100 사이의 정수를 반환한다.") //메소드 명보다 우선해서 표현됨
    @Test
    void test2() {
        int n = app.getNumber();
    ...
    @DisplayName("두 수의 합을 반환한다.") //메소드 명보다 우선해서 표현됨
    @Test
    void test3() {
        int a = 10;
        int b = 20;
...
//        assertThat(result).isBetween(100, 200);
        assertThat(result)
                ...
                .isLessThanOrEqualTo(200); // method 체인
    }
}