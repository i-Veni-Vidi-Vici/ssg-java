...
-- =====================
-- SQL 종류 : DDL, DML, DCL
-- Transaction Control Language : 트랙잭션 제어어
-- commit(변경사항 저장 및 반영), rollback(변경사항 취소) , savepoint(변경사항을 임시저장 ) 명령어 저장

# 트랜잭션이란
-- 한꺼번에 처리되어야 할 최소한의 작업단위
-- 트랜잭션 범위의 DML명령은 모두 성공, 모두 실패되어야 함
...
-- 3. 완료(commit) 또는 실패(Rollback) 처리
-- 1번 , 2번 둘다 성공해야만 commit처리 하는게 트랜잭션
...
insert into kb_bank
values (1000, '홍길동', 10000000);
insert into ssg_bank
values (2000, '신사임당', 10000000);

-- 계좌이체 트랜잭션
start transaction ; -- 트랜잭션을 수동으로 제어하게 해줌.
update
kb_bank
