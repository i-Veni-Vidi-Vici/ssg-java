...
-- 2. column 열(field, atrribute) table의 구조, 컬럼별로 자료형 가질 수 있다.
-- 3. row 행(record, tuple) 실제 data가 관리되는 단위  -> 데이터 구조를 볼 때는 행 단위로 데이터를 봐야함.
...
-- 문자형
-- ============================
-- 고정형 문자 타입 CHAR : 지정된 크기보다 작은 데이터를 입력해도 지정한 크기로 보관되는 타입
-- 가변형 문자 타입 VARCHAR : 지정된 크기보다 작은 데이터를 입력하면, 딱 그 크기만큼 보관되는 타입 <크기 5만큼 저장 -> 문자 2개만 저장하면 보관은 2개만큼만 저장됨>

-- 대소문자를 구분하지 않는 SQL 특성상 snake case방식으로 단어를 연결해서 쓴다.
...
-- decimal 고정 소수타입(지정한 소수점 이하로 정확한 데이터 관리가 필요할 때)
...
values(
    1234.5678, -- 1234
    1234.5678901234567890, -- 1234.67
    1234.5678901234567890, -- 1234.5678
    1234.5678901234567890  -- 1234,56780 00000 00000
);
...
-- 날짜 / 시간형
-- ============================
-- DATE : 날짜만 관리 YYYY - MM - DD
-- TIME : 시간만 관리 HH:MM:SS.SS 형식으로 관리
...
--      국제화가 필요한 앱에 적합하다.
-- YEARTIME : YYYY형식으로 관리

-- now() : 현재 시스템 시각을 DATETIME형식으로 제공
select now(); -- 특정 테이블 참조 없이 사용 가능. 단독적으로 사용가능

create  table tbl_data_types_data(
    a date,
    b time,
    c datetime,
    d timestamp,
...
select * from tbl_data_types_data;
-- 지역대 정보가 변경되면, timestamp타입은 그에 알맞게 변환해서 불러온다.
select @@session.time_zone; -- System 정보
