...
select * from  tbl_order_menu; --  주문별 제품
...
-- 2. column(열)(field, attribute), table의 구조, 컬럼별로 자료형을 가진다
-- 3. row(행)(record, tuple), 실제 data가 관리되는 단위
-- 4. domain 하나의 속성(열)에서 가질 수 있는 값의 집함== 각 단위 열의 집합
...
-- 대소문자를 구분하지 않는 SQL 특성상 snake case 방식으로 단어를 연결해
...
-- 데이터 추가 (행단위로 처리된다)
insert into tbl_data_types_char values ('korea','usa');
insert into tbl_data_types_char values ('개구리','개구리');
insert into tbl_data_types_char values ('1234567890','1234567890'); -- 글자수 10개니깐 들어간다
insert into tbl_data_types_char values ('1234','111111234567890'); -- 실행하면 row1에 오류가 난다
-- [22001][1406] Data truncation: Data too long for column 'a' at row 1

select
    a,length(a), char_length(a),-- 실제 데이터 길이를 보여준다
    b,length(b)
from
    tbl_data_types_char;
...
-- ---------------------------------소제목
-- int(Integer) 정수타입
-- float, double 실수 타입 (float - 유효자리수 6개, double - 유효자리수 16개
-- decimal 고정소수타입 (지정한 소수점이하로 정확한 데이터관리가 필요할 때
-- - decimal(전체자리수, 수수점이하자리수)

create table tbl_data_types_number(
  a int,
  b float,
  c double,
  d decimal(30, 20)-- 30자리인데 소수점은 20자리
);
...
values (
        ...
        1234.5678, -- 1235.57
        1234.5678, -- 1235.5678
        1234.5678 -- 1235.5678 0000 0000 0000 0000
       );
insert into
    tbl_data_types_number
values (
           1234.5678, -- 1235
           1234.5678654654654654, -- 1234.57
           1234.5678654654654654, -- 1234.5678654654655
           1234.5678654654654654 -- 1234.5678 6546 5465 4654 0000
       );
...
-- TIME HH:MM:SS.SSSSSS 형시으로 관리
-- DATETIME YYYY-MM-DD HH:MM:SS.SS 형식으로 관리
-- TIMESTAMP YYYY-MM-DD HH:MM:SS.SS 형식으로 관리.
--             지역대정보를 제거하고 UTC로 관리.불러올때는 지역대정보를 적용해서 보여줌.
--             국제화가 필요한 앱에 적합하다. ->set time_zone = '+09:00';  이 명령어로 관리한다
-- YEAR YYYY형식으로 관리
...
select @@session.time_zone; -- DB서버가 돌아가고 있는 SYSTEM

set time_zone = '+09:00'; -- KST가 UTC대비 9시간이 빠르다
set time_zone = '-04:00'; -- NewYork = UTC 대비 4시간이 느리다
