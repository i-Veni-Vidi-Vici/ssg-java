 ...
 * - HTTP요청을 처리할 수 있는 자바 클래스 종류이다
 *
 * - HttpServlet (웹요청을 처리하는 데 필요한 설정을 가진 클래스) 상속해서 커스텀 서블릿을 생설 할 수 있따
 *      - Servlet interface (규격)
 *          - GenericServlet class (구현체)
 *              - HttpServlet class (웹 http요청 처리용 클래스)
 *                  - CustomServlet
 *
 * - Tomcat은 요청별 Servlet 객체를 관리한다 (Servlet/JSP Container)
 * - Tomcat으 Servlet객체를 Singletone패턴으로 관리한다
 ...
 *      2. 전송방식별로 그에 상응하는 메소드 호출 doGet, soPost 등이 있다
 * - Tomcat 종료시에 모든 Servlet객체들이 destroy메소드 호출 후 반환한다
 */
