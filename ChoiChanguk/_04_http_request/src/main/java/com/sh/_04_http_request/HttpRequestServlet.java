 ...
 *   2. Request Header : 응답할 리소스나 요청클라이언트에 대한 상세 정보
 *   3. Response Header : 응답위치 / 서버 자체와 같은 응답관련 상세 정보
 *   4. Entity Header : 본문의 응답 리소스에 대한 상세 정보. (리소스 크기, 타입 ...)
 *
 * // header 내용들
 *  accept : 요청을 보낼 때 서버에게 요청할 응답 타입 명시 // 이런 타입의 리소스를 제공해 주시겠어요?? // 브라우져 입장에서 서버한테 이런 종류(html같은)로 줬으면 좋겠어
 ...
 *  -- referer : 이 페이지 이전에 대한 주소 // 어디서 이 요청을 보냈어??,
...
public class HttpRequestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String userAgent = req.getHeader(("user-Agent")); // 이렇게 하나만 꺼내올 수 있다
