...
@Data
@Setter(AccessLevel.PRIVATE) // 외부에서 값을
@NoArgsConstructor
public class Order {
...
//    @Column(name = "total_price")
    ...
    public void changeDeliveryAddress(Address newAddress) {
        if(this.orderStatus == orderStatus.배송준비중 || this.orderStatus == OrderStatus.주문확인중)
        {
            this.deliveryInfo.changeAddress(newAddress);
        }
        else new IllegalStateException("주문 상품이 이미 발송되어서, 배송지정보를 수정할 수 없습니다");
    }
}
