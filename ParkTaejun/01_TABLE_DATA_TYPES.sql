...
-- ================================
 -- TABLE 구조
 -- ===============================
 -- mysql의 table 은 항상 특정 database 하위에 존재한다.
-- 특정 database에 접근권한이 있는 사용자만 table을 관리할 수 있다.

-- RDBMS에서 구분
-- 1. table (entity, relation) 실제 data가 보관되는 주체
-- 2. colum 열 (field , attribute) table의 구조, 컬럼별로 자료형 가질 수 있다.
...
-- TIMESTAMP YYY-MM-DD HH:MM:SS 형식으로 관리
...
insert into tbl_data_types_date values (
 now(),
 now(),
 now(),
 now(),
...
select @@session.time_zone; -- SYSREM 시간 따라간다?
