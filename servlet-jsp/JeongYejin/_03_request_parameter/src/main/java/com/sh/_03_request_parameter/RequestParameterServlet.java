...
public class RequestParameterServlet extends HttpServlet {
     ...
     * - name=홍길동&age = 33&married=true Query String 부분은 request객체 안에서 HashMap으로 관리할 수 있다.
     * - HttpServletRequest#getParameter(name):String으로 처리할 수 있다.
     * - 모든 사용자입력값은 문자열로써 처리된다.
     * - 단순조회(멱등)요청(DQL쿼리)은 전송방식 GET을 사용한다.
     *      멱등 : 요청 전과 후에 DB서버의 상태에 변함이 없다
     * @param req
     * @param resp
    ...
    /**
     * POST요청에서 사용자 입력값은 http요청메세지의 본문영역에 작성된다. (url에 노출되지 않는다.)
...
//        req.setCharacterEncoding("UTF-8");
        ...
        parameterMap.forEach((k, v) -> {
            System.out.println(k + " = " + Arrays.toString(v)); // 반복문 없이 배열안의 요소를 출력하는 메소드
        });
    }
}