...
public class RequestParameterServlet extends HttpServlet {
     ...
     * - name=홍길동&age=33&married=true Query String (직렬화)부분은 request객체 안에서 HashMap으로 관리된다.
...
//        String age = req.getParameter("age"); // 숫자값을 넘겨줘도 무조건 문자열로 반환
        int age = Integer.parseInt(req.getParameter("age")); // int를 쓸 경우 형변환 필요
        LocalDate birthday =
