 ...
 *          1. 요청이 오면, service(Serviece(ServletRequest,ServletResponse)가 호출된다.
 *          2. 전송방식별로 그에 상을하는 메소드 호출 doGet, doPost 등이 있다.
 * - Tomcat 종료시에 모든 Servlet객체를 destroy메소드 호출후 반환한다.
 *
 */
@WebServlet("/lifecycle")
public class LifecycleServlet extends HttpServlet {
    public LifecycleServlet() {
        System.out.println("Lifecycle Servlet Instantiated");
    }
    @Override
