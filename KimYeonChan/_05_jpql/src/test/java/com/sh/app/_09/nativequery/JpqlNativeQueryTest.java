...
public class JpqlNativeQueryTest {
    ...
    @Test
    @DisplayName("결과 타입을 지정해서 Native 쿼리 사용하기")
    void test1() {
        // given
        Long menuCode = 15L;
        // when
        String sql = """
                ...
                where
                    menu_code = ?
                """;
        // Entity 클래스를 반환타입으로 지정하면, 조회결과를 영속성컨텍스트에서 관리하게 된다.
    ...
    @Test
    @DisplayName("결과 타입을 지정하지 않고 Native 쿼리 사용하기")
    void test2() {
        // given
        Long menuCode = 15L;
        // when
        ...
        assertThat(row)
                .hasSize(3);
        // 엔티티 타입이 아닌경우 EntityManager#contains()의 인자로 전달할 수 없다.
//        assertThat(this.entityManager.contains(row)).isTrue();
    }

    @Test
    @DisplayName("@NamedNativeQuery 사용하기")
    void test3() {
        // given
        Long menuCode = 15L;
        // when
        Query query = this.entityManager.createNativeQuery("_09.nativequery.Menu.findByMenuCode", Menu.class)
