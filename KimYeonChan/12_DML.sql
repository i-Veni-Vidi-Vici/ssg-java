...
values
    (301, '함지민', '781020-2123453', 'hamham@ohgiraffers.kr', '01012343334', 'D1', 'J4', 'S3', '4300000', '0.2', 200),
...
create table tbl_coupon (
    ...
    coupon_name varchar(1000) not null, -- 쿠폰 명
...
desc tbl_coupon;
-- inset 시에 생략 가능한 컬럼은? id, discount_price, discount_rate, issued_at

-- 추가할 데이터
insert into
    tbl_coupon
values
    (null, '대한독립기념쿠폰', 5000, null, '2024-04-22 11:18:46', '2024-05-22'),
...
values
    ('대한독립기념쿠폰', 5000, null, '2024-04-22 11:18:46', '2024-05-22'),
    ('건국기념쿠폰', null, 0.15, '2024-04-22 11:18:47', '2024-05-22'),
...
truncate table tbl_coupon; -- 데이터 초기화
...
where
#     menu_name = '파인애플탕'
    menu_code = 24; -- 중복값이 없는 식별 컬럼(PK)을 통해 행을 찾는 것이 좋다.

-- 서브 쿼리를 통한 update
-- 우럭스무디의 같은 카테고리로 파인애플탕의 카테고리를 수정하세요.
-- 수정하고자 하는 테이블은 서브쿼리에서 접근할 수 없다. (mysql)
update
    tbl_menu
set
    -- [HY000][1093] You can't specify target table 'tbl_menu' for update in FROM clause
...
-- 한식 메뉴의 가격을 1000원씩 인상하는 Update 문을 작성
-- DML 은 트랜잭션 모드를 수동(Manual)로 지정해서 쿼리 실행, db 반영을 구분할 수 있다.
-- 트랜잭션 수동 모드에서는 쿼리 실행후, 별도의 commit 명령을 통해 결과를 반영할 수 있다. (rollback 명령하면 변환 가능하다.)
...
with t as (select * from EMPLOYEE)
update
    EMPLOYEE
set
    SALARY = (select SALARY from t where t.EMP_NAME = '유재식'),
    BONUS = (select BONUS from t where t.EMP_NAME = '유재식')
where
    EMP_ID = (select t.EMP_ID from t where t.EMP_NAME = '방명수');
select * from EMPLOYEE where EMP_NAME in ('유재식', '방명수');
-- 임시환 사원의 직급을 과장, 부서를 해외 영업 3부로 변경하세요.
update
    EMPLOYEE
set
    JOB_CODE = 'J5',
...
-- 테이블의 행을 삭제하는 명령
-- 실행결과 행 수가 줄어든다.
-- WHERE 로 특정 행을 한정해서 사용해야 한다. (WHERE 절을 생략하면 모든행이 삭제된다.)
...
-- REPLACE
-- -------------------------------------
-- upsert. pk 컬럼 기준으로 데이터가 존재하면 update 처리, 존재하지 않으면 inset 처리
...
-- 서브쿼리로 테이블을 생성하면, 테이블 not null 제약조건 외에는 복사되지 않는다.(제약조건, default 값 등은 제외)
