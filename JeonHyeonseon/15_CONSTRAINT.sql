...
-- 3. PRIMARY KEY : 기본키, 고유식별컬럼 (NOT NULL, UNIQUE 자동으로 포함)
-- 4. FOREIGN KEY : 외래키, 참조테이블의 참조컬럼값만 작성가능
-- 5. CHECK : 지정한 도메인에서만 값 사용가능
...
values
    (3, 'leess', '이순신', '남', '01012341234');  -- [HY000][1364] Field 'user_pwd' doesn't have a default value(notnull이면서 기본값도 없으니까 생략할 수 없다.)
...
create table if not exists user_unique (
    ...
    constraint uq_user_unique_user_id unique(user_id), -- (테이블레벨 작성가능) 이름(uq_user_unique_user_id)을 달아줘야 함
...
insert into
    user_primarykey
values
#     (1, 'honggd', '1234', '홍길동', '남', '01012341234', 'honggd@naver.com'); -- [23000][1062] Duplicate entry '1' for key 'user_primarykey.PRIMARY' 중복된 값을 줄 수 없다
...
create table tbl_order_composite_pk (
    prod_no int, -- 상품번호 (상품번호가 pk라면 상품이 하나 밖에 없어서 다른 회원이 살 수 없다)
    user_id varchar(50), -- 회원아이디 (회원아이디가 pk라면 회원은 그 제품을 한 번밖에 못 산다)
    count int default 1,
...
where
    CONSTRAINT_SCHEMA = 'menudb'
    and TABLE_NAME = 'tbl_order_composit_pk';
...
#     prod_no = 300
#     and user_id = 'leess'
#     and order_date = '2024-04-23 11:19:31';
    (prod_no, user_id, order_date) = (300, 'leess', '2024-04-23 11:19:31'); -- and연산을 이렇게 작성할 수 있다.

-- 복합기본키 대안
create table tbl_order_composite_pk2 (
    id int auto_increment primary key, -- 식별번호(로 따로 관리함)
    prod_no int, -- 상품번호
...
-- 외래키를 통해서 테이블과 테이블간의 관계가 만들어진다.(Relational DBMS) -- Relational이 가능한 이유가 외래키때문임.
-- 참조테이블의 참조컬럼은 PK, UQ(unique)제약조건이 걸려있어야 한다.
-- 제공되는 값외엔 NULL을 허용한다.
...
create table if not exists user_foreignkey (
    ...
    phone char(11) not null,
    email varchar(100),
    constraint fk_user_foreignkey_grade_code -- 이름
        foreign key(grade_code) -- 컬럼
        references user_grade(grade_code) -- 어느 테이블을 참조한다
#         on update set null -- 부모행 참조컬럼이 수정되면, 자식fk컬럼을 null로 변경
#         on delete set null, -- 부모행이 삭제되면, 자식fk컬럼을 null로 변경
        on update cascade -- 부모행 참조컬럼이 수정되면, 자식fk컬럼을 수정. cascade는 연쇄되는 참조를 의미
...
select * from user_foreignkey; -- grade_code가 실제 user_grade에 존재하는 값이기 때문에 들어간다.
...
-- 외래키 제약조건에 부합되지 않아요. 10, 20,30 중에 작성해야 하는데 50(없는 값)을 작성해서 에러처리
...
-- 부모테이블(user_grade)을 수정,삭제할 수 없다. fk가 망가지기 때문임
select * from user_grade;
...
insert into
    user_check
values
#     (3, 'leess', '1234', '리순신', '남', 20, '01012341234', 'leess@naver.com'); -- [HY000][3819] Check constraint 'ck_user_check_gender' is violated.
    (3, 'leess', '1234', '리순신', 'm', 20, '01012341234', 'leess@naver.com'); -- 소문자 m도 허용, 대소문자 상관없다. 하지만, table생성할 때 binary를 작성하면 대소문자 구분해야한다.
...
update
    user_check
set
    age = age - 20 -- [HY000][3819] Check constraint 'ck_user_check_age' is violated.
where
    user_no = 1;
