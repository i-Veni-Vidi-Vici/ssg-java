package com.sh._04_paging.order.model.service;

import com.sh._04_paging.order.model.dao.OrderMapper;
...
/**
    * 전파행위옵션 (propagation)
        REQUIRED(기본값) : 진행 중인 트랜젝션이 있으면 현재 메서드를 그 트랜젝션에서 실행하되 그렇지 않은 경우 새 트랜젝션을 시작해서 실행한다.
 ...
 *                  오염된 값(수정된 값) 읽기 문제는 해결되지만 재현 불가능한 읽기 및 허상읽기는 여전히 발생할 수 있다.
 ...
 *
 * @Transactional 메소드 안에서 던져진 예외가 RuntimeException 후손인 경우만, rollback처리한다. (기본값)
...
@Service
//@RequiredArgsConstructor // 이것을 사용하게 되면 아래와 같이 @Autowired어노테이션 하의 코드를 작성할 필요가 없다.
public class OrderService {
    /**
     * @Mapper 구현클래스 빈이 의존 주입되어 처리된다.
     * - SqlSessionTemplate#getMapper는 더이상 작성하지 않아도 된다.
     */
    private final OrderMapper orderMapper; // 의존 주입된 것임
    ...
    public int createOrder(OrderDto orderDto) {
        ...
        // 2. n건의 주문메뉴 등록
        List<OrderMenuDto> orderMenuDtos = orderDto.getOrderMenus();
        for(OrderMenuDto orderMenuDto : orderMenuDtos){
            // 방금 발급된 orderCode를 대입
            orderMenuDto.setOrderCode(orderDto.getOrderCode());
            result = createOrderMenu(orderMenuDto);
        };
        return result; // 0이 아닌 값만 넘어가면 된다.
    }
