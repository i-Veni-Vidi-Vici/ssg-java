...
create table if not exists product1(
    id int comment '상품식별번호', -- 컬럼명 주석 comment
    name varchar(10) comment '상품명',
    created_at datetime comment '등록일시'
) comment = '상품테이블1';  -- 테이블의 주석 comment

-- 코멘트 확인
-- information_schema DB객체에 대한 메타정보를 제공하는 스키마 (기본제공)
-- 테이블 코멘트
select TABLE_COMMENT
from information_schema.tables -- 스키마.테이블   테이블이름이 tables이다.
...
-- time 타입 : 기본값 current_time
-- datetime타입 : current_timestamp, now() 사용
-- 날짜/시간 관련 default는 괄호를 꼭 감싸준다.
create table tbl_rent (
    rent_id int auto_increment primary key,
    contractor_name varchar(50) default '익명',
    car_name varchar(50),
    rent_date date default (current_date), -- 괄호로 감싸줘야 한다.
...
select * from tbl_rent; -- 주어진 값이 없을 때 기본값으로 주어진다.
...
delete from product2 where id = 3; -- 3번을 삭제하면, 다시는 3번을 발행하지 않는다. 2번 다음 4번으로 발행됨
...
-- add : 컬럼/제약조건 추가
-- drop : 컬럼/제약조건 삭제
-- modify : 컬럼에 대한 자료형/not null/auto_increment/컬럼순서 변경
-- change : 컬럼명 변경
-- rename : 테이블명 변경
...
add price int default 1000 not null comment '가격' after name; -- 위치 : 자료형 기본값 not null comment 순서(first / after 컬럼명)
...
-- auto_increment추가는 modify 사용해야한다
alter table product3
modify id int auto_increment; -- 제약조건 추가하지 않고서는 auto_increment사용이 불가하다.
...
alter table product3
drop primary key; -- auto_increment제거 후 사용가능

alter table product3
modify id int; -- auto_increment 제거

-- alter문은 여러 개의 서브명령어를 동시에 사용가능
alter table product3
...
create table if not exists product4 ( -- if not exists를 사용 = 존재하지 않으면 만들어주세요 라는 의미
    id int,
    name varchar(10),
    created_at datetime
);
drop table if exists product4; -- if exists 존재한다면 삭제해주세요 라는 의미

-- -----------------------------------
-- TRUNCATE
-- -----------------------------------
-- 테이블의 모든 행을 잘라내는 DDL
-- where절 없는 delete(DML) 대비 속도가 빠르다. (DML의 before image작업이 없기 때문임)
...
truncate table product5; -- 모든 행 잘라냄. 새로 생성하면 id가 4부터 시작하는 것이 아니라 다시 1부터 시작한다.(노션참고)