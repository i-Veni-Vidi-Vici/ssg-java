 ...
 * - 방언(dialect)을 통해 해당 DBMS에 맞는 SQL을 실행하게 된다.
...
public class JpqlTest {
    ...
    @Test
    @DisplayName("TypedQuery 테스트 - Menu 테스트")
    void test1() {
        // given
        // 테이블 별칭을 선언하고, select절에서 사용해야 한다. *를 사용할 수 없다.
        String jpql = "select m from Menu01 as m"; // Menu01이 작성된 이유는 Menu클래스에서 엔티티 name을 작성했기 때문이다.
        // when
        TypedQuery<Menu> query = this.entityManager.createQuery(jpql, Menu.class); // Menu.class가 TypedQuery<>안에 있는 Menu와 같아야 한다.
        ...
        Hibernate:
            ...
            from
                tbl_menu m1_0
         */
        menus.forEach(System.out::println); // forEach로 한 건씩 찍을 때 사용하는 코드
        // then
    ...
    void test2() {
        // given
        // 조회하고자 하는 select절, where조건절에서도 table이 아닌 entity기준으로 조회한다.
        String jpql = "select m.menuName from Menu01 m where m.menuCode = 7";
        // when
        TypedQuery<String> query = this.entityManager.createQuery(jpql, String.class); // String을 뱉어내는 query객체이다
    ...
    void test4() {
        ...
        String jpql = """
                ...
                """;
        // when
        TypedQuery<Long> query = this.entityManager.createQuery(jpql, Long.class); // 타입을 결정하는 것은 select절에서 나오는 값을 보고 결정한다.
        List<Long> categoryCodes = query.getResultList();
        categoryCodes.forEach(System.out::println);
        /*
        null => null값도 하나의 값으로 취급한다.
        4
        5
        6
        8
