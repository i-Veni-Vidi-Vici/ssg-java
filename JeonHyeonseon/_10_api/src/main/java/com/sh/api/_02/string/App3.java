 ...
 * <pre>
 * CSV데이터
 *  - Comma Seperated Value 구분자(, / $ 등)로 구분된 값
...
//        String[] arr3 = data3.split("/"); // 마지막 요소 처리안됨
        String[] arr3 = data3.split("/", -1); // 요소개수 제한없음["300" "인순신" "경기도" )
        System.out.println(Arrays.toString(arr3));

        System.out.println(data3.split("/", 2)); // [300, 이순신, 경기도, ]]
        System.out.println();

        // 2. StringTokenizer -> 잘려진 문자열 = token
        // CSV데이터를 토큰으로 잘라내어 관리하는 객체
        StringTokenizer tokenizer = new StringTokenizer(data1, "/");
        System.out.println(tokenizer); // java.util.StringTokenizer@7a81197d
        // 반복처리릍 통해서 토큰을 열람(소비)
        System.out.println("현재 토큰수 : " + tokenizer.countTokens());
        while (tokenizer.hasMoreTokens()) {  // 다음 토큰이 있는지 물어보는 것
