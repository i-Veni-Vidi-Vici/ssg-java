...
#      menu_name, -- group by와 연관이 없는 일반 컬럼은 사용할 수 없다.
     -- Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'menudb.tbl_menu.menu_name' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
    category_code,
    count(*),
    sum(menu_price),
    avg(menu_price),
...
-- 2개의 컬럼값을 가지고 그루핑을 진행한다. (이때 컬럼순서는 상관없다.)
-- 21행 -> 21그룹
select
...
-- 카테고리별로 그룹을 나누었을 때 메뉴개수가 1개보다 많은 그룹을 조회
select
    category_code,
...
-- 카테고리, 가격으로 그룹 조회
-- with rollup : group by category행 추가, 모든 행을 한 그룹으로 처리했을 때, 행 추가
