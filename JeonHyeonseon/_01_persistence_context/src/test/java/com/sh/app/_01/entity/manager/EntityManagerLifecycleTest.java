 ...
 * thread-safe하기 때문에 여러 스레드가 동시에 접근해도 안전하므로 서로 다른 스레드 간 공유해서 재사용한다. 상대값을 가지지 않는다.
...
public class EntityManagerLifecycleTest {
    ...
    // 각각의 테스트를 하나의 요청이라고 생각해야 한다.
    @Test
    @DisplayName("EntityManagerFactory는 공유, EntityManager는 각자 사용")
    void test1() {
        System.out.println(entityManagerFactory.hashCode()); // 1182469998
        System.out.println(entityManager.hashCode()); // 1195473402
    }
    ...
    void test2() {
        System.out.println(entityManagerFactory.hashCode()); // 1182469998
        System.out.println(entityManager.hashCode()); // 400593724
    }
}
