 ...
 * - 이를 응답받은 클라이언트(브라우저)는 응답해더에 작성된 Location값으로 즉시 재요청을 보낸다.
 */
...
//        response.sendRedirect("https://naver.com"); // Location 주소값 설정
        ...
        // 만약 요청간의 데이터 등을 공유하려면, Session객체를 이용해야 한다.
...
//        response.sendRedirect("another"); // 상대주소 (/07_redirect_response/redirect -> /07_redirect_response/another)
        response.sendRedirect(request.getContextPath() + "another"); // 절대주소 (브라우저에서 요청할 주소이므로 application context를 포함해야 한다.)
    }
}