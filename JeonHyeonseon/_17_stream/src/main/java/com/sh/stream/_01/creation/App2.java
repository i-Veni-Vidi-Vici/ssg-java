 ...
 * 스트림 연산(3가지)
 * 1. 스트림 생성(1번)
 * 2. 스트림 중간 연산(n번) - sorted
 * 3. 스트림 단말 연산(1번) - forEach
 * 사용한 스트림은 재사용할 수 없다.
 * </pre>
 */
public class App2 {
...
//        stream1.forEach((s) -> System.out.println(s + " ")); // s + " " 가공한 식이기에 띄어쓰기는 불가능하다.

        Stream<String> stream2 = Arrays.stream(names, 0, 2);// 0번지에서 2번지 전까지만 stream으로 출력
        ...
        // builder가 제네릭 메소드이므로 타입변수 선언할 것
