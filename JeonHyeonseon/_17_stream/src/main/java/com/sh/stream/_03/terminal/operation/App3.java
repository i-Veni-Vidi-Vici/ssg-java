 ...
 * <pre>
 * collect 단말연산
 *  - 스트림의 연산결과를 리스트/셋/맵/String으로 변환하는 작업이다.
...
public class App3 {
    public static void main(String[] args) {
        ...
        List<Integer> intList3 = intList.stream()
                .mapToInt((n) -> n * 10)  // Stream
                .boxed()
                .collect(Collectors.toList());


        Set<Integer> intSet = List.of(1, 2, 3, 4, 2, 3, 2, 4, 5).stream()
        ...
        // 배열 변환
        Object[] arr = List.of(1, 2, 3, 4, 2, 3, 2, 4, 5).stream().toArray(); // Object[]로 나타난다.
        System.out.println(Arrays.toString(arr));

        // 문자열 조합
        String result = List.of("홍길동", "신사임당", "홍길동", "이순신", "홍길동").stream()
                .collect(Collectors.joining(" | ", "<", ">")); // .joining() : 구분자, 접두사, 접미사
