...
public class IndexController {
     ...
     * ViewResolver빈
     *  - Resolver = ~~을 처리하다
     *  - controller에서 반환받은 viewName을 가지고 view단을 선정하는 작업을 의미한다.
     *  - thymleaf를 사용시에는 ThymeleafViewResolver빈을 사용한다.
     *  - prefix로 "resources/templates", suffix(접미사)를 ".html"로 추가하여 viewName을 완성한다.
     */
    ...
    public String stringRedirect(Model model) {
        ...
        // 302응답 Location=/app/
//        return "redirect:/"; // Location=http://localhost:8080/app/
        return "redirect:http://naver.com"; // 위의 코드로 작성해도 가능하다.
    }
    ...
    public String stringRedirectFlash(RedirectAttributes redirectAttributes) {
        ...
        // 이번 요청에 생성된 데이터
        redirectAttributes.addFlashAttribute("flashMessage", "🍨🍨🍨"); // 1회용
        return "redirect:/";
    }
    ...
    public String stringRedirectParam(RedirectAttributes redirectAttributes) {
        log.info("GET /string-redirect-flash");
        // 이번 요청에 생성된 데이터
        redirectAttributes.addFlashAttribute("paramMessage", "🍑🍒🍍"); // 1회용
