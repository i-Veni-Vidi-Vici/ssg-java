...
//        return "first/regist"; // 이건 viewName이다. src/main/resources/templates/first/regist.html
//    }

    public void regist(){} // 이렇게만 작성해도 잘 작동된다. 이유는 /first/regist path에서 viewName을 유추하기 때문이다.

    /**
     * WebRequest
     *  - 스프링버전의 HttpServletRequest객체이다.
    ...
    public String regist(Model model, WebRequest request){
        ...
        log.debug("name = {}", name); // 값이 잘 나왔는지 체크하는 용도
        log.debug("price = {}", price); // 값이 잘 나왔는지 체크하는 용도
        log.debug("categoryCode = {}", categoryCode); // 값이 잘 나왔는지 체크하는 용도
     ...
     *  - name값과 변수명이 다르다면, 실제로 name값을 직접 명시할 수 있다.
     *  - required=true(기본값)이므로, 해당 name속성을 찾을 수 없다면, 400오류가 난다.
     *  - required=false로 변경하면, 해당 name값이 없어도 오류가 나지 않는다.
     *      - 이때, defaultValue값이 지정되어 있다면, 해당값을 사용한다.
     */
    @PostMapping("/modify")
    ...
    /**
     * Command(커맨드)객체
     *  - 사용자 요청의 name값과 일치하는 객체의 필드에 값을 대입해준다.
     *
     * @ModelAttribute
     *  - 모델 속성에 대한 getter 어노테이션이다.
     *  - Command객체는 이미 model에 등록되어 있다.
     *  - 이때 모델 속성명을 변경할 수도 있다. (기본값은 소문자로 시작하는 클래스명이다)
     */
    @PostMapping("/search")
    ...
    /**
     * @SessionAttributes로 등록된 session속성을 폐기할 때는 기존 HttpSession#invalidate()를 사용할 수 없다.
     * SessionStatus객체(@SessionAttributes로 등록된 속성을 관리하는 객체가 따로 있다.)의 setComplete()를 호출해 만료시킬 수 있다.
     *  - 속성을 개별적으로 삭제시킬 수는 없다.
     *
     * @return
     */
    @GetMapping("/logout2")
    public String logout2(SessionStatus sessionStatus) { // 너가 관리하는 sessionStatus 좀 줘봐
    ...
    @GetMapping("/etc")
    public String etc(@RequestHeader("User-agent") String userAgent, @CookieValue(name = "JSESSIONID", required = true) String jSessionId){
