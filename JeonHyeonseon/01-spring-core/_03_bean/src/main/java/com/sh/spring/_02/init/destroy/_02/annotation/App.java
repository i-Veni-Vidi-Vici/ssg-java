 ...
 * - globalSession(어플리케이션)  전역 세션 당 하나의 인스턴스를 생성하고, 전역 세션이 종료되면 인스턴스를 폐기한다. 포털 애플리케이션 컨텍스트에만 해당된다.
 * </pre>
 */
...
public class App {
    public static void main(String[] args) {
        ...
        // scope속성이 prototype인 경우, 해당빈은 ApplicationContext 초기화시 만들지 않고,
        ...
        // first객체와 second객체가 같기 때문에(BeanScope가 하나이기 때문) second를 출력할 때 carpBread와 milk에 대한 정보도 같이 출력된다.
        ...
        // bean의 destroyMethod를 명시적으로 호출하기 위해서는 process 종료전에 context를 종료하면 된다.
        // stream계열의 빈에서 자원반환을 위해 destroyMethod를 사용한다.
        ((AnnotationConfigApplicationContext)context).close(); // destroy 메소드 호출
    }
}
