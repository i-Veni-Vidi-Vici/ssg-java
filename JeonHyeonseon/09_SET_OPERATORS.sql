...
-- mysql에서는 지원하지 않는다.
-- 1. inner join으로 우회적으로 구현
-- 2. in연산자 이용해서 우회적으로 구현
...
from
    tbl_sales
where
#     year(sale_date) = month(curdate() - interval 2 month)
...
from
    tbl_sales
where
#     year(sale_date) = month(curdate() - interval 1 month)
...
-- 202403 판매데이터 테이블 생성
-- 테이블 삭제할 경우, drop table tbl_sales_202403;
