...
@Data
@Setter(AccessLevel.PRIVATE) // 외부에서 값을 임의로 변경하는 것을 방지한다.
@NoArgsConstructor
public class Order {
    private Long id; // OrderEntity의 식별자
    private Orderer orderer; // 주문자
    private DeliveryInfo deliveryInfo; // 배송정보
    ...
    public void changeDeliveryAddress(Address newAddress) {
        if (this.orderStatus == OrderStatus.주문확인중 || this.orderStatus == OrderStatus.배송준비중) {
            ...
            throw new IllegalStateException("주문상품이 이미 발송되어서, 배송지 정보를 수정할 수 없습니다.");
        }
    }
}
