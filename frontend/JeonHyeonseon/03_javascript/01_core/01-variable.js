...
// var (직접 사용할 필요없다. 레거시브라우저 호환용. 사용금지)
const n = 123;
console.log(n);
...
const str1 = "홍길동";
const str2 = "신사임당";
const str3 = "이순신"; // ES6(ES2015) 추가된 Template String 문법 : ``안에서 개행/탭 문자를 인식. ${}표현식을 통해 다른 변수도 출력가능
 ...
 * 5. null
 *   - 값없음을 의미하는 타입이자 값
 *   - 초기화된 값이 없음을 의미하는 undefined와 존재할 값이 없는 null은 다름으로 유의해서 써야한다.
 *   - 기존값을 제거하는 경우 반드시 undefined가 아닌 null을 사용해야 한다.
 */
let x = 13;
x = null;
console.log(x, typeof x); // null object (js 초기부터 object타입에 대한 type tag와 실제값인 0이 동일하게 생긴 이슈)
...
/**
 * 8. object
 *   - js자료형 중 유일한 객체형 (나머지 7가지는 모두 원시형)
 *   - 속성/메소드를 가진 자료형
 * 
 * - object의 하위타입
 *   - object
...
/**
 * 자바 스크립트는 동적타입언어이다.
 *  - 정적타입언어 (c, c++, java, ...) 변수에 자료형이 고정된 언어. 컴파일타임에 타입체크가 가능하므로, 런타임오류를 줄일 수 있다.
 *  - 동적타입언어 (js, pythonm, ...) 
