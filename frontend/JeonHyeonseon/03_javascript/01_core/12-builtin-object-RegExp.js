...
sass
xxx`;
console.log(target.replace(/^j/gim, '<$&>')); // <J>avascript <j>Qeury Ajax sass xxx
console.log(target.replace(/x$/gim, '<$&>')); // Javascript jQeury Aja<x> sass xx<x>
...
console.log(re.test('acbbbbbbbbbbb')); // true
...
console.log(target.replace(/[A-Z]/g, '<$&>')); // 영문자 검색이 아님 (주의하기) <A><B><C><D><E><F><G>hijklmn 가나다라마바사ㅋㅋㅎㅎㅓㅓㅏㅏ 1234567890 !@#$%^&*() 
...
console.log(target.replace(/[A-Z^E]/g, '<$&>')); // X E가 아닌 영문자 찾기 실패. ^은 맨 앞에 사용할 때만 반전의 의미를 갖는다. 
 ...
 * 
 * \W \w 반전(영문자/숫자/_ 가 아닌 것)
 * \D \d 반전(숫자가 아닌 것)
 * \S \s 반전(공백/탭/개행문자가 아닌 것)
 */
...
console.log(target.replace(/(수요일|목요일|금요일)/g, "<$&>")); // 요일에는 월요병, 화가 부르르르 화요일 홧병, 수수술한잔 하는 <수요일>, 목이 컥~ <목요일>, 금방 오지 않는 <금요일>, 하지만 오늘은 <금요일>
console.log(target.replace(/[수목금]요일/g, "<$&>")); // 월요일에는 월요병, 화가 부르르르 화요일 홧병, 수수술한잔 하는 <수요일>, 목이 컥~ <목요일>, 금방 오지 않는 <금요일>, 하지만 오늘은 <금요일>
console.log();

/**
 * Escaping
 * . ^ $ | [] 정규식에서 특수한 의미가 있는 문자를 문자그대로 찾고 싶은 경우 escaping이 필요하다.
 ...
 * 
 * - Look Ahead : 전방탐색
 *      a(?=b) b가 뒤따르는 a를 검사(찾는 건 a인데 뒤에 b가 있는지 없는지 검사)
 *      a(?!b) b다 뒤따르지 않는 a를 검사
 * 
 * - Look behind : 후방탐색
 *      (?<=b)a b가 앞서있는 a를 검사
