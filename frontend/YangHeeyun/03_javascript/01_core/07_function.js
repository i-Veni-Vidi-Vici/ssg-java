...
/*
나머지 파라미터 Rest Parameter : 매개변수부에서 복수개의 값을 처리할 수 있는 변수, 맨 마지막에 한번만 사용하기
전개연산자 Spread Operator : 배열의 요소를 뿌려서 전달(매개인자/값)
파라미터 기본값 : 매개인자가 존재하지 않는 경우(undefined) 지정한 기본값 사용가능
*/

// 일일이 매개변수를 작성하는 불편함이 있어서 ...chars로 통일하면 편하다
...
const arr = [1,2,3];
test2(arr[0],arr[1],arr[2]); // 1 2 3 일일이 쓰는 거 귀찮아서 발전한게 전개연산
test2(...arr); // 1 2 3

// 전개연산자는 여러개 나열이 가능하다.
const arr1 = ['🚗'];
const arr2 = ['🚑', '🚒'];
test2(...arr1, ...arr2); // 🚗 🚑 🚒 매개인자가 부족하면 부족한 칸에는 undefined로 나온다 ex) 🚗 🚑 undefined
...
test3(100,undefined,200); // 100 8888 undefined
...
const hello = function(){
    // return 123;
    return; // default로 return undefined 반환
}
const result = hello();
...
// const myArrow = function(){ // 익명함수(X)
//   return a + b;
// };

// 1. function 키워드를 생략하고, =>를 사용해 매개변수부와 몸통부 연결
// 2. 몸통부에 {return;}절을 생략 (return구문 한줄만 존재할때)
// 3. 몸통부에 {} 생략 (실행코드 한줄인 경우)
// 4. 매개변수가 1개인 경우, 소괄호 생략가능 (가독성이 좋지않아 사용 비추천)
const myArrow = (a,b) =>  a + b; // 익명함수(O)
...
/**
 * 자바스크립트에서 함수는 일급객체이다. (일급객체란? 값으로써 취급하는것)
 * 1. 변수에 대입 가능해야 한다. 
 * 2. 함수호출시 전달이 가능해야 한다. 
...
/*
호출했냐 안했냐의 차이
const carFactory = carFactoryBuilder; // 객체 주소를 주겠다. 함수 그자체를 준다
const carFactory = () => {
  return () => console.log('🚙');
};

const carFactory = carFactoryBuilder(); // 호출한 함수의 결과를 주겠다 return 뒤에 값을 주겠다
...
// 경찰차 factory를 생산해서(함수) 10대 출력하세요.
const carFactoryBuilder3 = (car) => {
  return (n) => {
    for(let i=0; i<n; i++)
      console.log(car,i);
  };
...
const policecarFactory = carFactoryBuilder3('🚔');
policecarFactory(10);