 ...
 */
@Data
@Setter(AccessLevel.PRIVATE)    //외부에서 값을 변경하는 것을 방지
@NoArgsConstructor
...
public class Order {
    ...
    public Order(Long id, Orderer orderer, DeliveryInfo deliveryInfo, List<OrderLine> orderLines, int totalPrice, OrderStatus orderStatus, LocalDateTime orderDateTime) {
        this.id = id;
        this.orderer = orderer;
        this.deliveryInfo = deliveryInfo;
        //주문 내역 설정과 총 결제금액 계산은 setter에서 일관되게 작업 되어야 한다
    ...
    public void changeDeliveryAddress(Address newAddress) {
        if(this.orderStatus == OrderStatus.주문확인중 || this.orderStatus == OrderStatus.배송중준비중){
