...
public class App {
    public static void main(String[] args) {
        ...
        Order order = new Order(
                ...
                OrderStatus.주문확인중,
                LocalDateTime.now());
        //order.setTotalPrice(30000);   //Entity 객체 하위 정보를 임의로 수정해서 안된다.
        System.out.println(order);

        //주문을 취소하라
        //배송지 주소를 변경하라 order > deliveryInfo >address
        //1. order 배송지 주소를 변경하라
        //2. order Address를 알지 못하므로, DeliveryInfo에게 배송지 주소를 변경해라
        //3. DeliveryInfo가 Address를 알고 있으므로, 직접 Address 객체를 변경
//        order.getDeliveryInfo().setAddress(new Address("미국 뉴욕주 뉴욕시", "브롱크스가 123", "99999"));
        order.changeDeliveryAddress(new Address("미국 뉴욕주 뉴욕시", "브롱크스가 123", "8989"));
