 ...
 * - 타입을 가지지 않고, 개수 제한 없이 배열을 제어할 수 있다. (실제로는 하나의 타입 요소를 관리)
 * 
 */
...
//length속성 enumerable: false 이므로 처리되지 않는다
console.log("--------for....in--------");
...
/**
 * 배열 메소드
 * - Array.prototype객체 (Array 부모객체)에 선언된 메소드를 자식 배열객체에서는 선언없이 이용가능
 */
...
/**
 * arr1.concat(arr2) : 두 배열의 요소를 합친 새 배열을 반환
 */
const chars1 = ['가', '나', '다'];
const chars2 = ['a', 'b', 'c'];
...
/**
 * slice(startIndex, endIndex) : startIndex부터 endIndex전까지 요소를 새 배열에 반환
 */
...
/**
 * splice(startIndex, delCount, newItem1, newItem2, ...) : 삭제와 요소 추가를 동시에 하는 메소드
 * 
 */
 ...
 * toReversed() : 순서가 뒤집힌 새 배열을 반환한다 (immutable 방식)
 */
