...
public class RequestParameterServlet extends HttpServlet {
     ...
     * - 위 URL에서 ?name=홍길동&age=33&married=true 부분이 Query String에 해당된다.
     * - 쿼리 스트링은 request 객체 내부에서 HashMap으로 관리된다.
     * - HttpServletRequest#getParamether(name):String로 처리할 수 있다.
     * - 모든 사용자 입력값은 문자열로써 처리된다.
     * - 단순조회 요청은 전송방식 GET을 사용한다.
     * -
     * @param req
     * @param resp
    ...
    /**
     * Post요청에서 사용자 입력값은 http 요청 메세지의 본문 영역에 작성된다 (url에 노출되지 않는다)
