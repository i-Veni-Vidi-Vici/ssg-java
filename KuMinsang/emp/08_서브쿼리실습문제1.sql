...
select
    ...
    e.SALARY
from
    employee e join (select * from department where DEPT_TITLE like '%영업%') d
...
select
    ...
    n.NATIONAL_NAME 국가명
from
    department d join location l on d.LOCATION_ID = l.LOCAL_CODE
...
select e.EMP_ID 사원번호,
       ...
       d.국가명,
       e.SAL_LEVEL,
       e.salary + (select MIN_SAL from sal_grade where e.SAL_LEVEL = SAL_LEVEL)
from employee e
         left join (select d.DEPT_ID       부서코드,
                           d.DEPT_TITLE    부서명,
                           l.LOCAL_NAME    지역명,
                           n.NATIONAL_NAME 국가명
                    from department d
                             join location l on d.LOCATION_ID = l.LOCAL_CODE
                             left join nation n on l.NATIONAL_CODE = n.NATIONAL_CODE) d on e.DEPT_CODE = d.부서코드
where 국가명 = '러시아'
order by 국가명 desc;