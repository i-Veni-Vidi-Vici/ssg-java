 ...
 *     - 요리사, 손님 = 쓰레드
 *     - 접시 : 공유자원
 *     - 동기화 처리를 하지 않는다면, 제공하지 않은 빵을 손님이 먹거나, 빵을 2개 이상 준비하는 등의 오류 발생
 *
 *     쓰레드의 상태관리 메소드
 *     wait()   WAITING 상태로 변환
 *     notify(), notifyAll() WAITING상태의 임의의 쓰레드를 RUNNABLE로 변환
 *
 *     구현해야할 동기화 기능
 *     - 손님은 접시가 비어있을 때 요리사가 빵을 만들길(notify) 기다려야 한다(waiting);
 *     - 요리사는 접시가 비어있지 않을 때 손님이 먹길(notify) 기다려야 한다. (waiting)
 *     - waiting 중에 notify를 받으면, 블럭된 다음 부분을 실행한다
 * </pre>
 */
public class App {
