...
/**
 * Cookie
 * - client에 데이터를 보관하는 기술
 * - 이용순서
 * 1. 서버에서 쿠키를 생성
 * 2. 응답헤더 Set-cookie에 쿠키 전송
 * 3. 브라우저는 해당 쿠키를 저장
 * 4. 클라이언트 요청시에 저장된 쿠키를 함께 전송한다. (이떄 요청헤더 Cookie)
 */
...
public class CookieServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //쿠키 생성
        Cookie cookie = new Cookie("cookiename", username);   //인자가 String::CookieName, String::value
        //쿠키 유효기간 (브라우저에 보관할 기간. 초단위 작성)
